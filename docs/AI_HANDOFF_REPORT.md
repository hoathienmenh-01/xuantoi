...
- **Current `main` commit**: `520249f` (Merge PR #217 feat(shared): mission pack 1 (+54 mission, ngũ hành chain quest)).
- **Latest merged PR**: [#217 feat(shared): mission pack 1 — rebased onto main `ba1443c` + audit fix](https://github.com/hoathienmenh-01/xuantoi/pull/217) — `packages/shared/src/missions.ts` 12 → 66 mission (+54: 11 daily T2/T3/T4 + 11 weekly element-themed + 32 once milestone/chain), MissionDef forward-compat extend, +27 vitest, 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track**: PR #221..#228 (8/8 catalog foundation) MERGED. **This PR** = **Phase 11.8.A Buff/Debuff catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2519 vitest** = api **1133** + shared **798** + web **588** (post-#217 merge). **This PR** thêm **+56 vitest** (shared 798 → 854) → total **2575/2575** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **11.0 (#221) / 11.1.A (#222) / 11.2.A (#223) / 11.4.A (#224) / 11.5.A (#225) / 11.6.A (#226) / 11.X.A (#227) / 11.7.A (#228) MERGED**. **PR 11.8.A (this PR open)** Buff/Debuff catalog foundation — `packages/shared/src/buffs.ts` NEW + 56 vitest, 18 buff/debuff baseline (10 buff + 8 debuff), 5 element coverage `kim/moc/thuy/hoa/tho` + 5 neutral, schema `BuffDef { key, polarity, element, source, durationSec, stackable, maxStacks, dispellable, effects[] }`, 8 source `pill/skill/sect_aura/event/gear/talent/boss_skill/tribulation`, 10 effect kind `stat_mod/regen/damage_bonus/damage_reduction/control/dot/shield/taunt/invuln/cultivation_block`, helpers `getBuffDef/buffsByPolarity/Element/Source/EffectKind/composeBuffMods/computeBuffExpiresAt/isBuffExpired` deterministic. **PR 11.1.B/11.2.B/11.3/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B/11.8.B (Pending)** runtime hookups tương ứng với từng catalog foundation đã mở. Phase 11 catalog track = **9/9 DONE** sau khi merge this PR.
- **Open PRs**:
  1. **this PR** — `feat(shared): buff / debuff catalog foundation (Phase 11.8.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.8.A Buff/Debuff catalog) → Phase 11 catalog foundation track = 9/9 DONE.
  ...
  4. **Phase 11.7.B + 11.8.B PR runtime**: Prisma model `CharacterTalent { characterId, talentKey, learnedAt }` + `CharacterBuff { id, characterId, buffKey, stacks, source, expiresAt }` + service learn/use talent + apply/prune buff + wire `composePassiveTalentMods` + `composeBuffMods` vào `CharacterStatService.computeStats`.
