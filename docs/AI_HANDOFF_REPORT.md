...
- **Current `main` commit**: `22c9c0c` post PR #264 (Phase 11.X.O Buff control wire). PR #246..#264 MERGED. **PR pending (this session 9r-26 part 2)**: **Phase 11.X.N Buff shield wire vào CombatService.action()** — wire `buffMods.shieldHpMaxRatio` damage absorb monster reply pre-charHp-loss. `shieldAbsorb = floor(char.hpMax × shieldHpMaxRatio)` re-applied mỗi turn buff active (per-turn refresh aura model). Catalog `talent_shield_phong` (kind=shield value=0.3 hpMax, durationSec=10) — trước đó shieldHpMaxRatio compute nhưng KHÔNG consume runtime, shield buff de facto no-op. 5 vitest mới: shield + low reply (full absorb), shield > reply (full absorb), no shield identity, BuffService not injected identity, shield + DOT (shield chỉ absorb reply không absorb DOT). Branch `devin/1777725565-phase-11-x-n-shield-combat`. Test baseline post-PR-#264: **2952 vitest** (1410 api + 954 shared + 588 web). Post-this-PR target: **2957 vitest** (1415 api).
- **Latest merged PR**: [#264 feat(combat,character): Buff control (root/stun/silence) wire vào CombatService.action() (Phase 11.X.O)](https://github.com/hoathienmenh-01/xuantoi/pull/264) — Wire `buffMods.controlTurnsMax > 0` throw `CombatError('CONTROLLED')` ngay sau khi compose buffMods, TRƯỚC mọi state mutation. Catalog `debuff_root_thuy` (3 turns), `debuff_stun_tho` (1 turn), `debuff_silence_kim` (2 turns). 5 vitest. Main bumped `a70c733` → `22c9c0c`. CI 5/5 GREEN.
