...
- **Current `main` commit**: `35db2d5` post PR #267 (Phase 11.7.E Talent regen wire). PR #246..#267 MERGED. **PR pending (this session 9r-26 part 5)**: PR #268 (Phase 11.X.Q Buff control boss) + PR #269 chained (this PR). **PR #269**: **feat(boss,character): Buff cultivationBlocked wire vào BossService.attack() (Phase 11.X.R)** — wire `buffMods.cultivationBlocked` (Tâm Ma debuff_taoma) throw `BossError('CULTIVATION_BLOCKED')` BEFORE state mutation. Tâm Ma'd char đã bị block tu luyện EXP (Phase 11.8.D wire ở CultivationProcessor) giờ cũng bị block boss attack — semantically nhất quán. Cùng lần `getMods` với Phase 11.X.Q control check. Add `'CULTIVATION_BLOCKED'` vào `BossError` codes. Map → HTTP 409. 3 vitest mới. Branch chained from PR #268 branch (cùng BuffService DI). Test baseline post-PR-#267 = 2962 → post-PR-#268 = 2967 → post-this-PR = **2970 vitest** (1428 api + 954 shared + 588 web).
- **PR pending (this session 9r-26 part 5, prev)**: **feat(boss,character): Buff control wire vào BossService.attack() (Phase 11.X.Q)** — parallel to Phase 11.X.O combat wire, throw `BossError('CONTROLLED')` BEFORE state mutation. Catalog `debuff_root_thuy`/`debuff_stun_tho`/`debuff_silence_kim`. Inject `@Optional() buffs?: BuffService`. 5 vitest mới. Branch `devin/1777727566-phase-11-x-q-boss-control`. PR #268.
- **Latest merged PR**: [#267 feat(cultivation,character): Talent hp/mpRegenFlat wire vào CultivationProcessor (Phase 11.7.E)](https://github.com/hoathienmenh-01/xuantoi/pull/267) — Wire talent regen additively với buff regen ở cultivation tick. Refactor `talentMods` fetch ONCE per tick share với expMul. Catalog `talent_moc_linh_quy` (5 HP/sec). 5 vitest. Main bumped `411912a` → `35db2d5`. CI 5/5 GREEN.
