...
- **Current `main` commit**: `a2721b1` post PR #242 (Phase 11.6.B Tribulation MVP runtime). PR #243 (Phase 11.8.B Buff MVP runtime) IN FLIGHT — branch `devin/1777697442-phase-11-8-b-buff-mvp-runtime`.
- **Pending PR #243**: Phase 11.8.B Buff/Debuff MVP runtime — duration-based persistence service. New `BuffService` (`applyBuff`/`removeBuff`/`listActive`/`pruneExpired`/`getMods`) qua `prisma.$transaction` + new model `CharacterBuff` (composite unique `(characterId, buffKey)` + 2 indexes) + wire `composeBuffMods` từ `packages/shared/src/buffs.ts` (Phase 11.8.A catalog đã merge). MVP đổ sang persistence + `getMods()` helper; KHÔNG wire vào `CharacterStatService.computeStats` / `breakthrough()` / `tribulation()` yết (defer Phase 11.8.C cho safety). 16 vitest cover apply (new/refresh/stack cap), remove (idempotent), listActive (auto-prune), pruneExpired (per-character + global), getMods (`composeBuffMods` parity), error class, cross-character isolation.
- **Latest merged PR**: [#242 feat(character): Tribulation/Tâm Ma MVP runtime](https://github.com/hoathienmenh-01/xuantoi/pull/242) — Phase 11.6.B server-authoritative deterministic kiếp. `TribulationService.attemptTribulation(charId, rng?, now?)` qua `prisma.$transaction` + new model `TribulationAttemptLog` + 2 nullable Character fields (`tribulationCooldownAt`/`taoMaUntil`) + `CurrencyLedger` reason `TRIBULATION_REWARD` + `POST /character/tribulation` endpoint + 15 vitest. Main bumped `76a5061` → `a2721b1`.
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** (Cultivation Method + Skill Template + Linh căn runtime đầy đủ) ✅. **Phase 11.4.B MERGED** (Gem MVP runtime — PR #239) ✅. **Phase 11.5.B MERGED** (Refine MVP runtime — PR #240) ✅. **Phase 11.6.B MERGED** (Tribulation MVP runtime — PR #242) ✅. **Phase 11.8.B IN FLIGHT** (Buff persistence MVP — PR #243, this PR). **NEXT runtime track** (theo catalog foundation đã mở): 11.7.B Talent runtime, 11.8.C Buff wire (compose `getMods()` vào `CharacterStatService.computeStats` + tribulation FAIL applyBuff), 11.9.B Title runtime, 11.10.B Achievement runtime, 11.3.D UI Linh căn (reroll dialog + element wheel), 11.1.C/11.2.C/11.5.C/11.6.C UI cultivation method + skill book + refine + tribulation.
- **Current test baseline (main `a2721b1` + PR #243 in flight)**: **~2824 vitest** = api **1282** (PR #243 +16 BuffService over baseline 1266) + shared **954** + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN trên main per CI run của PR #242. **Local verified PR #243 staging**: typecheck ✓ (api+shared+web no error), lint ✓ (zero warnings), shared `pnpm test` 954/954 ✓, web `pnpm test` 588/588 ✓, build ✓. **API test baseline 1282 cần Postgres** → CI verify (rely on PR #243 CI). **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
