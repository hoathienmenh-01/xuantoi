...
- **Current `main` commit**: `1e3124e` (Merge PR #235 feat(character,cultivation): Cultivation Method runtime — learn/equip + onboard auto-grant + processor compose (Phase 11.1.B)).
- **Latest merged PR**: [#235 feat(character,cultivation): Cultivation Method runtime — Phase 11.1.B](https://github.com/hoathienmenh-01/xuantoi/pull/235) — Prisma `Character.equippedCultivationMethodKey` + `CharacterCultivationMethod` model + `CultivationMethodService` (learn/equip/getState/grantStarterIfMissing) + `CultivationProcessor` compose `methodMul × cultivationMul × baseGain` + onboard auto-grant + 2 controller endpoint. +29 vitest. Backward-compat preserved.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.3 PR-A/B/C MERGED** (Linh căn runtime đầy đủ) ✅. **Phase 11.1.B MERGED** (Cultivation Method runtime — PR #235) ✅. **This PR** = **Phase 11.2.B — Skill Template runtime hookup**: Prisma `CharacterSkill` model + `CharacterSkillService` (learn/upgradeMastery/equip/unequip/getState/grantStarter/getEffectiveSkillFor) + wire `applyMasteryEffect` vào `CombatService.action()` (replace direct `skill.atkScale`/`skill.mpCost`) + onboard auto-grant + auto-equip starter `basic_attack` + 4 controller endpoints (skill state/equip/unequip/upgrade-mastery).
- **Current test baseline (main)**: **2724 vitest** = api **1186** + shared **950** + web **588** (post-#235 merge). **This PR** thêm vitest API (27 character-skill service + 4 combat mastery wire + 3 character.service onboard hook = +34) → total ~**2758** sau merge (api ~1220 + shared 950 + web 588). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN local. **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — fails on main `1e3124e` cũng flake — NOT caused by this PR; passes on rerun).
...
- **Phase 11 progress**: **11.0..11.10.A catalog 11/11 MERGED** ✅. **11.3.A** (#232 SpiritualRootService runtime + Prisma migration) MERGED ✅. **11.3.B** (#233 element multiplier wire CombatService) MERGED ✅. **11.3.C** (#234 cultivationMultiplier + statBonusPercent wire) MERGED ✅. **11.1.B** (#235 Cultivation Method runtime + processor compose + onboard hook) MERGED ✅. **THIS PR = Phase 11.2.B — Skill Template runtime hookup**: see Snapshots block below for full details — Prisma `CharacterSkill` model + `CharacterSkillService` (learn/upgradeMastery/equip/unequip/getState/grantStarter/getEffectiveSkillFor) + wire `applyMasteryEffect` vào `CombatService.action()` + onboard auto-grant `basic_attack` + 4 controller endpoints + 34 vitest. **PR 11.3.D/11.4.B/11.5.B/11.6.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups còn lại (gem socket/refine/tribulation/talent/buff/title/achievement) theo catalog foundation đã mở.
- **Open PRs**:
  1. **this PR** — `feat(character,combat): Skill Template runtime — learn/upgradeMastery/equip + onboard auto-grant + applyMasteryEffect wire (Phase 11.2.B)`.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.2.B) → skill mastery ảnh hưởng damage/MP cost trong combat (compose với linh căn statBonus + Ngũ Hành element multiplier).
  ...
  3. **Phase 11.1.C**: UI character profile display equipped method + list learned + equip switcher (call `POST /character/cultivation-method/equip`). Optional 24h cooldown anti-spam re-equip.
  4. **Phase 11.3 PR-D**: UI character profile display Linh căn (icon + grade tooltip + element wheel) + reroll service consume `linh_can_dan` ItemLedger + insert log `source='reroll'`.
  5. **Phase 11.7.B + 11.8.B + 11.9.B + 11.10.B PR runtime** (compose stack): Prisma `CharacterTalent` + `CharacterBuff` + `CharacterTitleUnlock` + `CharacterAchievement { id, characterId, achievementKey, progress, completedAt? }` + service `incrementAchievement` (idempotent) + `claimAchievement` (validate `completedAt!=null` + grant reward + auto-`unlockTitle(rewardTitleKey)`) + event listener cho mỗi `goalKind` (KILL_MONSTER / CLEAR_DUNGEON / BOSS_HIT / BREAKTHROUGH / GAIN_EXP / CULTIVATE_SECONDS / BUY_LISTING / SELL_LISTING / CHAT_MESSAGE / SECT_CONTRIBUTE).
  6. **Optional smoke gap polish**: `smoke:cultivation` / `smoke:topup` / `smoke:admin` v2.
