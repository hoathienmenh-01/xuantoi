...
- **Current `main` commit**: `7244e6f` post PR #261 (docs audit refresh session 9r-25 part 2 close-out). PR #246..#261 MERGED. **PR pending (this session)**: **Phase 11.X.M Buff DOT (debuff_burn_hoa, debuff_poison_moc) wire vào CombatService.action() end-of-turn HP loss** — wire `buffs.getMods().dotPerTickFlat` (đã tính theo stack ở composeBuffMods) cộng damage cuối lượt cho encounter còn ACTIVE (đã không WON/LOST). Catalog `debuff_burn_hoa` (8 dmg × stack, hoa skill) + `debuff_poison_moc` (6 dmg × stack, moc skill) — trước đó dotPerTickFlat compute nhưng KHÔNG consume runtime, de facto no-op. Thêm logic: nếu charHp ≤ 0 sau DOT → status LOST + clamp HP=1 (giống monster reply LOST handling). 5 vitest mới: 1 stack 8 dmg, 2 stack 16 dmg, dot kill (LOST + clamp), no debuff identity, no service inject identity. Branch `devin/1777720156-phase-11-x-m-buff-dot-combat`. Test baseline: **2947 vitest** (1405 api + 954 shared + 588 web).
- **Latest merged PR**: [#260 docs(CHANGELOG): catch-up session 9r-25 part 2 — PR #258 + #259](https://github.com/hoathienmenh-01/xuantoi/pull/260) — Append section "[session 9r-25 part 2 wire batch — PR #258 → #259, merged 2/5 2026]" vào `docs/CHANGELOG.md` với scope/player-impact/tests/risk per PR. Pure docs-only. Main bumped `8ce1994` → `b47686f`. CI GREEN.
