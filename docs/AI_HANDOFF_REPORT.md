...
### PR #150 — `test(web): smart AdminView render-level smoke tests — 18 vitest (session 9k task C / admin ops safety)` — **CI xanh 5/5, Pending merge**

- **Branch**: `devin/1777541055-adminview-render-tests`. **Base**: `main` @ `5a815b3`. **Status**: Pending merge (open trên GitHub, CI 5/5 xanh sau rerun empty commit trigger flaky `apps/api` chat rate-limit `window trượt` sliding window test — unrelated to PR).
...
#### PR session 9k-C (planned, after 9k-B merge) — `test(web): AdminView render-level vitest — tab switch + Export CSV + Giftcode revoke modal + Economy alerts + Audit-ledger button`
- **Branch**: TBD `devin/<ts>-adminview-render-tests`. **Base**: `main` post-9k-B.
- **Plan**: `apps/web/src/views/__tests__/AdminView.test.ts` (new) — 12-15 vitest; mock `@/api/admin` + `@/api/economy`; render-level only.
- **Risk**: 🟢 thấp — test-only.
- **Rollback**: revert single PR.

#### PR session 9k-D (in-flight, this PR) — `docs: bootstrap PRIVACY.md + TOS.md (closed-beta tester agreement)`
- **Branch**: `devin/1777542440-docs-privacy-tos`. **Base**: `main` @ `5a815b3` (post 9k-A merge; 9k-C PR #150 CI xanh đang chờ merge — không block 9k-D vì docs-only không đụng cùng file).
