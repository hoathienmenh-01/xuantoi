...
- **Current `main` commit**: `1c86705` post PR #315 (Phase 11.5.C UI Refine — Luyện Khí button + cost preview + protection toggle + 5 outcome toast MERGED, CI all GREEN). PR #246..#315 MERGED into main. **This session mở PR docs-only**: cập nhật AI_HANDOFF_REPORT.md để mark PR #313 CLOSED + delete branch (anti-duplicate guard cho AI/dev sau).
...
- **Immediate next task** (theo thứ tự khuyến nghị):
  ...
  8. **Phase 11.4.C UI Gem socket** — UI khảm/tháo/combine gem cho `InventoryView.vue` consume `GemService` runtime (Phase 11.4.B PR #239 merged). **Backend gap phát hiện**: `inventory.service.ts:list()` `itemByKey(r.itemKey)` không index `GEMS` catalog → gem inventory rows bị skip ở line 110, gems vô hình trên UI hoàn toàn. Phase 11.4.C cần combined backend+UI medium PR fix `itemByKey` fallback `getGemDef` (synthesize ItemDef từ GemDef) hoặc extend `inventory.service.list()` lookup chain trước khi UI hiển được. Ước tính 6-10 files / 400-600 LOC.
