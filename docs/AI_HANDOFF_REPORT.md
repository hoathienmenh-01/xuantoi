...
- **Current `main` commit**: `28faf77` post PR #312 (Phase 11.X.AT — frontend wire Talent Học button + Pinia store + status filter + budget bar MERGED). PR #246..#312 MERGED into main. **This session mở PR Phase 11.10.F**: thêm `reward.items` non-empty cho 2 milestone bronze achievements (`first_breakthrough` → 5× huyet_chi_dan; `first_dungeon_clear` → 3× tinh_thiet) — exercise wire của PR #277 (`AchievementService.claimReward` → `InventoryService.grantTx` qua `ItemLedger reason='ACHIEVEMENT_REWARD'`) bằng catalog production thay vì spy. Local checks GREEN: shared 1015 → **1019 vitest** (+4 Phase 11.10.F catalog: 2 entries/qty/itemKey + 1 ≥ 2 count + 1 bronze qty ≤ 10), api 1501 → **1503 vitest** (+2 real-catalog claim flow integration: `first_breakthrough` + `first_dungeon_clear` end-to-end grant + ItemLedger entry), web 629 unchanged. Full suite **3151 vitest** GREEN. **PR Phase 11.10.F mở, đang chờ CI**.
