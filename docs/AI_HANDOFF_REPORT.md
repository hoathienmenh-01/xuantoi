...
- **Current `main` commit**: `153be68` post PR #292 (Phase 11.X.X boss title atkMul/spiritMul wire). PR #246..#292 MERGED. **No open PR / no pending branch** (this PR is in-flight Phase 11.X.AB — catalog: `talent_moc_linh_co` moc +10% spirit producer thứ 2). Baseline test suite: api 1485 vitest (85 file), shared 960 vitest (29 file), web 588 vitest (61 file) — total **3033 vitest**, all GREEN locally @ `153be68`.
- **PR pending (this session 9r-29 part 7)**: Phase 11.X.AB — feat(catalog,talents): thêm `talent_moc_linh_co` moc +10% spirit — producer thứ 2 cho `composePassiveTalentMods.spiritMul`, fill 5-element coverage roadmap (thuy + moc; hoa/tho còn defer). Symmetric structure với `talent_huyen_thuy_tam` (Phase 11.X.AA, thuy +10% spirit). Activate Phase 11.4.G boss spirit branch wire (PR #289) + Phase 11.X.U combat effSpirit defense wire (PR #274) + Phase 11.X.X title spirit wire (PR #292) cho moc-element character. Catalog metadata-only — KHÔNG đổi runtime/schema/seed. Risk: 🟢 thấp (catalog +1 entry, deterministic compose pure). Rollback: revert PR.
- **Latest merged PR**: [#292 feat(boss,character): Title atkMul/spiritMul wire vào BossService.attack() (Phase 11.X.X)](https://github.com/hoathienmenh-01/xuantoi/pull/292) — Inject `TitleService` (10th positional `@Optional()`) + wire `titleMods.atkMul` × `titleMods.spiritMul` multiplicative compose với `talentMods` × `buffMods`. Boss path full triple compose, parity với combat path PR #274. 4 vitest mới (1477 → 1481 api). Main bumped `300676c` → `153be68`. CI 5/5 GREEN.
- **Previous merged PR**: [#291 feat(boss,character): Buff atkMul/spiritMul wire vào BossService.attack() (Phase 11.X.W)](https://github.com/hoathienmenh-01/xuantoi/pull/291) — Wire `buffMods.atkMul` × `buffMods.spiritMul` multiplicative compose với `talentMods`. Refactor `buffMods` fetch consolidate (move ra ngoài `if (this.buffs)` block). 4 vitest mới (1473 → 1477 api). Main bumped `db218b6` → `300676c`. CI 5/5 GREEN.
...
> **Snapshot (session 9r-29 part 7, PR open — feat(catalog,talents): thêm `talent_moc_linh_co` moc +10% spirit (Phase 11.X.AB))**: catalog metadata-only. Branch `devin/1777748633-phase-11-x-ab-talent-moc-spirit-producer` từ `153be68` (main post-PR-#292 merged). **Scope = thêm 1 talent passive `talent_moc_linh_co` (Mộc Linh Cổ)** vào `TALENTS` catalog — producer thứ 2 cho `composePassiveTalentMods.spiritMul`. Symmetric structure với `talent_huyen_thuy_tam` (Phase 11.X.AA, thuy +10% spirit) — fill 5-element coverage roadmap. Thứ tự 5-element spirit producer roadmap: thuy ✅ (`talent_huyen_thuy_tam` PR #290), moc ✅ (`talent_moc_linh_co` PR #293), hoa ⏳ (defer), tho ⏳ (defer), kim ⏳ (defer — kim đã có atk producer `talent_kim_thien_co`). Files changed: (1) `packages/shared/src/talents.ts` (+18 / -1) — thêm catalog entry sau `talent_moc_linh_quy`, cập nhật comment `15` → `16 talent baseline`. (2) `packages/shared/src/talents.test.ts` (+22 / -0) — 2 vitest mới: (1) `moc_linh_co → spiritMul = 1.1` isolation (atk/def/hpMax/hpRegenFlat identity), (2) `combine huyen_thuy_tam + moc_linh_co → spiritMul = 1.21` multiplicative cross-element. Catalog: `element: 'moc'`, `realmRequirement: 'kim_dan'`, `talentPointCost: 1`, `passiveEffect.kind: 'stat_mod'`, `value: 1.1`, `statTarget: 'spirit'`. **Local verified**: shared 960/960 ✅ (bởi vì catalog test count dùng `TALENTS.length` dynamic, không hardcode), api 1485/1485 ✅, web 588/588 ✅ (chains theo PR #292), typecheck ✅, lint ✅. **Risk**: 🟢 **thấp** — catalog +1 entry deterministic. Player mới `kim_dan` realm có element `moc` có thể learn talent này → +10% spirit (bằng với thuy). Multiplicative stack với `talent_huyen_thuy_tam` (nếu player learn cả hai) → 1.21 spirit mul — chi phí 2 talent points. Talent point budget @ kim_dan = 3 (per `computeTalentPointBudget`) nên stack feasible nhưng không infinite. Game design `learnTalent` KHÔNG check element gating — player mục đích element-flexible build có thể stack. Cumulative 4 spirit producer (sau khi hoa/tho thêm) = 1.1⁴ = 1.4641 spirit max nhưng tiers còn đợc gate by talent point budget. **Rollback**: revert PR (single catalog entry + 2 test, không có schema/data migration).
