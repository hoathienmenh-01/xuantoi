...
- **Current `main` commit**: `95cead6` post PR #245 (Phase 11.9.B Title MVP runtime). PR #246 (Phase 11.10.B Achievement MVP runtime) IN FLIGHT — branch `devin/1777700304-phase-11-10-b-achievement-mvp-runtime` rebase on top of merged #245.
- **Pending PR #246**: Phase 11.10.B Achievement (Thành Tựu) MVP runtime — progress + completion persistence. New `AchievementService` (`incrementProgress` + `trackEvent` + `getProgress` + `listAll` + `listCompleted` + `getProgressByGoalKind`) qua `prisma.$transaction` + new model `CharacterAchievement` (composite UNIQUE `(characterId, achievementKey)` + 1 index + FK cascade, `progress Int default 0`, `completedAt DateTime?`) + reuse `getAchievementDef`/`achievementsByGoalKind` từ `packages/shared/src/achievements.ts` (Phase 11.10.A catalog 32 achievement đã merge). MVP scope = persistence + tracking + completion detection; KHÔNG có `claimReward` + KHÔNG auto-unlock title qua `titleForAchievement` + KHÔNG event listener wire vào combat/dungeon/breakthrough (defer Phase 11.10.C). Progress capped tại `goalAmount` (service-side); `completedAt` set một lần khi đạt goal, không bao giờ unset. 21 vitest cover increment happy/cap/idempotent post-complete, errors (ACHIEVEMENT_NOT_FOUND/CHARACTER_NOT_FOUND/INVALID_AMOUNT 0/neg/non-int), trackEvent bulk by goalKind, getProgress null/found, listAll empty/sorted/defensive catalog rename, listCompleted filter, getProgressByGoalKind, cross-character isolation, error class, catalog count sanity.
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** (Cultivation Method + Skill Template + Linh căn runtime đầy đủ) ✅. **Phase 11.4.B MERGED** (Gem MVP — PR #239) ✅. **Phase 11.5.B MERGED** (Refine MVP — PR #240) ✅. **Phase 11.6.B MERGED** (Tribulation MVP — PR #242) ✅. **Phase 11.8.B MERGED** (Buff persistence MVP — PR #243) ✅. **Phase 11.7.B MERGED** (Talent persistence MVP — PR #244) ✅. **Phase 11.9.B MERGED** (Title persistence MVP — PR #245) ✅. **Phase 11.10.B IN FLIGHT** (Achievement persistence MVP — PR #246, this PR). **NEXT runtime track**: 11.10.C Achievement wire (`claimReward` atomic grant via CurrencyService.applyTx + ItemLedger 'ACHIEVEMENT_REWARD' + auto-unlock title qua `titleForAchievement` + event listener wire vào combat/dungeon/breakthrough/cultivate/market), 11.7.C Talent wire (`composePassiveTalentMods` vào `CharacterStatService.computeStats` + active talent execution + cooldown), 11.8.C Buff wire (compose `getMods()` vào stat compute + tribulation FAIL applyBuff('debuff_taoma') thay cho `Character.taoMaUntil`), 11.9.C Title wire (compose `getMods()` vào stat compute), 11.3.D UI Linh căn (reroll dialog + element wheel), 11.1.C/11.2.C/11.5.C/11.6.C UI cultivation method + skill book + refine + tribulation + title + achievement.
- **Current test baseline (main `95cead6` + PR #246 in flight)**: **~2885 vitest** = api **~1343** (PR #246 +21 AchievementService over post-#245 baseline ~1322; CI-verified exact) + shared **954** + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN trên main per CI run của PR #245. **Local verified PR #246 staging**: typecheck ✓ (api+shared+web no error), lint ✓ (zero warnings), shared `pnpm test` 954/954 ✓, web `pnpm test` 588/588 ✓, build ✓. **API test ~1343 cần Postgres** → CI verify (local Postgres unavailable). **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
