...
- **Current `main` commit**: `6fd1120` (Merge PR #212 ci(e2e): run full-stack Playwright golden path, 1/5 ~05:26 UTC).
- **Latest merged PR**: [#212 ci(e2e): run full-stack Playwright golden path](https://github.com/hoathienmenh-01/xuantoi/pull/212) (CI/test infra, workflow `.github/workflows/e2e-full.yml` gated path-filter + workflow_dispatch). CI trên main HEAD chạy 1m35s (Playwright step 20s, browsers cached) — verified GREEN.
- **Current phase**: **Phase 9 — Closed beta stabilization** (audit refresh, smoke:economy ✅, smoke:ws ✅, Playwright golden path 16 spec ✅ [#211], CI integration full-stack E2E ✅ [#212], **Phase 9 readiness audit (this PR)**, bug triage P0/P1, mobile/i18n polish, release checklist). Xem [`LONG_TERM_ROADMAP.md`](./LONG_TERM_ROADMAP.md) §Phase 9.
- **Current test baseline**: **1959 vitest** = api **1133** + shared **238** + web **588** — baseline KHÔNG đổi (PR này docs-only). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s (CI 20s cached / local 23.7s). Workflow `e2e-full.yml` đã chạy thật trên main GREEN sau merge #212 → CI integration verified, KHÔNG còn pending. Vẫn gated path-filter + manual `workflow_dispatch` (NOT required every PR).
- **Phase 9 readiness** (snapshot session 9r-2 — chi tiết bảng A–O ở [`BETA_CHECKLIST.md`](./BETA_CHECKLIST.md) §"Phase 9 readiness audit"): **9/15 nhóm Done**, **5 nhóm Partial / Needs runtime smoke** (cultivation breakthrough end-to-end, dungeon enter+combat+loot, mail claim attachment, mission claim end-to-end, admin/topup/giftcode UI), **1 nhóm Needs prod review** (CSP/Sentry/structured logs). Defer rationale ghi rõ trong từng nhóm — **cover bằng vitest unit / integration test thay vì runtime smoke**, KHÔNG phải gap chưa cover. **Phase 10 gate**: ✅ ĐỦ điều kiện mở Phase 10 content scale — không có P0/P1 blocker, không có core gameplay flow nào hoàn toàn unverified (mỗi flow ít nhất unit test + 1 trong: smoke / golden path / vitest integration).
- **Open PRs**: this PR — `docs(phase9): audit closed beta readiness after full-stack e2e` (docs-only — readiness table A–O + Phase 10 gate decision + stale test count fix). Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 10 PR-1..5** — content scale: items pack 1 (+50 item), skills pack 1 (+15 skill), monsters & dungeons pack 1 (+20 monster, +6 dungeon), missions pack 1 (+50 mission), boss catalog (+10 named boss). Theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md). **Pre-gate**: chạy `pnpm smoke:economy` + `pnpm smoke:ws` + `E2E_FULL=1 pnpm --filter @xuantoi/web e2e` để confirm baseline trước khi expand catalog.
  2. **Bug triage P0/P1** — chưa có GitHub issue P0/P1 nào open (verified 1/5 — `gh issue list --state all` empty); khi user closed beta báo bug, ưu tiên fix nhỏ-1-PR trước khi tiếp tục Phase 10.
  ...
  4. **Optional Phase 9 polish** — nếu có thời gian: viết `smoke:combat` (chạy combat flow end-to-end RNG-deterministic seed) hoặc `smoke:admin` (login admin → grant mail → user claim) để cover 2 deferred runtime smoke gap.
