...
- **Current `main` commit**: `197f825` (Merge PR #239 feat(inventory,character): Gem MVP runtime — socket/unsocket/combine via ItemLedger + applySocketBonus wire, Phase 11.4.B).
- **Latest merged PR**: [#239 feat(inventory,character): Gem MVP runtime — socket/unsocket/combine via ItemLedger + applySocketBonus wire (Phase 11.4.B)](https://github.com/hoathienmenh-01/xuantoi/pull/239) — Server-authoritative `GemService` 3 method (`socketGem`/`unsocketGem`/`combineGems`) qua `prisma.$transaction` + `ItemLedger` (3 reason mới: `GEM_SOCKET`/`GEM_UNSOCKET`/`GEM_COMBINE`) + Prisma migration `InventoryItem.sockets String[]` + wire `composeSocketBonus` vào `InventoryService.equipBonus()` + 3 controller endpoint (`POST /character/gem/socket|unsocket|combine`) + zod input + error-status mapping + 15 GemService unit test (success / capacity / compatibility / qty / combine 3→1 / atomic / catalog drift no-grant).
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** (Cultivation Method + Skill Template + Linh căn runtime đầy đủ) ✅. **Phase 11.4.B MERGED** (Gem MVP runtime — PR #239) ✅. **NEXT runtime track**: Phase 11.5.B Refine MVP (current PR — `RefineService` + `InventoryItem.refineLevel` + wire `getRefineStatMultiplier` vào `equipBonus`), 11.6.B Tribulation/Tâm Ma MVP, 11.7.B Talent runtime, 11.8.B Buff runtime, 11.9.B Title runtime, 11.10.B Achievement runtime — theo catalog foundation đã mở.
- **Current test baseline (main `197f825`)**: **~2793 vitest** = api **1238** (PR #239 +18 GemService) + shared **954** (Phase 11.4.B catalog foundation +3) + web **588**. Phase 11.5.B mở rộng: api **1251** (+13 RefineService test). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN local trên main. **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
...
- **Open PRs**: 1 open PR đang được handle bởi session hiện tại — **Phase 11.5.B Refine MVP runtime** (PR #240 TBD link sau khi push), branch `devin/1777693382-phase-11-5-b-refine-mvp-runtime` rebase trên `197f825`.
