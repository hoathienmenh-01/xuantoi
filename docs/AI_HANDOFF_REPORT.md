...
- **Current `main` commit**: `35db2d5` post PR #267 (Phase 11.7.E Talent regen wire). PR #246..#267 MERGED. **PR pending (this session 9r-26 part 5)**: **feat(boss,character): Buff control wire vào BossService.attack() (Phase 11.X.Q)** — parallel to Phase 11.X.O combat wire, throw `BossError('CONTROLLED')` BEFORE state mutation (cooldown/mp/stamina/hp deduct). Catalog producer `debuff_root_thuy` (3t), `debuff_stun_tho` (1t), `debuff_silence_kim` (2t). Inject `@Optional() buffs?: BuffService`. Map `CONTROLLED` → HTTP 409 trong `BossController.handleErr`. 5 vitest mới (stun + state-unchanged, root, silence, no-debuff identity, no-service identity). Branch `devin/1777727566-phase-11-x-q-boss-control` từ `35db2d5`. Test baseline: post-PR-#267 = 2962 vitest → post-this-PR = **2967 vitest** (1425 api + 954 shared + 588 web).
