...
- **Current `main` commit**: `1972756` (Merge PR #219 chore(scripts): smoke:combat HTTP runtime smoke (Phase 9.5 polish)).
- **Latest merged PR**: [#219 chore(scripts): smoke:combat HTTP runtime smoke (Phase 9.5 polish)](https://github.com/hoathienmenh-01/xuantoi/pull/219) — 18-step HTTP smoke for combat encounter end-to-end (basic attack RNG → WON → reward shape → ledger invariant SUM(CurrencyLedger) == Character.linhThach + Inventory.qty per itemKey == SUM(ItemLedger.qtyDelta) + COMBAT_LOOT refType=Encounter), 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged: items #214 / skills #215 / monsters & dungeons #216 / **mission #217** / boss #218). **Phase 9.5 smoke polish gap closed 2/2** (this PR `smoke:admin` thêm vào `smoke:combat` PR #219). Phase 9 closed beta stabilization đã APPROVED gate ([#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). **Next phase**: Phase 11 — Spiritual Root MVP (P11-1 schema + onboarding roll service).
- **Current test baseline (main)**: **2079 vitest** = api **1133** + shared **358** + web **588**. **This PR** không thêm vitest (chỉ thêm `scripts/smoke-admin.mjs` HTTP smoke — Node 20 native fetch, no vitest). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step, PR #219) / **`smoke:admin` (30 step, this PR)**.
- **Phase 9 readiness** (snapshot session 9r-7 — chi tiết bảng A–O ở [`BETA_CHECKLIST.md`](./BETA_CHECKLIST.md) §"Phase 9 readiness audit"): **11/15 nhóm Done**, **3 nhóm Partial / Needs runtime smoke**, **1 nhóm Needs prod review**. **Phase 10 gate**: ✅ APPROVED + COMPLETE. **Phase 9.5 smoke gap CLOSED**: nhóm D (Dungeon / combat / loot) và nhóm J (Admin / topup / giftcode) đều có HTTP smoke. Còn PARTIAL: nhóm C (cultivation breakthrough — deferred `smoke:cultivation` với DB-direct exp inject), nhóm H (mission daily/weekly claim end-to-end — chỉ vitest + smoke:ws WS push, cần long-mode), nhóm G (mail UI claim trong Playwright golden — smoke:admin đã cover HTTP).
- **Phase 10 progress**: **PR-1 (DONE — #214)** Item Pack 1 — catalog **31 → 81** items (+50). **PR-2 (DONE — #215)** Skill Pack 1 — catalog **10 → 25** skill (+15, Ngũ Hành). **PR-3 (DONE — #216)** Monster & Dungeon Pack 1 — monster catalog **9 → 29** (+20, Ngũ Hành), dungeon catalog **3 → 9** (+6), DUNGEON_LOOT **3 → 9**, ITEMS **81 → 87** (+6 yêu phù). **PR-4 (DONE — #217)** Mission Pack 1 — mission catalog **12 → 66** (+54 mission), MissionDef forward-compat, +27 vitest. **PR-5 (DONE — #218)** Boss Pack 1 — boss catalog **2 → 12** (+10 named boss), BossDef forward-compat extends, +28 vitest. **Phase 10 status**: 5/5 PR merged. **CLOSED**.
- **Open PRs**: 
  1. this PR — `chore(scripts): smoke:admin HTTP runtime smoke (Phase 9.5 polish)` — không phụ thuộc Phase 10/11.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (smoke:admin) → close 2/2 deferred runtime smoke gap.
  2. **Phase 11.0 prep** — schema migration `Character.spiritualRoot` (5-tier element grade + primaryElement + secondaryElements + rootPurity + rootRerollCount) + onboarding roll service + lookup `elementMultiplier(skill, target)` để wire `element` field hiện chỉ-metadata thành runtime damage modifier (Phase 11.2/11.3).
  3. **Optional smoke gap polish**: `smoke:cultivation` (DB-direct exp inject → breakthrough event), `smoke:topup` (cli pay client → topup order → admin approve/reject), `smoke:admin` v2 (MOD role + mail broadcast). Tất cả nice-to-have, không block Phase 11.
