...
- **Current `main` commit**: `621c252` post PR #286 (Phase 11.8.D-2 Tribulation FAIL → debuff_taoma atomic apply via BuffService.applyBuffTx). PR #246..#286 MERGED. **No open PR / no pending branch** (this audit refresh PR is docs-only, post-#286 close-out). Baseline test suite: api 1464 vitest (85 file), shared 958 vitest (29 file), web 588 vitest (61 file) — total 3010 vitest, all GREEN locally @ `621c252`.
- **Latest merged PR**: [#286 feat(character): Tribulation FAIL path → atomic apply `debuff_taoma` qua BuffService.applyBuffTx (Phase 11.8.D-2 wire)](https://github.com/hoathienmenh-01/xuantoi/pull/286) — Inject `BuffService` (4th positional optional) vào `TribulationService`. FAIL path: sau `tx.character.update` (legacy `taoMaUntil` field giữ nguyên cho backward-compat), nếu `penalty.taoMaActive` → `await this.buffs?.applyBuffTx(tx, charId, 'debuff_taoma', 'tribulation', now, penalty.taoMaExpiresAt)` cùng tx — atomic rollback. Extend `applyBuffTx` với optional 6th param `expiresAtOverride?: Date` để tribulation per-tier duration (15/30/60/120 phút) override default catalog `durationSec=3600`. 5 vitest mới (1459 → 1464 api). Public surface 100% backward-compat (3-arg + 4-arg constructor, `expiresAtOverride` optional). Main bumped `32a3052` → `621c252`. CI GREEN.
