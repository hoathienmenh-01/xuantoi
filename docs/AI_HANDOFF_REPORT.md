...
> **Snapshot (session 9n-M in-flight, realtime.service unit tests — rebased post-#176)**: `main` @ `a8c3c64` (Merge PR #176 `test(api) mail.service WS+prune+bugfix 22 vitest`, 30 Apr 2026 ~16:50 UTC). **Session 9n progress (12 PR merged + 1 in-flight)**: PR #165..#176 merged. **This PR (9n-M, branch `devin/1777570000-realtime-service-tests`)**: unit test cho `apps/api/src/modules/realtime/realtime.service.ts` (84 dòng, trước đây 0 test trực tiếp — chỉ test qua gateway). +20 vitest pure unit (no DB, fake socket.io Server) cover: (a) `attach/detach/isOnline/countOnline` — user-level counting (2 tab = 1 user online), cleanup khi hết socket, no-op khi user không tồn tại; (b) `emitToUser` — no-op trước bind, no-op khi không có socket, emit tới MỌI socket với frame `{type,payload,ts}`; (c) `broadcast` / `emitToRoom` — frame shape đầy đủ; (d) `joinUserToRoom` / `leaveUserFromRoom` — gọi socket.join/leave cho mọi socket của user; (e) `bind()` idempotent — lần 2 bị bỏ qua để tránh double-init ghi đè. API test baseline 545 → **565**.

> **Snapshot (session 9n-L merged as PR #176)**: test bổ sung cho `apps/api/src/modules/mail/mail.service.ts` — +22 vitest cover WS emit (spy `realtime.emitToUser`), pruneExpired (claim cũ / expired no-reward xoá; expired có reward (bao gồm item-only) GIỮ), validateInput edge cases (length bounds, qty ranges, negativity) + **bugfix `pruneExpired` item-only mail loss** (Devin Review catch: thiếu `rewardItems.equals=[]` trong Prisma JSON filter → expired mail có item-only rewards bị xoá nhầm → mất item người chơi).

> **Snapshot (session 9n-K merged as PR #175)**: smart catalog integrity tests cho `packages/shared/src/{enums,ws-events,api-contracts}.ts` — +66 vitest cover enum values/order/uniqueness + WS heartbeat/reconnect/cultivation tick constants + zod schema validation (Email/Password/Login/Register/ChangePassword/Forgot/Reset/PublicUser) + AuthErrorCode enum + AUTH_ERROR_VI i18n map.
...
| Realtime | **30 test** (`realtime.gateway.test.ts` 10 + `realtime.service.test.ts` 20 session 9n-M) — +20 pure unit (no DB, fake Socket.IO) cover attach/detach/userSocket map, emitToUser/broadcast/emitToRoom frame shape, joinUserToRoom/leaveUserFromRoom, bind() idempotent, no-op guards pre-bind | Ban user during connection | Medium |
