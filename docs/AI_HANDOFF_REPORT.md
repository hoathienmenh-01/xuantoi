...
- **Current `main` commit**: `4c6165d` post PR #310 (Phase 11.X.AS — server-authoritative talent learn API + state endpoint MERGED). PR #246..#311 MERGED into main. **Session 4 mở PR #312**: Phase 11.X.AT — frontend wire "Học" button + Pinia `useTalentsStore` + status filter (đã học / có thể học / chưa đủ điều kiện) + budget bar + i18n error toast trong `TalentCatalogView`. **PassiveTalentMods catalog symmetry HOÀN TẤT** (7/7 statTarget × 5/5 elements = 35 stat producers + 2/2 damage_bonus chains × 5/5 elements = 10 damage_bonus producers, tương khắc 1.15 + tương sinh 1.1 multiplicative stack 1.265). **Talent UI catalog read-only LIVE**: 54 cards (47 passive + 7 active) tại `/talents` route, filter type/element + effect summary. **Server-authoritative learn API LIVE**: `POST /character/talents/learn` + `GET /character/talents/state` wire `TalentService` (Phase 11.7.B MVP atomic transaction + composite UNIQUE chống double-learn race) qua `CharacterController`. Frontend wire (Phase 11.X.AT) hoàn tất qua PR #312: Pinia store `useTalentsStore` server-authoritative (race-protect inFlight Set, error code propagate), `apps/web/src/api/talents.ts` envelope client, learn button states (learned / realm too low / insufficient points / available / in-flight), status filter, budget bar `spent/budget` + remaining, toast success + i18n error mapping. Test suite local checks GREEN: api 1501 vitest (unchanged), shared 1015 vitest (unchanged), web 602 → **629 vitest** (+27 = 5 api/talents client + 11 stores/talents + 11 view filter/learn). Full suite **3145 vitest** GREEN (1 pre-existing chat rate-limit flake fleet-wide, unaffected). **PR #312 mở, đang chờ CI**.
