...
- **Current `main` commit**: `db218b6` post PR #290 (Phase 11.X.AA talent spirit producer). PR #246..#290 MERGED. **No open PR / no pending branch** (this PR is in-flight Phase 11.X.W boss buff atkMul/spiritMul wire). Baseline test suite: api 1477 vitest (85 file), shared 960 vitest (29 file), web 588 vitest (61 file) — total **3025 vitest**, all GREEN locally @ `db218b6`.
- **PR pending (this session 9r-29 part 5)**: Phase 11.X.W — feat(boss,character): Buff atkMul/spiritMul wire vào BossService.attack() — wire `buffMods.atkMul` × `buffMods.spiritMul` multiplicative compose với `talentMods.atkMul`/`spiritMul`. Symmetric với Phase 11.X.S/U combat path (PR #274). Move `buffMods` fetch ra ngoài `if (this.buffs)` block để reuse cho cả control/cultivationBlocked check (Phase 11.X.Q/R) + damage compose (single fetch consolidate). Service không inject → `composeBuffMods([])` identity (atkMul=1.0, spiritMul=1.0, no-op). Catalog producer atkMul: `pill_atk_buff_t1` (+12%), `sect_aura_hoa` (+5%), `debuff_boss_atk_down` (×0.82). Catalog producer spiritMul: `pill_spirit_buff_t1` (+18%), `event_double_exp` (×2.0). 4 vitest mới: atk buff +12% damage > baseline, spirit buff +18% spirit branch damage > baseline, no-inject identity (DB row ù nhưng `boss` bypass), atkScale=1 isolation (spiritMul ignored). Public surface 100% backward-compat. Risk: 🟡 thấp-trung (boss balance — buff/debuff active player sẽ có boss damage adjust). Rollback: revert PR.
- **Latest merged PR**: [#290 feat(catalog,talents): thêm `talent_huyen_thuy_tam` thuy +10% spirit (Phase 11.X.AA)](https://github.com/hoathienmenh-01/xuantoi/pull/290) — Producer đầu tiên cho `composePassiveTalentMods.spiritMul`, activate Phase 11.4.G boss spirit branch wire (PR #289) + Phase 11.X.U combat effSpirit defense wire (PR #274). Catalog: thuy element passive +10% spirit, realm `kim_dan`, cost 1. 2 vitest mới (958 → 960 shared). Main bumped `7d23694` → `db218b6`. CI 5/5 GREEN.
- **Previous merged PR**: [#289 feat(boss,character): Talent spiritMul wire vào BossService.attack() spirit branch (Phase 11.4.G)](https://github.com/hoathienmenh-01/xuantoi/pull/289) — Wire `talents?.getMods(char.id).spiritMul` × `(char.spirit + equip.spiritBonus)` ở spirit branch (`skill.atkScale > 1`). 4 vitest mới (1469 → 1473 api). Main bumped `5c1cd0d` → `7d23694`. CI 5/5 GREEN.
