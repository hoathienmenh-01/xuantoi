...
- **Current `main` commit**: `67de83a` post PR #276 (docs CHANGELOG catch-up session 9r-27). PR #246..#276 MERGED. **PR pending (this session 9r-27 part 5)**: **feat(character,inventory): Achievement item rewards via InventoryService.grantTx (Phase 11.10.D)** — wire `def.reward.items` non-empty trong `AchievementService.claimReward` qua `InventoryService.grantTx` reason `'ACHIEVEMENT_REWARD'` (`ItemLedger` audit). Inject `InventoryService` vào `AchievementService` qua `forwardRef` (cycle `CharacterModule` ↔ `InventoryModule`). Add `'ACHIEVEMENT_REWARD'` vào `ItemLedgerReason` union. Replace `throw AchievementError('ITEMS_NOT_SUPPORTED')` (defensive Phase 11.10.C-1 deferral) bằng grant call. Identity hiện tại: 32 baseline catalog không có achievement với items → no-op. Future-proof khi catalog thêm. 3 vitest mới (item grant + items rỗng + double-grant prevention). CI baseline 2980 → 2983 vitest (1441 api + 954 shared + 588 web).
- **Latest merged PR**: [#276 docs(CHANGELOG): catch-up session 9r-27 — PR #272 + #273 + #274 + #275](https://github.com/hoathienmenh-01/xuantoi/pull/276) — Pure docs/CHANGELOG catch-up cho 4 PR. Main bumped `809bc0f` → `67de83a`. CI 5/5 GREEN.
- **Previous merged PR**: [#275 feat(combat,character): Buff invuln wire vào CombatService.action() override damage (Phase 11.X.V)](https://github.com/hoathienmenh-01/xuantoi/pull/275) — Wire `buffMods.invulnActive` (kind=invuln) PRE-shield gate trong reply branch (line 404-438) + DOT branch (line 449). Pattern coverage nhất quán với cultivationBlocked (#270) + control (#264). Identity false (catalog không có producer kind=invuln) → zero balance impact. 4 vitest. Main bumped `eed0d77` → `809bc0f`. CI 5/5 GREEN.
...
- **Previous merged PR**: [#273 docs(audit): refresh AI_HANDOFF_REPORT post PR #271 merged + PR #272 open](https://github.com/hoathienmenh-01/xuantoi/pull/273) — Pure docs/audit refresh. CI 4/4 GREEN.
