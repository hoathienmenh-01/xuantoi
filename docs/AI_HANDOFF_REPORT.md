...
- **Current `main` commit**: `8123509` post PR #254 (Phase 11.7.D Talent expMul cultivation wire). PR #246..#254 MERGED. **PR pending (this session)**: **Phase 11.8.D Buff cultivationBlocked (Tâm Ma) wire vào CultivationProcessor** — compose `BuffService.getMods().cultivationBlocked` flag vào tick. Khi character có debuff `debuff_taoma` (Tâm Ma Triền Thân — durationSec 3600 sau khi vượt kiếp FAIL) → tick skip toàn bộ EXP gain + mission/achievement track + realtime emit (stamina regen ở top vẫn áp dụng). Inject `@Optional() BuffService`. Service không inject hoặc character không có buff với `cultivation_block` effect → identity (không block). 4 vitest mới. Branch `devin/1777716371-phase-11-8-d-buff-cultivation-block`.
- **Latest merged PR**: [#254 feat(cultivation,character): Talent expMul wire vào CultivationProcessor (Phase 11.7.D)](https://github.com/hoathienmenh-01/xuantoi/pull/254) — Wire `talents.getMods().expMul` vào `CultivationProcessor.process()` tick gain. Compose multiplicatively cultivationMul × methodMul × talentExpMul. 4 vitest. Main bumped `c9dc5cf` → `8123509`. CI 5/5 GREEN.
