...
> **Snapshot**: `main` @ `4b3d938` (Merge PR #69 G15 L2 market fee env, 29 Apr 2026 09:20 UTC). **Merged into main since session 6**: PR #62 (G8 — M11 profile rate-limit), #63 (G9 — M3 WS `mission:progress` + emitter throttle), #64 (G10 — H6 Playwright CI), #65 (G11 — FE handler `mission:progress`), #66 (G12 — L7 admin inventory revoke), #67 (G13 — L5 skeleton Leaderboard+Profile), #68 (G14 — L5 cont skeleton MissionView+AdminView), #69 (G15 — L2 `MARKET_FEE_PCT` env config), #72 (G18 — admin audit log filter), #73 (G19 — admin topup filter date+email). **3 PR open Pending merge** (CI ✅ 5/5 mỗi PR): #70 (G16 — admin user filter role+banned), #71 (G17 — M7 `GET /mail/unread-count` + FE badge hydrate), #74 (G20 — admin giftcode filter q+status, fix Devin Review truncation).
