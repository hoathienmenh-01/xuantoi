...
- **Current `main` commit**: `c66169f` (Merge PR #224 feat(shared): gem catalog foundation (Phase 11.4.A)).
- **Latest merged PR**: [#224 feat(shared): gem catalog foundation (Phase 11.4.A)](https://github.com/hoathienmenh-01/xuantoi/pull/224) — `packages/shared/src/gems.ts` NEW (~310 lines) catalog 25 gem (5 Ngũ Hành × 5 grade `PHAM..THAN`) deterministic generated với `GemDef/GemBonus/GemSource/GemCompatibleSlot`, helpers `getGemDef/gemsByElement/gemsByGrade/composeSocketBonus/combineGems/canSocketGem/gemUpgradePathCost`, combine recipe 3× → 1× next-tier (THAN top), combine sink rule `3× PHAM > LINH`, +39 vitest (shared 499 → 538), 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). **Phase 11 — Progression Depth** đã mở. **PR #221 (Phase 11.0 Spiritual Root catalog) MERGED** + **PR #222 (Phase 11.1.A CultivationMethod catalog) MERGED** + **PR #223 (Phase 11.2.A SkillTemplate catalog) MERGED** + **PR #224 (Phase 11.4.A Gem catalog) MERGED** vào main `c66169f`. Phase 11 catalog foundation track = **4/4 DONE**. **This PR** = **Phase 11.5.A Refine catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2259 vitest** = api **1133** + shared **538** + web **588** (post-#224 merge). **This PR** thêm **+56 vitest** (shared 538 → 594) → total **2315/2315** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **PR 11.0 (DONE — #221)** Spiritual Root catalog. **PR 11.1.A (DONE — #222)** CultivationMethod catalog. **PR 11.2.A (DONE — #223)** SkillTemplate catalog. **PR 11.4.A (DONE — #224)** Gem catalog. **PR 11.5.A (this PR open)** Refine catalog foundation — `packages/shared/src/refine.ts` NEW + 56 vitest, 15 level baseline (3 stage safe/risky/extreme × 5 level) với `RefineLevelDef`, `simulateRefineAttempt` deterministic RNG, `getRefinePathCostMin` planning calculator, protection charm key reservation. **PR 11.1.B (Pending)** CultivationMethod runtime. **PR 11.2.B (Pending)** SkillTemplate runtime. **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4.B (Pending)** Gem runtime (Equipment.sockets + socketGem/unsocketGem service + applySocketBonus). **PR 11.5.B (Pending)** Refine runtime (Equipment.refineLevel + service `refineEquipment` + wire `getRefineStatMultiplier`). **PR 11.6..7 (Pending)** Tribulation / Talent / Alchemy.
- **Open PRs**:
  1. **this PR (#TBD)** — `feat(shared): refine catalog foundation (Phase 11.5.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.5.A Refine catalog) → unblock Phase 11.5.B runtime.
  2. **Phase 11.6.A PR**: Tribulation catalog static (`packages/shared/src/tribulation.ts`) trước khi runtime — catalog-only, low-risk.
