...
- **Current `main` commit**: `2a351e6` post PR #317 (Phase 11.4.C UI Gem socket). PR #246..#317 MERGED into main. **This session mở PR**: [#318 Phase 11.11.B Alchemy (Luyện Đan) MVP runtime](https://github.com/hoathienmenh-01/xuantoi/pull/318) — `AlchemyService.attemptCraft / getFurnaceLevel / listAvailableRecipes` + `Character.alchemyFurnaceLevel Int default 1` + `LedgerReason 'ALCHEMY_COST'` + ItemLedger `'ALCHEMY_INPUT'` / `'ALCHEMY_OUTPUT'` reasons + 18 vitest. **Confirmed unique** (anti-duplicate check: `git log --oneline | grep -iE "alchemy"` → only catalog `packages/shared/src/alchemy.ts` Phase 11.X.A đã merge; no `alchemy.service.ts` exists in main).
