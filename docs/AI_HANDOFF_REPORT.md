...
- **Latest merged PR**: [#240 feat(inventory,character): Refine MVP runtime — refineEquipment via ItemLedger/CurrencyLedger + getRefineStatMultiplier wire (Phase 11.5.B)](https://github.com/hoathienmenh-01/xuantoi/pull/240) — Server-authoritative `RefineService.refineEquipment(charId, equipmentInvItemId, useProtection?, rng?)` qua `prisma.$transaction` + `ItemLedger` (2 reason mới: `REFINE_MATERIAL`/`REFINE_PROTECTION`) + `CurrencyLedger` (`REFINE`) + Prisma migration `InventoryItem.refineLevel Int @default(0)` + wire `getRefineStatMultiplier` vào `InventoryService.equipBonus()` + new `refine_protection_charm` HUYEN MISC item (Hộ Khí Phù) + `POST /character/refine` endpoint với zod input + 6 error code (`EQUIPMENT_NOT_FOUND`/`NOT_REFINABLE`/`MAX_LEVEL_REACHED`/`INSUFFICIENT_MATERIAL`/`INSUFFICIENT_PROTECTION`/`INSUFFICIENT_FUNDS`) + 13 RefineService unit test.
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** (Cultivation Method + Skill Template + Linh căn runtime đầy đủ) ✅. **Phase 11.4.B MERGED** (Gem MVP runtime — PR #239) ✅. **Phase 11.5.B MERGED** (Refine MVP runtime — PR #240) ✅. **NEXT runtime track** (theo catalog foundation đã mở): 11.6.B Tribulation/Tâm Ma MVP (per spec §P11-6), 11.7.B Talent runtime, 11.8.B Buff runtime, 11.9.B Title runtime, 11.10.B Achievement runtime, 11.3.D UI Linh căn (reroll dialog + element wheel), 11.1.C/11.2.C/11.5.C UI cultivation method + skill book + refine.
- **Current test baseline (main `8e3081e`)**: **~2793 vitest** = api **1251** (PR #240 +13 RefineService) + shared **954** + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN trên main per CI run của PR #240. **Local verified post-#240 (this audit-only PR)**: typecheck ✓ (api+shared+web no error), lint ✓ (zero warnings), shared `pnpm test` 954/954 ✓, web `pnpm test` 588/588 ✓. **API test baseline 1251 cần Postgres** → CI verify (rely on PR #240 CI green + this PR CI). **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
