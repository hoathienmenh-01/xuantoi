...
- **Current `main` commit**: `a448ce5` post PR #318 (Phase 11.11.B Alchemy MVP runtime). PR #246..#318 MERGED into main. **This session mở PR**: Phase 11.11.C Alchemy HTTP wire — branch `devin/1777790003-phase-11-11-c-alchemy-http-wire` từ `a448ce5`. Backend-only MEDIUM PR wire `AlchemyService` (Phase 11.11.B MVP, PR #318) qua `CharacterController`. 2 endpoints mới (`GET /character/alchemy/recipes` + `POST /character/alchemy/craft`) + Zod DTO + `mapAlchemyErrorStatus` (RECIPE_NOT_FOUND/CHARACTER_NOT_FOUND→404, FURNACE_LEVEL_TOO_LOW/REALM_REQUIREMENT_NOT_MET/INSUFFICIENT_INGREDIENTS/INSUFFICIENT_FUNDS→409). 16 vitest mới (`character.controller.alchemy.test.ts`). **Anti-duplicate verified**: `git ls-tree origin/main | grep alchemy` → catalog `packages/shared/src/alchemy.ts` (Phase 11.11.A) + service `apps/api/src/modules/character/alchemy.service.ts` (Phase 11.11.B PR #318) — KHÔNG có controller endpoint hoặc UI route, scope unique.
