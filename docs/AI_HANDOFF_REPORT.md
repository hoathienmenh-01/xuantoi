...
- **Current `main` commit**: `8e3081e` (Merge PR #240 feat(inventory,character): Refine MVP runtime — refineEquipment via ItemLedger/CurrencyLedger + getRefineStatMultiplier wire, Phase 11.5.B).
- **Latest merged PR**: [#240 feat(inventory,character): Refine MVP runtime — refineEquipment via ItemLedger/CurrencyLedger + getRefineStatMultiplier wire (Phase 11.5.B)](https://github.com/hoathienmenh-01/xuantoi/pull/240) — Server-authoritative `RefineService.refineEquipment(charId, equipmentInvItemId, useProtection?)` qua `prisma.$transaction` + `ItemLedger` (`REFINE_MATERIAL`/`REFINE_PROTECTION`) + `CurrencyLedger` (`REFINE`) + Prisma migration `InventoryItem.refineLevel Int @default(0)` + wire `getRefineStatMultiplier` vào `InventoryService.equipBonus()` + `POST /character/refine` endpoint + 13 unit test.
- **Open PR (this session)**: **Phase 11.6.B Tribulation/Tâm Ma MVP runtime** — branch `devin/1777694394-phase-11-6-b-tribulation-mvp-runtime` rebase trên `8e3081e`. Server-authoritative `TribulationService.attemptTribulation(userId)` qua `prisma.$transaction` + `simulateTribulation` (deterministic resist=1.0 MVP) + `CurrencyLedger.TRIBULATION_REWARD` + `ItemLedger.TRIBULATION_DROP` (unique drop heavenly/saint) + `computeTribulationFailurePenalty` (exp loss + cooldown + Tâm Ma trigger) + Prisma migration `Character.taoMaActive/taoMaExpiresAt/tribulationCooldownAt` + new `TribulationAttempt` log + wire `CharacterService.breakthrough()` gate (block nếu chưa cleared khi realm transition có tribulation) + `setCultivating()` Tâm Ma gate (auto-clear on expiry) + `POST /character/tribulation/attempt` + `GET /character/tribulation/preview` + 18 unit/integration test (success / ALREADY_CLEARED / fail no-Tâm-Ma / fail Tâm-Ma trigger / cooldown / retry sau cooldown / NO_CHARACTER / NOT_AT_PEAK / NO_TRIBULATION / previewNext / breakthrough gate × 3 / setCultivating Tâm Ma gate × 2).
- **Previous merged PR**: [#239 feat(inventory,character): Gem MVP runtime — socket/unsocket/combine via ItemLedger + applySocketBonus wire (Phase 11.4.B)](https://github.com/hoathienmenh-01/xuantoi/pull/239).
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** ✅. **Phase 11.4.B MERGED** (Gem MVP — #239) ✅. **Phase 11.5.B MERGED** (Refine MVP — #240) ✅. **Phase 11.6.B PENDING** (Tribulation/Tâm Ma MVP — current PR). **NEXT runtime track**: 11.7.B Talent runtime, 11.8.B Buff runtime, 11.9.B Title runtime, 11.10.B Achievement runtime — theo catalog foundation đã mở.
- **Current test baseline (main `8e3081e`)**: **~2811 vitest** = api **1251** (PR #240 +13 RefineService) + shared **954** + web **588**. Phase 11.6.B mở rộng: api **1269** (+18 TribulationService incl. breakthrough/cultivating gate). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN local. **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
...
- **Phase 11 progress**: **11.0..11.10.A catalog 11/11 MERGED** ✅. **11.3.A** (#232 SpiritualRootService runtime + Prisma migration) MERGED ✅. **11.3.B** (#233 element multiplier wire CombatService) MERGED ✅. **11.3.C** (#234 cultivationMultiplier + statBonusPercent wire) MERGED ✅. **11.1.B** (#235 Cultivation Method runtime + processor compose + onboard hook) MERGED ✅. **11.2.B** (#236 Skill Template runtime) MERGED ✅. **11.4.B** (#239 Gem MVP runtime — socket/unsocket/combine via ItemLedger + `composeSocketBonus` wire) MERGED ✅. **11.5.B Refine MVP** runtime PENDING — current open PR — `RefineService.refineEquipment(charId, equipmentInvItemId, useProtection?)` qua `prisma.$transaction` + `ItemLedger` (`REFINE_MATERIAL`/`REFINE_PROTECTION`) + `CurrencyLedger` (`REFINE`) + wire `getRefineStatMultiplier(refineLevel)` vào `InventoryService.equipBonus()` + `POST /character/refine` endpoint + 13 unit test. **PR 11.3.D/11.6.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups còn lại theo catalog foundation đã mở.
- **Open PRs**: 1 open PR đang được handle bởi session hiện tại — **Phase 11.6.B Tribulation/Tâm Ma MVP runtime** (PR #241 TBD link sau khi push), branch `devin/1777694394-phase-11-6-b-tribulation-mvp-runtime` rebase trên `8e3081e`.
- **Immediate next task** (theo thứ tự khuyến nghị, scope nhỏ + server-authoritative + ledger-bắt-buộc cho mọi grant/spend/reward):
  1. **Phase 10 PR-2 v2 — Skill Pack Ngũ Hành expansion** (IN-PROGRESS, catalog-only): +10 skill mới (5× kim_dan-tier ULT mỗi hệ + 5× mid-tier passive bù role gap) → SKILLS catalog 26 → 36 skill, vẫn trong stat budget `BALANCE_MODEL.md` §4. Test invariants: unique key, valid element, required fields, mỗi hệ ≥ 2 ACTIVE + ≥ 2 PASSIVE (nâng từ ≥1+≥1) + mỗi hệ ≥ 1 ULT tier `kim_dan`. SKILL_TEMPLATES parity (cardinality + tier consistency) preserved.
  2. **Phase 11.2.C**: UI skill book / skill grimoire display learned skills + equip switcher (call `POST /character/skill/equip`) + mastery upgrade button (call `POST /character/skill/upgrade-mastery` với cost preview). Item-based skill book drop + consume via `ItemLedger` để wire kind='item' SkillUnlockRequirement.
  3. **Phase 11.4.B Gem MVP runtime** (per spec `XuanToi_AI_Build_Functional_Spec.docx` §P11-4): Prisma `Equipment.sockets Json[]` (hoặc `EquipmentSocket` table) + `GemService` (`socketGem`/`unsocketGem`/`combineGems`) consume gem qty qua `ItemLedger` reason `USE` + `applySocketBonus(equipment, gems[])` wire vào `CharacterStatService.computeStats`. Idempotency: server-authoritative qty check, không double-consume; combine 3→1 deterministic.
  4. **Phase 11.5.B Refine MVP runtime** (per spec §P11-5): `InventoryItem.refineLevel Int @default(0)` + `RefineService` (cost/success/fail/bảo hộ via item) + cap per quality tier + property test deterministic RNG seed.
  5. **Phase 11.6.B Tribulation/Tâm Ma MVP** (per spec §P11-6): Manual breakthrough risk realm cao, server resolve deterministic RNG, `TribulationAttemptLog` model, không multi-phase kiếp ngay.
  6. **Phase 11.1.C**: UI character profile display equipped method + list learned + equip switcher (call `POST /character/cultivation-method/equip`). Optional 24h cooldown anti-spam re-equip.
  7. **Phase 11.3 PR-D**: UI character profile display Linh căn (icon + grade tooltip + element wheel) + reroll service consume `linh_can_dan` ItemLedger + insert log `source='reroll'`.
  8. **Phase 11.7.B + 11.8.B + 11.9.B + 11.10.B PR runtime** (compose stack): Prisma `CharacterTalent` + `CharacterBuff` + `CharacterTitleUnlock` + `CharacterAchievement { id, characterId, achievementKey, progress, completedAt? }` + service `incrementAchievement` (idempotent) + `claimAchievement` (validate `completedAt!=null` + grant reward + auto-`unlockTitle(rewardTitleKey)`) + event listener cho mỗi `goalKind` (KILL_MONSTER / CLEAR_DUNGEON / BOSS_HIT / BREAKTHROUGH / GAIN_EXP / CULTIVATE_SECONDS / BUY_LISTING / SELL_LISTING / CHAT_MESSAGE / SECT_CONTRIBUTE).
  9. **Optional smoke gap polish**: `smoke:cultivation` / `smoke:topup` / `smoke:admin` v2.
