...
- **Current `main` commit**: `fce6d89` (Merge PR #222 feat(shared): cultivation method catalog foundation (Phase 11.1.A)).
- **Latest merged PR**: [#222 feat(shared): cultivation method catalog foundation (Phase 11.1.A)](https://github.com/hoathienmenh-01/xuantoi/pull/222) — `packages/shared/src/cultivation-methods.ts` NEW (~390 lines) catalog 12 method baseline (1 pham starter + 5 huyen Ngũ Hành + 3 tien sect-locked + 3 than endgame) với `CultivationMethodGrade/Source/StatBonus`, `expMultiplier 1.0..1.8`, `forbiddenElements` chu kỳ tương khắc safety, helper `getCultivationMethodDef/methodsByElement/methodsForSect/canLearnMethod`, +35 vitest (shared 401 → 436), 5/5 CI GREEN sau merge conflict resolution với main `6076fb8` (PR #221 spiritual-root catalog).
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). **Phase 11 — Progression Depth (Spiritual Root + Cultivation Method + Elemental Combat)** đã mở. **PR #221 (Phase 11.0 Spiritual Root catalog) MERGED** + **PR #222 (Phase 11.1.A CultivationMethod catalog) MERGED** vào main `fce6d89`. **This PR** = **Phase 11.2 SkillTemplate catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2157 vitest** = api **1133** + shared **436** + web **588** (post-#222 merge). **This PR** thêm **+63 vitest** (shared 436 → 499) → total **2220/2220** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **PR 11.0 (DONE — merged #221)** Spiritual Root catalog foundation — `packages/shared/src/spiritual-root.ts` + 43 vitest. **PR 11.1.A (DONE — merged #222)** CultivationMethod catalog foundation — `packages/shared/src/cultivation-methods.ts` + 35 vitest, 12 method baseline. **PR 11.2.A (this PR open)** SkillTemplate catalog foundation — `packages/shared/src/skill-templates.ts` NEW + 63 vitest, 26 template baseline 1-1 với `SKILLS` (combat.ts), 5 tier (basic/intermediate/advanced/master/legendary), mastery curve generator + `applyMasteryEffect` helper + 2 legendary evolution branches forward-compat phase 11.2.B runtime. **PR 11.1.B (Pending)** CultivationMethod runtime (Prisma + service + UI). **PR 11.2.B (Pending)** SkillTemplate runtime (Prisma `CharacterSkill { skillKey, masteryLevel, learnedAt }` + service + UI + wire `applyMasteryEffect` vào `CombatService`). **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4..7 (Pending)** Thiên kiếp / Alchemy / Refinery / Talent.
- **Open PRs**:
  1. **this PR (#TBD)** — `feat(shared): skill template catalog foundation (Phase 11.2.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.2.A SkillTemplate catalog) → unblock Phase 11.2.B runtime + complete Phase 11 catalog foundation track (11.0 + 11.1.A + 11.2.A all DONE).
  ...
  3. **Phase 11.1.B + 11.2.B PR runtime**: Prisma model `CharacterCultivationMethod { characterId, methodKey, learnedAt, isActive }` + `CharacterSkill { characterId, skillKey, masteryLevel, learnedAt }` + service learn/equip/upgrade + UI + wire `applyMasteryEffect` vào CombatService.
