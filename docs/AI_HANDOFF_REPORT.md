...
> **Snapshot (session 9k task C — this PR)**: `main` @ `5a815b3` (Merge PR #149 `docs(handoff): session 9k kickoff — bump snapshot to e342513 (post #148) + mark PR #134..#148 all Merged + sync baseline web 466/466 (50 file) + shared 96/96 (6 file) + session 9k roadmap`, 30 Apr 2026 ~09:00 UTC). **Session 9k task A**: **Merged** — audit refresh docs-only PR #149 (bump snapshot, mark PR #134..#148 Merged, sync baseline test count). **In-flight (this PR — session 9k task C)**: `test(web): smart AdminView render-level smoke tests — 18 vitest (session 9k task C / admin ops safety)` — cover onMounted role guard (unauth redirect, PLAYER noPermission toast + `/home` redirect, ADMIN + MOD pass guard → adminStats + adminEconomyAlerts + adminListTopups(PENDING,0) đều gọi), tab badge rendering (alertsCount > 0 render `admin-tab-stats-alerts-badge`, alertsCount = 0 → no badge, pendingTopupCount render `admin-tab-topups-pending-badge`, activeGiftcodeCount render `admin-tab-giftcodes-active-badge` qua `giftCodeStatusOf` ACTIVE filter), tab switch fetch (Users → adminListUsers, Audit → adminListAudit), Export CSV flow (click → adminExportUsersCsv + toast success `admin.users.exportedToast`, truncated=true → toast warning `exportTruncatedToast`, error UNAUTHENTICATED → toast i18n `admin.errors.UNAUTHENTICATED`), và Giftcode revoke ConfirmModal wiring (click revoke → modal open với title chứa code + impact preview, cancel → modal close không gọi API, confirm → adminRevokeGiftcode + toast success `revokedToast` + modal close + refreshGiftcodes reload, confirm error CODE_REVOKED → toast i18n `admin.errors.CODE_REVOKED` không dùng handleErr UNKNOWN, mã REVOKED/EXPIRED không render nút revoke). Pending merge. File mới: `apps/web/src/views/__tests__/AdminView.test.ts` (18 vitest). Baseline: web `466 → 484` (50 → 51 file).
>
