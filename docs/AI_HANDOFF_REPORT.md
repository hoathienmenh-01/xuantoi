...
- **Current `main` commit**: `e82be8e` post PR #251 (Phase 11.7.C Talent passive wire). PR #246 + #247 + #248 + #249 + #250 + #251 MERGED. **PR pending (this session)**: **Phase 11.8.C Buff wire** — compose `BuffService.getMods()` vào `CombatService.action()` stat compute. Wire 6 mods: `buffMods.atkMul` × effPower, `buffMods.defMul` × effDef, `buffMods.spiritMul` × spirit defense calc, `buffMods.damageBonusByElement[skill.element]` × dmg, `buffMods.damageReductionByElement[monster.element]` × incoming reply. Inject `@Optional() BuffService`. Service không inject → fallback `composeBuffMods([])` identity (all mul 1.0, all flat 0). 5 vitest mới. Branch `devin/1777708470-phase-11-8-c-buff-wire-combat`.
- **Latest merged PR**: [#251 feat(combat,character): Talent passive wire (Phase 11.7.C)](https://github.com/hoathienmenh-01/xuantoi/pull/251) — Wire `talents.getMods()` vào `CombatService.action()`: atkMul × effPower, defMul × effDef, damageBonusByElement × dmg, expMul × expDrop, dropMul × linhThachDrop. 4 vitest. Main bumped `42a15fc` → `e82be8e`. CI GREEN.
