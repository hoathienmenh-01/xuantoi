...
- **Current `main` commit**: `4393833` (Merge PR #230 feat(shared): title (danh hiệu) catalog foundation (Phase 11.9.A)).
- **Latest merged PR**: [#230 feat(shared): title (danh hiệu) catalog foundation (Phase 11.9.A)](https://github.com/hoathienmenh-01/xuantoi/pull/230) — `packages/shared/src/titles.ts` NEW catalog 24 title baseline (9 realm milestone + 5 element mastery + 4 achievement + 3 sect rank + 2 event + 1 donation × 5 rarity), +51 vitest, 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track**: PR #221..#230 (10/10 catalog foundation) MERGED. **This PR** = **Phase 11.10.A Achievement catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2626 vitest** = api **1133** + shared **905** + web **588** (post-#230 merge). **This PR** thêm **+45 vitest** (shared 905 → 950) → total **2671/2671** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **11.0 (#221) / 11.1.A (#222) / 11.2.A (#223) / 11.4.A (#224) / 11.5.A (#225) / 11.6.A (#226) / 11.X.A (#227) / 11.7.A (#228) / 11.8.A (#229) / 11.9.A (#230) MERGED**. Phase 11 catalog track = **10/10 DONE** ✅. **PR 11.10.A (this PR open)** Achievement catalog foundation — `packages/shared/src/achievements.ts` NEW + 45 vitest, 32 achievement baseline cover 7 category (combat 8 / cultivation 6 / exploration 5 / social 4 / economy 4 / milestone 3 / collection 2) × 5 tier `bronze/silver/gold/platinum/diamond`, schema `AchievementDef { key, nameVi, nameEn, description, category, tier, goalKind (reuse MissionGoalKind), goalAmount, element, rewardTitleKey, reward, hidden }`, helpers `getAchievementDef/achievementsByCategory/Tier/GoalKind/Element/visibleAchievements`. **rewardTitleKey link** với `titles.ts` `titleForAchievement` cho Phase 11.10.B + 11.9.B service auto-grant title trên achievement complete. **PR 11.1.B/11.2.B/11.3/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups tương ứng với từng catalog foundation đã mở. Phase 11 catalog track = **11/11 DONE** sau khi merge this PR.
- **Open PRs**:
  1. **this PR** — `feat(shared): achievement (thành tựu) catalog foundation (Phase 11.10.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.10.A Achievement catalog) → Phase 11 catalog foundation track = 11/11 DONE.
  2. **Phase 11.3 PR runtime**: Prisma migration `Character.spiritualRootGrade/primaryElement/secondaryElements/rootPurity/rootRerollCount/cultivationMethodKey` + onboarding roll service + wire `elementMultiplier` + `cultivationMethod.expMultiplier` vào runtime.
