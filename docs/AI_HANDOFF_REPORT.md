...
- **Current `main` commit**: `f0cd240` (Merge PR #220 chore(scripts): smoke:admin HTTP runtime smoke (Phase 9.5 polish)).
- **Latest merged PR**: [#220 chore(scripts): smoke:admin HTTP runtime smoke (Phase 9.5 polish)](https://github.com/hoathienmenh-01/xuantoi/pull/220) — 30-step HTTP smoke covering admin login → stats/economy/audit-ledger shape → grant LT → giftcode + redeem + duplicate-reject → mail send + claim + duplicate-reject → ledger/inventory invariants → RBAC + revoke + audit footprint, multi-jar pattern (admin+player), 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged: items #214 / skills #215 / monsters & dungeons #216 / mission #217 / boss #218). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). Phase 9 closed beta stabilization đã APPROVED gate ([#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). **Next phase**: **Phase 11 — Progression Depth (Spiritual Root + Elemental Combat)**. **This PR** = **Phase 11.0 catalog foundation** (`spiritual-root.ts` 5-grade tier + Ngũ Hành helper, KHÔNG runtime/schema, không block).
- **Current test baseline (main)**: **2079 vitest** = api **1133** + shared **358** + web **588**. **This PR** thêm **+43 vitest** → shared 358 → 401 → total **2122/2122** (sau merge). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (this PR catalog-only không đụng runtime API → smoke không cần re-run).
- **Phase 9 readiness** (snapshot session 9r-8 — chi tiết bảng A–O ở [`BETA_CHECKLIST.md`](./BETA_CHECKLIST.md) §"Phase 9 readiness audit"): **11/15 nhóm Done**, **3 nhóm Partial / Needs runtime smoke**, **1 nhóm Needs prod review**. **Phase 10 gate**: ✅ APPROVED + COMPLETE. **Phase 9.5 smoke gap CLOSED**: nhóm D (Dungeon / combat / loot) và nhóm J (Admin / topup / giftcode) đều có HTTP smoke. Còn PARTIAL: nhóm C (cultivation breakthrough), nhóm H (mission claim end-to-end), nhóm G (mail UI claim Playwright).
...
- **Phase 11 progress**: **PR 11.0 (this PR)** Spiritual Root catalog foundation — `packages/shared/src/spiritual-root.ts` NEW + 43 vitest. **PR 11.1 (Pending)** CultivationMethod model. **PR 11.2 (Pending)** SkillTemplate + CharacterSkill DB. **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4..7 (Pending)** Thiên kiếp / Alchemy / Refinery / Talent.
- **Open PRs**: 
  1. this PR — `feat(shared): spiritual root catalog foundation (Phase 11.0)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.0 catalog) → mở nhánh Phase 11 official.
  2. **Phase 11.1 PR**: CultivationMethod catalog static (`packages/shared/src/cultivation-methods.ts`) trước khi runtime — catalog-only, low-risk, theo đúng pattern Phase 10.
  3. **Phase 11.3 PR runtime** (sau khi 11.0 + 11.1 catalog ổn): schema migration `Character.spiritualRootGrade/primaryElement/secondaryElements/rootPurity/rootRerollCount` + onboarding roll service + wire `elementMultiplier` vào `CombatService.applySkillDamage`.
  4. **Optional smoke gap polish**: `smoke:cultivation` / `smoke:topup` / `smoke:admin` v2.
...
> **Snapshot (session 9r-8, this PR — Phase 11.0 catalog foundation: Spiritual Root + Ngũ Hành elementMultiplier helpers)**: feat/data — sau khi PR #220 `smoke:admin` đã merge vào main `f0cd240` (Phase 9.5 smoke gap CLOSED 2/2 cả nhóm D combat + nhóm J admin), Phase 10 content scale đã CLOSED 5/5 (items / skills / monsters & dungeons / missions / boss). Bắt đầu Phase 11 với PR catalog-only foundation an toàn nhất theo đúng pattern Phase 10 (catalog-first → runtime later). **Pre-gate** (verify baseline post-#220): `pnpm test` → `2079/2079` (api 1133 + shared **358** + web 588 — không đổi từ #220 chỉ scripts), `pnpm typecheck` ✅, `pnpm lint` ✅, `pnpm build` ✅ (PWA precache 47 entries 781.62 KiB). Tất cả PASS → đủ điều kiện Phase 11.0. **`packages/shared/src/spiritual-root.ts`** NEW (~340 lines) catalog đặt nền cho Spiritual Root MVP runtime (P11-3 tương lai): (1) `SpiritualRootGrade` 5-tier `'pham' | 'linh' | 'huyen' | 'tien' | 'than'` + ordered list `SPIRITUAL_ROOT_GRADES`; (2) `SPIRITUAL_ROOT_GRADE_DEFS` table 5 entry với fields `key/name/description/tier/cultivationMultiplier/statBonusPercent/secondaryElementCount/rollWeight` — monotonic invariant (cultivation 1.0/1.15/1.30/1.50/1.80, statBonus 0/5/10/18/30%, secondaryCount 0/1/2/3/4, rollWeight 60/25/10/4/1 sum=100); (3) `getSpiritualRootGradeDef(grade)` lookup throw on invalid + `compareSpiritualRootGrade(a, b)` cho re-roll anti-downgrade logic phase 11.3; (4) `elementGenerates(element)` Ngũ Hành tương sinh cycle Kim→Thuỷ→Mộc→Hoả→Thổ→Kim (cycle invariant: 5 lần apply về ban đầu); (5) `elementOvercomes(element)` Ngũ Hành tương khắc cycle Kim→Mộc→Thổ→Thuỷ→Hoả→Kim; (6) **`elementMultiplier(attacker, defender)`** core helper trả về 1.30 tương khắc / 1.20 tương sinh / 0.70 bị khắc / 0.85 bị sinh / 0.90 cùng hệ / 1.00 vô hệ — bounds [0.7, 1.3] verified ở test, mutual product symmetry property (a→b × b→a ≤ 1.05) để chống double-buff exploit; (7) `SpiritualRootState` interface + `validateSpiritualRootState(state)` defensive parse (reject invalid grade / element / count mismatch / duplicate elements / out-of-bound purity / non-integer rerollCount) — sẽ dùng khi load Character từ DB ở phase 11.3; (8) `characterSkillElementBonus(character, skillElement, targetElement)` compose base multiplier + bonus (+0.10 cùng primaryElement, +0.05 ∈ secondaryElements, +0 vô hệ skill) — sẽ wire `CombatService.applySkillDamage` ở phase 11.2 Elemental Combat MVP. **`packages/shared/src/spiritual-root.test.ts`** NEW (43 vitest) cover: 5 grade tier list invariant, getDef throw on invalid, compare order, sinh/khắc cycle (5-step return-to-start), multiplier value cho 25 cặp 5×5, bounds [0.7, 1.3], mutual product ≤ 1.05, validate happy path / invalid grade / invalid element / count mismatch / duplicate / out-of-bound purity / non-integer rerollCount / null state, characterSkillElementBonus với cùng primary / secondary / không thuộc / null skill / null character / cùng-hệ-cùng-primary edge case. **`packages/shared/src/index.ts`** +1 dòng re-export `./spiritual-root`. **Docs updated**: `BALANCE_MODEL.md` §4.2 mở rộng với bảng 5-grade Linh căn + bảng 6-case `elementMultiplier` value + character bonus rules; `GAME_DESIGN_BIBLE.md` §C.4 Linh căn từ "Đề xuất phase 11" → "Phase 11.0 catalog foundation đã có" với chi tiết grade table + multiplier value; `LONG_TERM_ROADMAP.md` Phase 11 thêm PR 11.0 entry **DONE** trước 11.1; `AI_HANDOFF_REPORT.md` Executive Summary cập nhật main `f0cd240` + Phase 11 progress section + next task. **Files changed**: `packages/shared/src/spiritual-root.ts` NEW +340, `packages/shared/src/spiritual-root.test.ts` NEW +303, `packages/shared/src/index.ts` +1 line, `docs/BALANCE_MODEL.md` ~+45 line §4.2 expand, `docs/GAME_DESIGN_BIBLE.md` §C.4 expand, `docs/LONG_TERM_ROADMAP.md` +6 line PR 11.0 entry, `docs/AI_HANDOFF_REPORT.md` Executive Summary + new snapshot. **Commands run** (local pre-PR): `pnpm typecheck` ✅, `pnpm lint` ✅, `pnpm test` ✅ **2122/2122** (api 1133 + **shared 401 (+43)** + web 588), `pnpm build` ✅ PWA 781.62 KiB. **NO smoke run** — catalog-only PR không đụng API runtime, không cần smoke (lần smoke gần nhất verified ở PR #220 main `f0cd240`). **Risk**: very-low — pure catalog/data + helper functions, KHÔNG sửa runtime code (`apps/api/**` / `apps/web/**` không đụng), KHÔNG schema migration (`prisma/schema.prisma` không đổi), KHÔNG breaking change (helper là thuần additive). Combat runtime hiện vẫn `damage = atk * scale - def * 0.5` không đọc element — phase 11.2 sẽ wire. Rollback: `git revert <commit>` xoá `spiritual-root.ts` + `spiritual-root.test.ts` + revert index.ts 1 line + revert docs. **Next task**: chờ user duyệt + merge → tự chọn Phase 11.1 catalog (CultivationMethod static catalog) HOẶC `smoke:cultivation` polish HOẶC tiếp tục Phase 11.3 schema migration (cần Prisma migration prepared kỹ).
