...
- Forgot-password + reset-password API — **Nice-to-have** for beta. `POST /_auth/forgot-password` + `POST /_auth/reset-password`. **Risk 🟢 thấp** nếu chỉ thêm endpoint + FE form, không đụng schema (dùng existing `passwordVersion` + time-limited token).
