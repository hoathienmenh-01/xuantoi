...
- **Current `main` commit**: `c0ce729` post PR #270 (Phase 11.X.R cultivationBlocked boss). PR #246..#270 MERGED. **PR pending (this session 9r-26 part 6)**: **feat(boss,inventory): Equipment atk/spirit bonus wire vào BossService.attack() (Phase 11.4.E)** — wire `inventory.equipBonus().atk` cộng vào `charAtk` + `equipBonus().spiritBonus` cộng vào `char.spirit` cho atkScale > 1 skill. Trước đây boss attack chỉ dùng `char.power`/`char.spirit` raw, hoàn toàn bỏ qua equip bonus (atk + sockets + refine). Subset của Phase 11.X.S full stat wire — chỉ wire equip, KHÔNG wire talent/buff/title atkMul (defer scope balance review). 3 vitest mới (so_kiem +5 atk → damage cao hơn baseline, no-equip identity, huyen_kiem spirit wire). Branch `devin/1777729517-phase-11-4-e-boss-equip-atk-wire` từ `c0ce729`. Test baseline post-PR-#270 = 2970 → post-this-PR = **2973 vitest** (1431 api + 954 shared + 588 web).
- **Latest merged PR**: [#270 feat(boss,character): Buff cultivationBlocked wire vào BossService.attack() (Phase 11.X.R)](https://github.com/hoathienmenh-01/xuantoi/pull/270) — Wire `buffMods.cultivationBlocked` (Tâm Ma debuff_taoma) throw `BossError('CULTIVATION_BLOCKED')` BEFORE state mutation. Cùng `getMods` call với Phase 11.X.Q control check. 3 vitest. Main bumped `761af38` → `c0ce729`. CI 5/5 GREEN. (PR #269 v1 auto-closed khi chained base deleted, recreated as #270.)
...
- **Open PRs**: 0 open PR on main. **This session**: Phase 11.8.C Buff wire PR pending.
