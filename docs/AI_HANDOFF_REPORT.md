...
- **Current `main` commit**: `09a1bc5` post PR #257 (docs CHANGELOG catch-up session 9r-25 wire batch). PR #246..#257 MERGED. **PR pending (this session)**: **Phase 11.4.D Equip spiritBonus wire vào CombatService.action()** — wire `inventory.equipBonus.spiritBonus` (item base spirit + gem socket bonus + refine multiplier, đã compute ở Phase 11.4.B / 11.5.B) vào `effSpirit` cho monster reply defense calc. Trước đó equip.spiritBonus chỉ được compute nhưng KHÔNG consume runtime — gem moc/thuy/tho spirit bonus de facto no-op. Pattern same as atk wire: `(base + flat) × multipliers`. Cùng pattern với `(char.power + equip.atk) * statMul * talentMods.atkMul * buffMods.atkMul * titleMods.atkMul` ở line 232. 2 vitest mới (gem socket → effSpirit increase, no equip → identity baseline). Branch `devin/1777718318-phase-11-4-d-equip-spirit-bonus-combat`.
- **Latest merged PR**: [#257 docs(CHANGELOG): catch-up session 9r-25 wire batch (PR #251 → #256)](https://github.com/hoathienmenh-01/xuantoi/pull/257) — Append entry vào `docs/CHANGELOG.md` mô tả 6 PR Phase 11 passive system wire batch. Pure docs-only. Main bumped `9d101c0` → `09a1bc5`. CI GREEN.
