...
- **Current `main` commit**: `32a3052` post PR #284 (Phase 11.8.D-1 BuffService.applyBuffTx refactor) + PR #285 (Phase 11.10.G-2 catalog cross-ref tests). PR #246..#285 MERGED. **PR pending (this session 9r-29 part 1)**: **feat(character): Tribulation FAIL path → atomic apply `debuff_taoma` qua BuffService.applyBuffTx (Phase 11.8.D-2 wire)** — Inject `BuffService` (4th positional optional) vào `TribulationService`. FAIL path: sau `tx.character.update` (legacy `taoMaUntil` field giữ nguyên cho backward-compat), nếu `penalty.taoMaActive` → `await this.buffs?.applyBuffTx(tx, charId, 'debuff_taoma', 'tribulation', now, penalty.taoMaExpiresAt)` cùng tx — atomic rollback. Extend `applyBuffTx` với optional 6th param `expiresAtOverride?: Date` để tribulation per-tier duration (15/30/60/120 phút) override default catalog `durationSec=3600`. 5 vitest mới (1459 → 1464 api). Public surface 100% backward-compat (3-arg + 4-arg constructor, `expiresAtOverride` optional).
- **Latest merged PR**: [#285 test(titles,achievements): catalog cross-ref invariants — unlockAchievementKey + reciprocity (Phase 11.10.G-2)](https://github.com/hoathienmenh-01/xuantoi/pull/285) — Test-only catalog referential integrity hardening. 2 vitest: (1) `titles.unlockAchievementKey PHẢI tồn tại trong ACHIEVEMENTS catalog`, (2) bidirectional reciprocity — `achievements.rewardTitleKey → title` PHẢI có `source='achievement' && unlockAchievementKey === achievement.key`. Main bumped `32a3052`. CI 5/5 GREEN.
