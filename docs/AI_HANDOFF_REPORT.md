...
- **Current `main` commit**: `9d101c0` post PR #256 (Phase 11.8.E Buff hp/mpRegenFlat cultivation wire). PR #246..#256 MERGED. **PR pending (this session)**: **docs CHANGELOG catch-up cho session 9r-25 wire batch (PR #251–#256)** — append entry vào `docs/CHANGELOG.md` mô tả 6 PR Phase 11 passive system wire (Talent passive combat, Buff passive combat, Title flavor combat, Talent expMul cultivation, Buff cultivationBlocked Tâm Ma, Buff hp/mpRegenFlat cultivation). Pure docs-only, không đụng code. Branch `devin/1777717682-docs-changelog-catchup-9r-25-wire`.
- **Latest merged PR**: [#256 feat(cultivation,character): Buff hp/mpRegenFlat wire vào CultivationProcessor (Phase 11.8.E)](https://github.com/hoathienmenh-01/xuantoi/pull/256) — Wire `buffs.getMods().hpRegenFlat` / `mpRegenFlat` × tickSeconds → raw SQL LEAST cap update. 6 vitest. Main bumped `af15ffc` → `9d101c0`. CI GREEN.
