...
>
> **Trạng thái (29/4 session 9g, ~19:18 UTC)**: PR #33..#106 đã merge `main` (PR #105 audit refresh task A merged @ `a907eb1`; PR #106 admin inventory revoke FE task B merged @ `7d1965e`). Toàn bộ Critical/High/Medium đã Resolved trừ M7 (CSP CDN — chỉ verify khi prod deploy), M9 (logout-all `passwordVersion` — intentional trade-off, document trong `docs/SECURITY.md`), M10 (shop daily limit — post-beta nice-to-have). Low: tất cả Resolved (L1..L7 đầy đủ cả BE lẫn FE — L7 FE Resolved by PR #106). **PR #107 in-flight** (i18n parity test + wire breakthroughReady sidebar badge — session 9g task C smart UX polish). Roadmap session 9g còn lại task D (runtime smoke 9d→9g tích hợp).
>
...
## Recent Changes (PR #33→#106 đã merged trên main; PR #107 i18n parity test + breakthroughReady badge in-flight session 9g)

### PR #107 — `feat(web): i18n parity test + wire breakthroughReady sidebar badge + smart UX polish` — **In-flight (Pending merge)** session 9g task C
...
C. **(IN PROGRESS — PR #107 in-flight)** **Smart UX polish + i18n parity guard** — wire `badges.breakthroughReady` (computed từ `/me/next-actions`) thành sidebar dot indicator trên RouterLink `/home`; thêm i18n parity test (`apps/web/src/i18n/__tests__/parity.test.ts`, 6 vitest) enforce vi/en symímetric + no empty + ICU placeholder parity — bảo vệ future contributor thêm key quên thêm locale còn lại. +1 i18n key mới (`shell.badge.breakthroughReady`).
D. **Runtime smoke tích hợp full session 9d→9g** — chạy local `pnpm infra:up && pnpm dev`, smoke 15 phút theo `docs/QA_CHECKLIST.md` (register/login → forgot-password flow → reset-password → admin self-demote guard → leaderboard tabs Power/Topup/Sect → admin inventory revoke modal happy/error path → mobile drawer 375px → breakthroughReady badge appear khi character đủ EXP → backup/restore script). Ghi kết quả + bug phát hiện vào `docs/AI_HANDOFF_REPORT.md` Recent Changes; nếu pass đầy đủ, đổi các mục "Needs runtime smoke" trong §6 Completed Features sang "Done".
...
#### PR #107 (session 9g task C, in-flight) — `feat(web): i18n parity test + wire breakthroughReady sidebar badge + smart UX polish`
- **Branch**: `devin/1777489851-i18n-gap-audit-smart-ux-polish`. **Base**: `main` @ `7d1965e` (sau PR #106 merge). **Status**: in-flight (this PR), local typecheck/lint/web test 174/174/build all xanh, CI ETA 5/5 ✅ sau khi push.
- **File**: `apps/web/src/components/shell/AppShell.vue` (+span badge breakthroughReady), `apps/web/src/i18n/{vi,en}.json` (+1 key `shell.badge.breakthroughReady`), `apps/web/src/i18n/__tests__/parity.test.ts` (mới, +6 vitest enforce vi/en symímetric).
- **Risk**: cực thấp — (a) badge wiring pure additive, không đổi logic cũ; (b) parity test validate state có sẵn; (c) không touch BE/schema/migration.
- **Rollback**: revert single PR. Không touch schema/migration/seed.
