...
- **Current `main` commit**: `ff08a3e` (Merge PR #210 expand Playwright golden path, 1/5 ~04:50 UTC).
- **Latest merged PR**: [#210 test(e2e): expand Playwright golden path for closed beta core loop](https://github.com/hoathienmenh-01/xuantoi/pull/210) (test/QA, 2 new + 3 modified, 0 runtime, zero new dep, 11 strict spec).
- **Current phase**: **Phase 9 — Closed beta stabilization** (audit refresh, smoke:economy ✅, smoke:ws ✅, Playwright golden path 11 spec ✅ [#210], golden path expansion v2 → 16 spec **(this PR)**, bug triage P0/P1, mobile/i18n polish, release checklist). Xem [`LONG_TERM_ROADMAP.md`](./LONG_TERM_ROADMAP.md) §Phase 9.
- **Current test baseline**: **1959 vitest** = api **1133** + shared **238** + web **588** — baseline không đổi (PR này chỉ đụng Playwright). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~22–25s (manual/gated, không trong CI mặc định — cần Postgres + Redis + API + Web dev cùng chạy, xem [`QA_CHECKLIST.md`](./QA_CHECKLIST.md) §12).
- **Open PRs**: this PR — `test(e2e): expand closed beta golden path` (16 spec, +5 spec post-#210). Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
