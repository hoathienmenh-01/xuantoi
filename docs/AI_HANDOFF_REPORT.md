...
- **Current `main` commit**: `7d23694` post PR #289 (Phase 11.4.G boss talent spiritMul wire). PR #246..#289 MERGED. **No open PR / no pending branch** (this PR is in-flight Phase 11.X.AA catalog producer). Baseline test suite: api 1473 vitest (85 file), shared 960 vitest (29 file), web 588 vitest (61 file) — total **3021 vitest**, all GREEN locally @ `7d23694`.
- **PR pending (this session 9r-29 part 4)**: Phase 11.X.AA — feat(catalog,talents): thêm `talent_huyen_thuy_tam` thuy element +10% spirit — producer đầu tiên cho `composePassiveTalentMods.spiritMul`, activate Phase 11.4.G boss spirit branch wire (PR #289) + Phase 11.X.U combat effSpirit defense wire (PR #274). Catalog: `key='talent_huyen_thuy_tam'`, `name='Huyền Thuỷ Tâm'`, `type='passive'`, `element='thuy'`, `realmRequirement='kim_dan'`, `talentPointCost=1`, `passiveEffect: { kind: 'stat_mod', value: 1.1, statTarget: 'spirit' }`. Symmetric với `talent_kim_thien_co` (atk +10% kim) và `talent_thuy_long_an` (hpMax +10% thuy). Thêm 2 vitest cho `composePassiveTalentMods` (1 isolated + 1 combine với kim_thien_co). Total TALENTS catalog 14 → 15 (8 passive + 7 active). Public surface 100% backward-compat. Risk: 🟡 thấp-trung (combat/boss balance — player realm `kim_dan` trở lên học được → +10% spirit damage ở skill atkScale > 1). Rollback: revert PR (catalog single entry + 2 test).
- **Latest merged PR**: [#289 feat(boss,character): Talent spiritMul wire vào BossService.attack() spirit branch (Phase 11.4.G)](https://github.com/hoathienmenh-01/xuantoi/pull/289) — Wire `talents?.getMods(char.id).spiritMul` × `(char.spirit + equip.spiritBonus)` ở spirit branch (`skill.atkScale > 1`). Wire dormant baseline (no catalog producer); future-proof spy test pattern. 4 vitest mới (1469 → 1473 api). Main bumped `5c1cd0d` → `7d23694`. CI 5/5 GREEN.
