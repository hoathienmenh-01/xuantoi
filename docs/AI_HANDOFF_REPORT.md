...
- **Current `main` commit**: `990dd14` (Merge PR #211 expand Playwright golden path v2 → 16 spec, 1/5 ~05:00 UTC).
- **Latest merged PR**: [#211 test(e2e): expand Playwright golden path v2](https://github.com/hoathienmenh-01/xuantoi/pull/211) (test/QA, 5 spec mới, 16 spec / 16 pass `E2E_FULL=1`, manual gated).
- **Current phase**: **Phase 9 — Closed beta stabilization** (audit refresh, smoke:economy ✅, smoke:ws ✅, Playwright golden path 16 spec ✅ [#211], **CI integration full-stack E2E (this PR)**, bug triage P0/P1, mobile/i18n polish, release checklist). Xem [`LONG_TERM_ROADMAP.md`](./LONG_TERM_ROADMAP.md) §Phase 9.
- **Current test baseline**: **1959 vitest** = api **1133** + shared **238** + web **588** — baseline không đổi (PR này chỉ thêm GitHub Actions workflow + docs, không đụng runtime). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~22–25s. CI integration: workflow `e2e-full.yml` chạy gated trên `pull_request` + `push` to main với path filter (`apps/web/**`, `apps/api/**`, `packages/shared/**`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `package.json`, `.github/workflows/e2e-full.yml`) + `workflow_dispatch` manual. KHÔNG required mọi PR — gated theo path để giữ CI nhanh; PR docs-only / unrelated path → skip workflow.
- **Open PRs**: this PR — `ci(e2e): run full-stack Playwright golden path` (workflow mới `e2e-full.yml` + docs update). Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
- **Immediate next task** (theo thứ tự khuyến nghị):
  ...
  2. **Phase 9 readiness audit** — chạy `docs/QA_CHECKLIST.md` 13 section trên local hoặc staging (15 phút) + verify `e2e-full` workflow xanh trên 1–2 PR mẫu để confirm gating hợp lý.
