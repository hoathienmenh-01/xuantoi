...
- **Current `main` commit**: `9130d43` post PR #282 (Phase 11.9.C-3 onboard title wire) + PR #283 (docs CHANGELOG catch-up 9r-28). PR #246..#283 MERGED. **PR pending (this session 9r-28 part 4)**: **refactor(character): BuffService.applyBuffTx tx-aware variant (Phase 11.8.D-1 prep)** — refactor `BuffService.applyBuff` thành thin wrapper gọi `applyBuffTx(tx, ...)` (mirror `TitleService.unlockTitleTx` pattern PR #245). Foundation cho Phase 11.8.D wire FUTURE — `TribulationService.attemptTribulation()` FAIL path apply `debuff_taoma` atomic cùng tx với character update + log write. Public surface giữ nguyên (`applyBuff` 4 params identical). 6 vitest mới (atomic commit/rollback, idempotent stackable trong tx, error trong tx, wrapper sanity). CI baseline 2997 → 3003 vitest (956 shared + 1459 api + 588 web).
- **Latest merged PR**: [#283 docs(CHANGELOG): catch-up session 9r-28 — PR #279 + #280 + #281](https://github.com/hoathienmenh-01/xuantoi/pull/283) — Pure docs/CHANGELOG catch-up cho 3 PR đã merged sau session 9r-27 part 5. Append section `[session 9r-28]` mô tả PR #279 achievement cross-ref + #280 breakthrough title + #281 tribulation title. Main bumped `9130d43`. CI 4/4 GREEN.
