...
- **Current `main` commit**: `fb6c451` post PR #252 (Phase 11.8.C Buff wire). PR #246 + #247 + #248 + #249 + #250 + #251 + #252 MERGED. **PR pending (this session)**: **Phase 11.9.C Title wire** — compose `TitleService.getMods()` vào `CombatService.action()` stat compute. Wire 3 flavor mods multiplicatively (compound với linh căn × talent × buff): `titleMods.atkMul` × effPower, `titleMods.defMul` × effDef, `titleMods.spiritMul` × spirit defense calc. `hpMaxMul`/`mpMaxMul` defer (cap modifier — future `CharacterStatService.computeStats`). Inject `@Optional() TitleService`. Service không inject hoặc character chưa equip title → fallback `composeTitleMods([])` identity (atkMul=defMul=spiritMul=1). 5 vitest mới. Branch `devin/1777714935-phase-11-9-c-title-wire-combat`.
- **Latest merged PR**: [#252 feat(combat,character): Buff passive wire into CombatService (Phase 11.8.C)](https://github.com/hoathienmenh-01/xuantoi/pull/252) — Wire `buffs.getMods()` vào `CombatService.action()`: atkMul × effPower, defMul × effDef, spiritMul × spirit defense, damageBonusByElement × dmg, damageReductionByElement × incoming reply. 5 vitest. Main bumped `e82be8e` → `fb6c451`. CI 5/5 GREEN.
