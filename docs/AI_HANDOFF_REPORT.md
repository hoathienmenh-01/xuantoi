...
- **Current `main` commit**: `03490f5` post PR #244 (Phase 11.7.B Talent MVP runtime). PR #245 (Phase 11.9.B Title MVP runtime) IN FLIGHT — branch `devin/1777699538-phase-11-9-b-title-mvp-runtime` rebase on top of merged #244.
- **Pending PR #245**: Phase 11.9.B Title (Danh hiệu) MVP runtime — title ownership persistence + equip slot service. New `TitleService` (`unlockTitle` + `equipTitle` + `unequipTitle` + `listOwned` + `getEquipped` + `getMods`) qua `prisma.$transaction` + new model `CharacterTitleUnlock` (composite UNIQUE `(characterId, titleKey)` + 1 index + FK cascade) + reuse `composeTitleMods` từ `packages/shared/src/titles.ts` (Phase 11.9.A catalog đã merge); equip slot dùng `Character.title String?` field sẵn có từ phase 0 (1 title slot tại 1 thời điểm). KHÔNG wire `composeTitleMods` vào `CharacterStatService.computeStats` + KHÔNG auto-grant trên realm breakthrough/achievement event (defer Phase 11.9.C). 22 vitest cover unlock happy/idempotent/errors, equip happy/swap/not-owned, unequip clear/idempotent, listOwned, getEquipped, getMods identity + element_kim_blade_master atkMul=1.05 + element_moc_forest_lord hpMaxMul=1.05 + composeTitleMods parity, cross-character isolation, error class.
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** (Cultivation Method + Skill Template + Linh căn runtime đầy đủ) ✅. **Phase 11.4.B MERGED** (Gem MVP — PR #239) ✅. **Phase 11.5.B MERGED** (Refine MVP — PR #240) ✅. **Phase 11.6.B MERGED** (Tribulation MVP — PR #242) ✅. **Phase 11.8.B MERGED** (Buff persistence MVP — PR #243) ✅. **Phase 11.7.B MERGED** (Talent persistence MVP — PR #244) ✅. **Phase 11.9.B IN FLIGHT** (Title persistence MVP — PR #245, this PR). **NEXT runtime track**: 11.7.C Talent wire (`composePassiveTalentMods` vào `CharacterStatService.computeStats` + active talent execution + cooldown), 11.8.C Buff wire (compose `getMods()` vào stat compute + tribulation FAIL applyBuff('debuff_taoma') thay cho `Character.taoMaUntil`), 11.9.C Title wire (compose `getMods()` vào stat compute + auto-grant realm breakthrough/achievement event), 11.10.B Achievement runtime, 11.3.D UI Linh căn (reroll dialog + element wheel), 11.1.C/11.2.C/11.5.C/11.6.C UI cultivation method + skill book + refine + tribulation + title.
- **Current test baseline (main `03490f5` + PR #245 in flight)**: **~2864 vitest** = api **~1322** (PR #245 +22 TitleService over post-#244 baseline 1300; CI-verified exact) + shared **954** + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN trên main per CI run của PR #244. **Local verified PR #245 staging**: typecheck ✓ (api+shared+web no error), lint ✓ (zero warnings), shared `pnpm test` 954/954 ✓, web `pnpm test` 588/588 ✓, build ✓. **API test ~1322 cần Postgres** → CI verify (local Postgres unavailable). **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
