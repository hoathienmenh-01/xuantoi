...
- **Current `main` commit**: `06edb3b` (Merge PR #238 feat(shared): Skill Pack Ngũ Hành expansion — +10 skill, Phase 10 PR-2 v2).
- **Latest merged PR**: [#238 feat(shared): Skill Pack Ngũ Hành expansion — +10 skill (5 kim_dan ULT + 5 mid passive role-gap, Phase 10 PR-2 v2)](https://github.com/hoathienmenh-01/xuantoi/pull/238) — Catalog-only Skill Pack expansion 26→36 skills (5 ULT kim_dan: hoa_thien_chu_tuoc, kim_quang_tham_tham, moc_xuan_huyen_lien, thuy_long_bach_truong, tho_son_thien_long; 5 mid passive: rein_kim, gia_moc, lien_thuy, hoa_thuoc, vung_tho). Mỗi hệ Ngũ Hành phủ ≥2 ACTIVE + ≥2 PASSIVE + ≥1 ULT kim_dan. Tier consistency check enforce qua `inferExpectedTier()`. Pure catalog metadata; runtime auto-inherit `applyMasteryEffect` (Phase 11.2.B) + element multiplier (Phase 11.3.B).
- **Previous merged runtime PR**: [#236 feat(character,combat): Skill Template runtime — learn/upgradeMastery/equip + onboard auto-grant + applyMasteryEffect wire (Phase 11.2.B)](https://github.com/hoathienmenh-01/xuantoi/pull/236) — Prisma `CharacterSkill` model + `CharacterSkillService` (learn/upgradeMastery/equip/unequip/getState/grantStarter/getEffectiveSkillFor) + wire `applyMasteryEffect` vào `CombatService.action()` + onboard auto-grant + auto-equip starter `basic_attack` + 4 controller endpoints + 34 vitest. Backward-compat preserved.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.3 PR-A/B/C MERGED** (Linh căn runtime đầy đủ) ✅. **Phase 11.1.B MERGED** (Cultivation Method runtime — PR #235) ✅. **Phase 11.2.B MERGED** (Skill Template runtime — PR #236) ✅. **NEXT runtime track**: Phase 11.4.B Gem MVP (socket/combine via ItemLedger), 11.5.B Refine MVP, 11.6.B Tribulation/Tâm Ma MVP, 11.7.B Talent runtime, 11.8.B Buff runtime, 11.9.B Title runtime, 11.10.B Achievement runtime — theo catalog foundation đã mở.
- **Current test baseline (main `06edb3b`)**: **~2779 vitest** = api **~1220** + shared **954** (skills-balance.test +6 sau Phase 10 PR-2 v2, +3 socketCapacityForQuality test sau Phase 11.4.B catalog foundation) + web **588** (PR #238 catalog-only +10 skill template tests; PR #239 Phase 11.4.B +18 api/shared test deltas). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN local trên main. **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
...
- **Phase 11 progress**: **11.0..11.10.A catalog 11/11 MERGED** ✅. **11.3.A** (#232 SpiritualRootService runtime + Prisma migration) MERGED ✅. **11.3.B** (#233 element multiplier wire CombatService) MERGED ✅. **11.3.C** (#234 cultivationMultiplier + statBonusPercent wire) MERGED ✅. **11.1.B** (#235 Cultivation Method runtime + processor compose + onboard hook) MERGED ✅. **11.2.B** (#236 Skill Template runtime — `CharacterSkill` Prisma + `CharacterSkillService` + wire `applyMasteryEffect` vào `CombatService.action()` + onboard auto-grant `basic_attack` + 4 controller endpoints + 34 vitest) MERGED ✅. **PR 11.3.D/11.4.B/11.5.B/11.6.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups còn lại (Linh căn reroll UI, gem socket/refine/tribulation/talent/buff/title/achievement) theo catalog foundation đã mở.
- **Open PRs**: 1 open PR đang được handle bởi session hiện tại — **Phase 11.4.B Gem MVP runtime** (PR #239 TBD link sau khi push), branch `devin/1777692252-phase-11-4-b-gem-mvp-runtime` rebase trên `06edb3b`.
