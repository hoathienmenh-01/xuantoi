...
- **Current `main` commit**: `b9fa932` (Merge PR #234 feat(combat,cultivation): wire Linh căn cultivationMultiplier + statBonusPercent (Phase 11.3.C)).
- **Latest merged PR**: [#234 feat(combat,cultivation): wire Linh căn cultivationMultiplier + statBonusPercent (Phase 11.3.C)](https://github.com/hoathienmenh-01/xuantoi/pull/234) — `CultivationProcessor` apply `cultivationMultiplier` vào EXP gain mỗi tick, `CombatService` apply `statBonusPercent` vào `effPower`/`effDef`. +5 vitest deterministic. Backward-compat preserved (legacy `spiritualRootGrade=null` → multiplier 1.0).
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.3 PR-A/B/C MERGED** (Linh căn runtime đầy đủ) ✅. **This PR** = **Phase 11.1.B — Cultivation Method (Công pháp) runtime hookup**: Prisma `Character.equippedCultivationMethodKey` + `CharacterCultivationMethod` model + `CultivationMethodService` (learn/equip/getState/grantStarter) + wire vào `CultivationProcessor` (compose `methodMul` × `cultivationMul`) + onboard auto-grant + auto-equip starter `khai_thien_quyet` + 2 controller endpoints.
- **Current test baseline (main)**: **2695 vitest** = api **1157** + shared **950** + web **588** (post-#234 merge). **This PR** thêm vitest API (23 cultivation-method service + 4 cultivation processor compose + 2 character.service onboard hook = +29) → total ~**2724** sau merge (api ~1186 + shared 950 + web 588). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN local. **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — fails on main `b9fa932` cũng flake — NOT caused by this PR).
...
- **Phase 11 progress**: **11.0..11.10.A catalog 11/11 MERGED** ✅. **11.3.A** (#232 SpiritualRootService runtime + Prisma migration) MERGED ✅. **11.3.B** (#233 element multiplier wire CombatService) MERGED ✅. **11.3.C** (#234 cultivationMultiplier + statBonusPercent wire) MERGED ✅. **THIS PR = Phase 11.1.B — Cultivation Method (Công pháp) runtime hookup**: (1) Prisma migration `20260502000000_phase_11_1_cultivation_method` add `Character.equippedCultivationMethodKey` (nullable) + `CharacterCultivationMethod { characterId, methodKey, source, learnedAt }` model với `@@unique([characterId, methodKey])` cho idempotent learn; (2) `CultivationMethodService` (NestJS) cung cấp `learn(charId, methodKey, source)` (validate realm tier ≥ method.unlockRealm + sect lock + forbiddenElements; idempotent qua P2002 catch) + `equip(charId, methodKey)` (re-validate, throw NOT_LEARNED nếu chưa học, set `equippedCultivationMethodKey`) + `getState(charId)` (return `{ equippedMethodKey, learned[] }`, lazy-grant starter cho legacy character) + `grantStarterIfMissing(charId)` (idempotent auto-grant + auto-equip `khai_thien_quyet`); (3) `CultivationProcessor.process()` compose `methodMul = methodExpMultiplierFor(equippedMethodKey)` × `cultivationMul` × `baseGain` (legacy null → methodMul 1.0); (4) `CharacterService.onboard()` gọi `cultivationMethod.grantStarterIfMissing(c.id)` sau khi tạo character — character mới luôn equip starter; (5) Controller endpoints `GET /character/cultivation-method` + `POST /character/cultivation-method/equip` (Zod validation, error mapping CONFLICT/NOT_FOUND); (6) `+29 vitest` (23 service learn/equip/getState/lazy-grant/error/multiplier + 4 processor compose + 2 onboard hook). **PR 11.2.B/11.3.D/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups còn lại theo catalog foundation đã mở.
- **Open PRs**:
  1. **this PR** — `feat(character,cultivation): Cultivation Method runtime — learn/equip + onboard auto-grant + processor compose (Phase 11.1.B)`.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.1.B) → công pháp đang equip ảnh hưởng exp gain mỗi tick (compose với linh căn).
  2. **Phase 11.1.C**: UI character profile display equipped method + list learned + equip switcher (call `POST /character/cultivation-method/equip`). Optional 24h cooldown anti-spam re-equip.
  3. **Phase 11.2.B PR runtime**: Prisma `CharacterSkill { skillKey, masteryLevel, learnedAt }` + service learn/upgrade + UI + wire `applyMasteryEffect` vào CombatService.
