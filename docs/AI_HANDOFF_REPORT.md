...
>
> **Baseline session 9j task E replay (đã verify local 30/4 ~07:55 UTC trên branch `devin/1777535897-replay-inventory-view-test-to-main` base main `2ed8c29` post-#137 merge)**: `pnpm --filter @xuantoi/web test -- --run InventoryView` ✅ **15/15**. Post-replay main sẽ có 345 + 15 = **360/360** (38 + 1 = 39 file). API test: chưa run trong session 9j (cần `pnpm infra:up`); CI matrix sẽ verify trên PR.
>
