...
- **Current `main` commit**: `868cd29` post PR #319 (Phase 11.11.C Alchemy HTTP wire). PR #246..#319 MERGED into main. **This session mở PR**: Phase 11.11.D Alchemy UI — branch `devin/1777790792-phase-11-11-d-alchemy-ui` từ `868cd29`. Frontend-only MEDIUM PR expose 2 alchemy endpoints (Phase 11.11.C, PR #319) qua `AlchemyView.vue` + Pinia store + i18n keys. New route `/alchemy` + sidebar nav `丹 Luyện Đan`. **Anti-duplicate verified**: `grep -rn alchemy apps/web/src --include='*.ts' --include='*.vue' --include='*.json'` → chỉ string "alchemy specialists" trong en.json (sect lore copy), KHÔNG có client/store/view alchemy → scope unique.
- **Previous merged PR**: [#319 feat(api,character): Phase 11.11.C Alchemy HTTP wire — POST /character/alchemy/craft + GET /character/alchemy/recipes (server-authoritative)](https://github.com/hoathienmenh-01/xuantoi/pull/319) — Phase 11.11.C controller endpoint expose `AlchemyService` (Phase 11.11.B). 16 vitest mới `character.controller.alchemy.test.ts`. Main bumped `a448ce5` → `868cd29`. CI 5/5 GREEN.
