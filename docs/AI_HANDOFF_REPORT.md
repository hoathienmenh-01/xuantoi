...
- **Current `main` commit**: `0c3cc51` (Merge PR #231 feat(shared): achievement (thành tựu) catalog foundation (Phase 11.10.A)).
- **Latest merged PR**: [#231 feat(shared): achievement (thành tựu) catalog foundation (Phase 11.10.A)](https://github.com/hoathienmenh-01/xuantoi/pull/231) — `packages/shared/src/achievements.ts` NEW catalog 32 achievement baseline (8 combat + 6 cultivation + 5 exploration + 4 social + 4 economy + 3 milestone + 2 collection × 5 tier bronze/silver/gold/platinum/diamond), +45 vitest, 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **This PR** = **Phase 11.3 PR-A — Linh căn / Spiritual Root runtime** (Prisma migration + service + onboard auto-roll). FIRST runtime PR cho Phase 11.
- **Current test baseline (main)**: **2671 vitest** = api **1133** + shared **950** + web **588** (post-#231 merge). **This PR** thêm vitest API (~14 test mới = SpiritualRootService 11 + CharacterService onboard 2 + 1 backward-compat) + 0 shared/web → total ~**2685** sau merge (api 1147 + shared 950 + web 588). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local; this PR runtime PR có thêm Prisma migration → CI sẽ chạy `prisma migrate deploy` + API integration tests.
...
- **Phase 11 progress**: **11.0 (#221) / 11.1.A (#222) / 11.2.A (#223) / 11.4.A (#224) / 11.5.A (#225) / 11.6.A (#226) / 11.X.A (#227) / 11.7.A (#228) / 11.8.A (#229) / 11.9.A (#230) / 11.10.A (#231) MERGED**. Phase 11 catalog track = **11/11 DONE** ✅. **THIS PR = Phase 11.3 PR-A — Linh căn / Spiritual Root runtime FOUNDATION** (FIRST runtime PR Phase 11). Scope: Prisma model `Character.spiritualRootGrade String? / primaryElement String? / secondaryElements String[] / rootPurity Int @default(100) / rootRerollCount Int @default(0)` + new model `SpiritualRootRollLog` (audit, idempotency `{characterId, source}` index) + migration `20260501000000_phase_11_3_spiritual_root` (nullable + default cho legacy character) + `SpiritualRootService { rollOnboard, getState }` server-authoritative (RNG inject deterministic for test, default `Math.random`) + auto-roll trong `CharacterService.onboard` (idempotent: kiểm tra existing log `source='onboard'` trước) + REST `GET /character/spiritual-root` + 14 vitest (11 service + 2 onboard integration + 1 backward-compat). KHÔNG runtime wire vào CombatService/CultivationService — đó là Phase 11.3 PR-B. **PR 11.1.B/11.2.B/11.3.B/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups tương ứng với từng catalog foundation đã mở.
- **Open PRs**:
  1. **this PR** — `feat(api): spiritual root runtime — prisma + service + onboard auto-roll (Phase 11.3.A)` — runtime PR, có Prisma migration, server-authoritative, idempotent.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.3.A Spiritual Root runtime) → Character có Linh căn server-authoritative, foundation cho Phase 11.3.B + 11.2.B.
  2. **Phase 11.3 PR-B runtime wire**: wire `elementMultiplier(attacker.primaryElement, defender.primaryElement)` vào `CombatService.computeDamage()` + wire `cultivationMethod.expMultiplier` vào `CultivationService.tick()` + UI character profile display Linh căn (icon + tooltip) + reroll service (consume `linh_can_dan` Phase 11.3.B item).
