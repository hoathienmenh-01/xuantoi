...
- **Current `main` commit**: `5c1cd0d` post PR #288 (Phase 11.4.F boss talent atkMul wire). PR #246..#288 MERGED. **No open PR / no pending branch** (this PR is in-flight Phase 11.4.G runtime wire). Baseline test suite: api 1473 vitest (85 file), shared 958 vitest (29 file), web 588 vitest (61 file) — total **3019 vitest**, all GREEN locally @ `5c1cd0d`.
- **PR pending (this session 9r-29 part 3)**: Phase 11.4.G — feat(boss,character): Talent spiritMul wire vào BossService.attack() spirit branch — symmetric với Phase 11.X.U combat path (`talentMods.spiritMul` × `effSpirit`). Wire `Math.floor((char.spirit + equip.spiritBonus) * talentMods.spiritMul)` ở spirit branch (`skill.atkScale > 1`). Hiện KHÔNG có talent catalog nào producer `kind: 'stat_mod'` `statTarget: 'spirit'` → no-balance impact baseline. Future-proof: nếu thêm talent producer (vd `talent_huyen_thuy_tam` +50% spirit), wire tự động áp dụng. Skill test: `ngung_thien_chuong` (atkScale=1.4, sect=null, mpCost=6). 4 vitest mới (1469 → 1473 api): no-learned identity, spy spiritMul=1.5 boost > baseline, no-inject TalentService identity (despite spy), atkScale=1 basic skill isolation. Public surface 100% backward-compat. Risk: 🟢 thấp (no current catalog producer → wire dormant). Rollback: revert PR (single function edit + 4 test).
- **Latest merged PR**: [#288 feat(boss,character): Talent atkMul wire vào BossService.attack() (Phase 11.4.F)](https://github.com/hoathienmenh-01/xuantoi/pull/288) — Wire `talents?.getMods(char.id).atkMul` × `(char.power + equip.atk)` ở basic-skill branch. Catalog producer `talent_kim_thien_co` (atkMul +10% kim element). 5 vitest mới (1464 → 1469 api). Main bumped `080dbd2` → `5c1cd0d`. CI 5/5 GREEN.
