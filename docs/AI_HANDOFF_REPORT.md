...
- **Current `main` commit**: `809bc0f` post PR #275 (Phase 11.X.V buff invuln wire). PR #246..#275 MERGED. **PR pending (this session 9r-27 part 4)**: **docs(CHANGELOG): catch-up session 9r-27 — PR #272 + #273 + #274 + #275** — append section [session 9r-27 — combat passive wire batch + docs catch-up — PR #272 → #275] mô tả 4 PR (2 combat wire feat + 2 docs catch-up). Pure docs, no code change. CI baseline = 2980 vitest (1438 api + 954 shared + 588 web), nó KHÔNG đổi vì docs only.
- **Latest merged PR**: [#275 feat(combat,character): Buff invuln wire vào CombatService.action() override damage (Phase 11.X.V)](https://github.com/hoathienmenh-01/xuantoi/pull/275) — Wire `buffMods.invulnActive` (kind=invuln) PRE-shield gate trong reply branch (line 404-438) + DOT branch (line 449). Pattern coverage nhất quán với cultivationBlocked (#270) + control (#264). Identity false (catalog không có producer kind=invuln) → zero balance impact. 4 vitest. Main bumped `eed0d77` → `809bc0f`. CI 5/5 GREEN.
...
- **Previous merged PR**: [#272 docs(CHANGELOG): catch-up session 9r-26 part 5 — PR #267 + #268 + #270 + #271](https://github.com/hoathienmenh-01/xuantoi/pull/272) — Pure docs/CHANGELOG catch-up. Append 1 new section to `docs/CHANGELOG.md`. CI 4/4 GREEN.
