...
> **Snapshot (session 9p audit-refresh, this PR)**: `main` @ `ecd08d6` (Merge PR #199 logs+shop+topup controller tests, 30/4 ~19:54 UTC). **Audit refresh (this PR)**: bump baseline tổng + collapse session 9p tail summary + cập nhật §17 missing APIs + roadmap untested controllers. Docs-only.
>
> **Session 9p tổng kết (PR #190 → #199, 10 PR merged trên main)**: +194 vitest accross API/web/shared.
>   - **API**: 619 → **749** (+130 vitest, 56 → 65 file). Tasks A (HealthController.readyz +10), B (admin/ledger-audit JSON +12), C (ops.service + mission.scheduler ghost-cleanup +12), H (leaderboard.controller +14), I (daily-login + next-action + giftcode controller +36), J (logs + shop + topup controller +46).
>   - **Web**: 547 → **588** (+41 vitest, 56 → 61 file). Tasks F (LocaleSwitcher +8), G (UI atoms MButton/MToast/SkeletonBlock/SkeletonTable +33).
>   - **Shared**: 220 → **238** (+18 vitest, 11 → 12 file). Task E (rollDungeonLoot/DUNGEON_LOOT/QUALITY).
>   - **Docs**: PR #193 CHANGELOG catch-up sessions 9n+ tail / 9o / 9p early.
>   - **Side update**: PR #197 sửa §17 dòng leaderboard (đã có cả 3 endpoint power/topup/sect, không còn missing).
>
> **Snapshot (session 9p task J, merged)**: `main` @ `ecd08d6` (Merge PR #199, 30/4 ~19:54 UTC). **PR #199**: `apps/api/src/modules/{logs,shop,topup}/*.controller.test.ts` (+46 vitest). API baseline **703 → 749**. CI ✅.
>
...
## Recent Changes (PR #33→#199 đã merged trên main; session 9p audit-refresh **this PR** docs-only sweep + bump tổng baseline + roadmap untested controllers)

### PR session 9p audit-refresh (in-flight, this PR) — `docs(handoff): audit refresh post PR #199 — bump baseline + session 9p summary + roadmap untested controllers` — **Pending merge**

- **Branch**: `devin/1777578804-audit-refresh-9p-tail`. **Base**: `main` @ `ecd08d6` (post PR #199 merge).
- **Vì sao**: User yêu cầu "cap nhat audit" sau khi PR #199 vừa merged. Sau 10 PR liên tiếp (#190 → #199, session 9p), report cần một sweep tổng:
  - Bump baseline tổng (API 749, web 588, shared 238) chứ không chỉ bump per-PR.
  - Collapse 10 entry snapshot 9p A→J thành một bản tổng kết để người đọc thấy ngay.
  - Cập nhật §17 Missing APIs row leaderboard (đã sửa ở PR #197 nhưng cần restate).
  - Cập nhật §21 row task J từ "this PR" → merged @ `ecd08d6`.
  - Cập nhật roadmap với các controller còn untested để session sau biết nhặt task tiếp.
- **Files**: `docs/AI_HANDOFF_REPORT.md` only.
- **Untested controllers còn lại** (do session 9p chỉ làm các controller nhỏ; các controller lớn còn để session sau):
  - `apps/api/src/modules/admin/admin.controller.ts` (562 line — lớn, cần split coverage)
  - `apps/api/src/modules/auth/auth.controller.ts` (236 line)
  - `apps/api/src/modules/sect/sect.controller.ts` (169 line)
  - `apps/api/src/modules/market/market.controller.ts` (146 line)
  - `apps/api/src/modules/inventory/inventory.controller.ts` (126 line)
  - `apps/api/src/modules/combat/combat.controller.ts` (124 line)
  - `apps/api/src/modules/boss/boss.controller.ts` (122 line)
  - `apps/api/src/modules/chat/chat.controller.ts` (104 line)
  - `apps/api/src/modules/mail/mail.controller.ts` (103 line)
  - `apps/api/src/modules/mission/mission.controller.ts` (83 line)
- **Tests**: docs-only, không ảnh hưởng. Baseline xác minh local 30/4 19:54 UTC: API **749/749** (65 file) · web **588/588** (61 file) · shared **238/238** (12 file).
- **Risk**: 🟢 zero — docs-only.
- **Rollback**: revert single PR.

### PR #199 — `test(api): logs + shop + topup controller +46 vitest (session 9p task J)` — **Merged into main** @ `ecd08d6` (30/4 ~19:54 UTC). CI ✅.
...
| `GET /api/leaderboard/{power,topup,sect}` | **All 3 done**: `topByPower` (PR #59), `topByTopup` + `topBySect` thêm sau đó (`leaderboard.service.ts` 199 line, 27 vitest, controller test +14 vitest PR #197 session 9p task H merged). | All done. |
...
| #199 | session 9p task J — test(api): logs + shop + topup controller +46 vitest | **Merged into main** @ `ecd08d6` (30/4 ~19:54 UTC, CI ✅) |
| this PR | session 9p audit-refresh — docs(handoff): bump baseline + session 9p summary + roadmap untested controllers | **Pending merge** (in-flight) |
