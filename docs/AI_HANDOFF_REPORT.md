...
- **Current `main` commit**: `c55c563` (Merge PR #218 feat(shared): boss pack 1 (+10 named boss, ngũ hành)).
- **Latest merged PR**: [#218 feat(shared): boss pack 1 (+10 named boss, ngũ hành)](https://github.com/hoathienmenh-01/xuantoi/pull/218) — Phase 10 PR-5 (boss catalog 2 → 12, BossDef forward-compat extend `level/element/regionKey/monsterType/lowDropPool`, +28 vitest balance guard, 5/5 CI GREEN).
- **Current phase**: **Phase 10 — Content scale** (PR-5 Boss Pack 1 đã merge, PR-4 Mission Pack 1 [#217](https://github.com/hoathienmenh-01/xuantoi/pull/217) Pending merge với CI GREEN, this PR Phase 9.5 `smoke:combat` polish — không block Phase 11). Phase 9 closed beta stabilization đã APPROVED gate (xem [#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). Phase 10 PR-1..5 plan: items (DONE) / skills (DONE) / monsters & dungeons (DONE) / missions (Pending merge #217) / **boss (DONE)** — theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md).
- **Current test baseline (main)**: **2079 vitest** = api **1133** + shared **358** + web **588**. **This PR** không thêm vitest (chỉ thêm `scripts/smoke-combat.mjs` HTTP smoke — Node 20 native fetch, no vitest). **PR #217 (PR-4)** thêm `missions-balance.test.ts` (+27 test) → shared 358 → 385, total 2106 (sau merge). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / **`smoke:combat` (18 step, this PR)**.
- **Phase 9 readiness** (snapshot session 9r-2 — chi tiết bảng A–O ở [`BETA_CHECKLIST.md`](./BETA_CHECKLIST.md) §"Phase 9 readiness audit"): **9/15 nhóm Done**, **5 nhóm Partial / Needs runtime smoke**, **1 nhóm Needs prod review**. **Phase 10 gate**: ✅ APPROVED. **Phase 9.5 smoke gap**: nhóm D (Dungeon / combat / loot) đã có HTTP smoke từ this PR (`smoke:combat`); chỉ còn nhóm I (Admin) → `smoke:admin` deferred (vẫn không block Phase 11).
- **Phase 10 progress**: **PR-1 (DONE)** Item Pack 1 — catalog **31 → 81** items (+50). **PR-2 (DONE)** Skill Pack 1 — catalog **10 → 25** skill (+15, Ngũ Hành). **PR-3 (DONE)** Monster & Dungeon Pack 1 — monster catalog **9 → 29** (+20, Ngũ Hành), dungeon catalog **3 → 9** (+6), DUNGEON_LOOT **3 → 9**, ITEMS **81 → 87** (+6 yêu phù). **PR-4 (Pending merge — #217)** Mission Pack 1 — mission catalog **12 → 66** (+54 mission, daily/weekly/once × Ngũ Hành element bonus + chain quest), MissionDef forward-compat (`element` / `regionKey` / `storyChainKey` / `realmTier`), +27 vitest. **PR-5 (DONE — #218)** Boss Pack 1 — boss catalog **2 → 12** (+10 named boss), BossDef forward-compat extends, +28 vitest. **Phase 10 status**: 4/5 PR merged (PR-1, 2, 3, 5), PR-4 Pending merge với CI GREEN.
- **Open PRs**: 
  ...
  2. this PR — `chore(scripts): smoke:combat HTTP runtime smoke (Phase 9.5 polish)` — không phụ thuộc Phase 10/11.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge PR #217** (mission pack) khi user duyệt → Phase 10 content scale CLOSE 5/5.
  2. **Merge this PR** (smoke:combat) → close 1/2 deferred runtime smoke gap.
  3. **Optional `smoke:admin`** — close nhóm I (Admin) deferred runtime smoke gap (không block Phase 11).
  4. **Phase 11.0 prep** — schema migration `Character.spiritualRoot` (5-tier element grade + primaryElement + secondaryElements + rootPurity + rootRerollCount) + onboarding roll service + lookup `elementMultiplier(skill, target)` để wire `element` field hiện chỉ-metadata thành runtime damage modifier (Phase 11.2/11.3).
