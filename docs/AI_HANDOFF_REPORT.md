...
- **Current `main` commit**: `5d5c667` post PR #279 (Phase 11.10.G Achievement catalog cross-ref test). PR #246..#279 MERGED. **PR pending (this session 9r-28)**: **feat(character): Title auto-unlock realm milestone wire vào CharacterService.breakthrough() (Phase 11.9.C)** — wire `TitleService.unlockTitle(charId, def.key, 'realm_milestone')` fail-soft after successful realm advance trong `CharacterService.breakthrough()`. Inject `TitleService` (positional optional 6th param, NestJS DI auto-resolve from same `CharacterModule`). Use `titleForRealmMilestone(newRealm)` từ shared catalog để find title def. Skip nếu (a) `next === null` (đã đạt cao nhất), (b) catalog không có title cho realm (vd hoa_than → luyen_hu), (c) `titles` injection missing (backward-compat). Idempotent qua `CharacterTitleUnlock` composite UNIQUE `(characterId, titleKey)`. Try/catch với Logger.warn — title unlock lỗi KHÔNG fail breakthrough core path. CI baseline 2985 → 2989 vitest (956 shared + 1445 api + 588 web).
- **Latest merged PR**: [#279 test(achievements): catalog cross-ref invariants — itemKey + rewardTitleKey (Phase 11.10.G)](https://github.com/hoathienmenh-01/xuantoi/pull/279) — Test-only catalog referential integrity hardening cho `packages/shared/src/achievements.test.ts`. Add 2 vitest: (1) `mọi reward.items[*].itemKey PHẢI tồn tại trong ITEMS catalog`, (2) `mọi rewardTitleKey PHẢI tồn tại trong TITLES catalog`. Bảo vệ Phase 11.10.D `claimReward` items grant path từ catalog drift. Main bumped `817c404` → `5d5c667`. CI 5/5 GREEN.
...
- **Open PRs (state 2/5 2026 session 9r-28)**: 1 open PR on main = this PR (feat(character): Title auto-unlock realm milestone wire vào CharacterService.breakthrough() — Phase 11.9.C).

- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 11.9.C-2 Title auto-unlock — Tribulation success path** — wire `titleForRealmMilestone(toRealmKey)` + `unlockTitleTx` trong `TribulationService.attemptTribulation()` SUCCESS branch (tx-aware, atomic). Mirror pattern from this PR (`CharacterService.breakthrough` low-tier).
  2. **Phase 11.9.C-3 Title auto-unlock — onboard `realm_luyenkhi_initiate`** — wire `titles?.unlockTitle(charId, 'realm_luyenkhi_initiate', 'realm_milestone')` fail-soft trong `CharacterService.onboard()` cho character mới (idempotent retry-safe). Optional — luyenkhi là realm khởi đầu, mọi nhân vật unlock 1 lần.
  3. **Phase 11.7.D Talent active wire** — wire active talent skill cast vào `CombatService.action(skillKey)` lookup.
  4. **Phase 11.8.D Buff → Tribulation Tâm Ma wire** — tribulation FAIL → `applyBuff('debuff_taoma', 'tribulation')` thay cho `Character.taoMaUntil`.
  5. **Phase 11.3.D UI Linh căn** — reroll dialog + element wheel.
  6. **Phase 11.1.C UI cultivation method** — equip switcher.
  7. **Phase 11.2.C UI skill book** — skill grimoire + mastery upgrade.
  8. **Phase 11.5.C UI Refine** — inventory equip detail refine button.
  9. **Phase 11.10.F Achievement catalog item rewards** — add 1-2 milestone achievements với `reward.items` non-empty để verify wire của PR #277 runtime thực tế (vd `first_market_buy` grant `huyet_chi_dan` x 5; balance review needed).
  10. **Optional smoke gap polish**: `smoke:cultivation` / `smoke:topup` / `smoke:admin` v2 / `smoke:breakthrough`.
