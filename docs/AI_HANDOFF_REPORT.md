...
- **Current `main` commit**: `995a459` (Merge PR #223 feat(shared): skill template catalog foundation (Phase 11.2.A)).
- **Latest merged PR**: [#223 feat(shared): skill template catalog foundation (Phase 11.2.A)](https://github.com/hoathienmenh-01/xuantoi/pull/223) — `packages/shared/src/skill-templates.ts` NEW (~470 lines) progression catalog 26 template 1-1 với `SKILLS` (combat.ts), 5-tier `SkillTier` (basic/intermediate/advanced/master/legendary) + `SKILL_TIER_DEFS` table monotonic + `generateMasteryCurve(tier)` deterministic curve + `SkillUnlockRequirement` 6-kind + `SkillEvolutionBranch` (legendary) + 2 legendary evolution skills (van_kiem_quy_tong, huyet_ma_giang_the) với 4 tổng evolution branch. Helper `getSkillTemplate/templatesByTier/templatesByUnlock/applyMasteryEffect/masteryUpgradeCost/findOrphan*`, +63 vitest (shared 436 → 499), 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). **Phase 11 — Progression Depth** đã mở. **PR #221 (Phase 11.0 Spiritual Root catalog) MERGED** + **PR #222 (Phase 11.1.A CultivationMethod catalog) MERGED** + **PR #223 (Phase 11.2.A SkillTemplate catalog) MERGED** vào main `995a459`. Phase 11 catalog foundation track = **3/3 DONE**. **This PR** = **Phase 11.4.A Gem catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2220 vitest** = api **1133** + shared **499** + web **588** (post-#223 merge). **This PR** thêm **+39 vitest** (shared 499 → 538) → total **2259/2259** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **PR 11.0 (DONE — #221)** Spiritual Root catalog — `packages/shared/src/spiritual-root.ts` + 43 vitest. **PR 11.1.A (DONE — #222)** CultivationMethod catalog — `packages/shared/src/cultivation-methods.ts` + 35 vitest, 12 method. **PR 11.2.A (DONE — #223)** SkillTemplate catalog — `packages/shared/src/skill-templates.ts` + 63 vitest, 26 template 1-1 SKILLS, 5 tier + mastery curve + 4 evolution branch. **PR 11.4.A (this PR open)** Gem catalog foundation — `packages/shared/src/gems.ts` NEW + 39 vitest, 25 gem baseline (5 element × 5 grade) + helpers (getGemDef/composeSocketBonus/combineGems/canSocketGem/gemUpgradePathCost). **PR 11.1.B (Pending)** CultivationMethod runtime. **PR 11.2.B (Pending)** SkillTemplate runtime. **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4.B (Pending)** Gem runtime (Equipment.sockets + socketGem/unsocketGem service + applySocketBonus). **PR 11.5..7 (Pending)** Refine / Tribulation / Talent.
- **Open PRs**:
  1. **this PR (#TBD)** — `feat(shared): gem catalog foundation (Phase 11.4.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.4.A Gem catalog) → unblock Phase 11.4.B runtime.
  2. **Phase 11.5.A PR**: Refine catalog static (`packages/shared/src/refine.ts`) trước khi DB-backed runtime — catalog-only, low-risk.
