...
> **Snapshot (session 9k task F — this PR, SECURITY.md logout-all doc)**: `main` @ `f1214a3` (Merge PR #150 AdminView render-level 18 vitest, 30 Apr 2026 ~09:10 UTC). **Session 9k close-out status**: task A Merged PR #149 @ `5a815b3` · task C Merged PR #150 @ `f1214a3` · task D Pending merge PR #151 (CI ✅) · task E Pending merge PR #152 (CI ✅) · task B Pending merge PR #153 (CI ✅). **In-flight (this PR — session 9k task F / backlog M9)**: `docs(security): document logout-all không bump passwordVersion — M9 close-out` — thêm bullet vào `docs/SECURITY.md §1` giải thích hành vi `POST /api/_auth/logout-all` revoke refresh tokens nhưng không bump `passwordVersion` → access tokens TTL 15 phút vẫn valid trên device khác cho tới khi hết hạn; force-kill ngay phải đổi password hoặc bump `JWT_ACCESS_SECRET`. Behavior intentional (code `auth.service.ts:364-375` comment đã nêu). Risk 🟢 thấp — docs-only, không touch code/test/CI. File: `docs/SECURITY.md` (+1 bullet) + `docs/AI_HANDOFF_REPORT.md` (M9 → Pending merge).
>
...
- `M9` document SECURITY.md trường hợp logout-all không bump `passwordVersion` — **Pending merge, session 9k task F (this PR, docs-only)**. Thêm bullet §1 note: logout-all revoke refresh tokens nhưng không bump passwordVersion → access tokens 15-phút TTL vẫn valid trên device khác tới khi hết hạn; cần force-kill ngay phải đổi password hoặc bump `JWT_ACCESS_SECRET`. Behavior intentional.
