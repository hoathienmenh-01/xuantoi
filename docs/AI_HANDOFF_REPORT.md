...
- **Current `main` commit**: `a8ece95` post PR #308 (Phase 11.X.AO+AP+AQ combined relaunch — 15 talent producers landed: 5 mpMax + 5 mpRegen + 5 tương sinh damage_bonus). PR #246..#308 MERGED into main. **PassiveTalentMods catalog symmetry HOÀN TẤT**: 7/7 statTarget × 5/5 elements = 35 stat producers + 2/2 damage_bonus chains × 5/5 elements = 10 damage_bonus producers (tương khắc 1.15 + tương sinh 1.1, multiplicative stack 1.265 trên cùng target). **1 open PR (this session)**: Phase 11.X.AR — UI Talent Catalog read-only view, expose 54-talent shared catalog (47 passive + 7 active, 10 talents/element + 4 neutral) cho user qua `/talents` route. Baseline test suite @ main `a8ece95`: api 1487 vitest, shared 1015 vitest (post-AN+AO+AP+AQ), web 588 vitest = **3090 vitest** GREEN. Local AR work: web 602 (+14 new vitest cho TalentCatalogView render/filter type/filter element/empty state/effect summary format/realm cost render), api 1487, shared 1015 = **3104 vitest** GREEN. Catalog 5-element symmetry roadmap → COMPLETE. Element-aware combat damage_bonus đã wire active runtime trong `composePassiveTalentMods()` từ Phase 11.7.C, talent UI read-only mở route đầu tiên cho user thấy roadmap visible.
- **PR pending (this session, branched on main `a8ece95`)**: Phase 11.X.AR — feat(web,talents,ui): TalentCatalogView read-only `/talents` route, render 54-talent từ shared catalog với filter (type passive/active + element kim/moc/thuy/hoa/tho/neutral/all), card layout (name + element badge + type badge + realm requirement + talent point cost + description + effect summary). MEDIUM PR scope = single objective "expose talent catalog cho user visibility". 7 files: `apps/web/src/views/TalentCatalogView.vue` (new), `apps/web/src/views/__tests__/TalentCatalogView.test.ts` (new), `apps/web/src/router/index.ts` (+route entry), `apps/web/src/router/__tests__/routes.test.ts` (+talents EXPECTED), `apps/web/src/components/shell/AppShell.vue` (+nav link 技 Ngộ Đạo), `apps/web/src/components/shell/__tests__/AppShell.test.ts` (+talents key), `apps/web/src/i18n/vi.json` + `en.json` (+shell.nav.talents + talents.* section). 14 vitest mới web (588 → 602). Risk: 🟢 thấp (read-only frontend, no API/DB/runtime change, catalog data từ static shared package). Rollback: revert PR.
- **Latest merged PR**: [#308 feat(catalog,talents): re-land 15 producers vào main — mpMax + mpRegen + tương sinh damage_bonus (Phase 11.X.AO+AP+AQ relaunch)](https://github.com/hoathienmenh-01/xuantoi/pull/308) — MEDIUM PR combined re-land 15 producers sau stacked-merge drift. 20 vitest mới (995 → 1015 shared, full suite 3070 → 3090). Main bumped `70337ca` → `a8ece95`. CI all GREEN. Catalog `PassiveTalentMods` 5-element symmetry roadmap COMPLETE.
