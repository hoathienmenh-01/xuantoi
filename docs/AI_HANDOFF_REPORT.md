...
- **Current `main` commit**: `f84afb8` (Merge PR #216 feat(shared): monster & dungeon pack 1 (+20 monster, +6 dungeon, ngũ hành)).
- **Latest merged PR**: [#216 feat(shared): monster & dungeon pack 1 (+20 monster, +6 dungeon, ngũ hành)](https://github.com/hoathienmenh-01/xuantoi/pull/216) — Phase 10 PR-3 (monster catalog 9 → 29, dungeon catalog 3 → 9, DUNGEON_LOOT 3 → 9 entries, ITEMS 81 → 87, MonsterDef/DungeonDef forward-compat extends, +45 vitest balance guard, 5/5 CI GREEN).
- **Current phase**: **Phase 10 — Content scale** (PR-5 Boss Pack 1 — Ngũ Hành — in-flight; PR-4 Mission Pack 1 [#217](https://github.com/hoathienmenh-01/xuantoi/pull/217) đang Pending merge với CI GREEN). Phase 9 closed beta stabilization đã APPROVED gate (xem [#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). Phase 10 PR-1..5 plan: items (DONE) / skills (DONE) / monsters & dungeons (DONE) / missions (Pending merge #217) / boss catalog (this PR) — theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md).
- **Current test baseline (main)**: **2051 vitest** = api **1133** + shared **330** + web **588**. **This PR (PR-5)** thêm `boss-balance.test.ts` (+28 test) → shared 330 → 358, total 2079. **PR #217 (PR-4)** thêm `missions-balance.test.ts` (+27 test) → shared 330 → 357, total 2078. **Sau khi cả #217 + this PR merge** → shared 330 → 385, total 2106. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR).
- **Phase 9 readiness** (snapshot session 9r-2 — chi tiết bảng A–O ở [`BETA_CHECKLIST.md`](./BETA_CHECKLIST.md) §"Phase 9 readiness audit"): **9/15 nhóm Done**, **5 nhóm Partial / Needs runtime smoke**, **1 nhóm Needs prod review**. **Phase 10 gate**: ✅ APPROVED.
- **Phase 10 progress**: **PR-1 (DONE)** Item Pack 1 — catalog **31 → 81** items (+50). **PR-2 (DONE)** Skill Pack 1 — catalog **10 → 25** skill (+15, Ngũ Hành). **PR-3 (DONE)** Monster & Dungeon Pack 1 — monster catalog **9 → 29** (+20, Ngũ Hành), dungeon catalog **3 → 9** (+6), DUNGEON_LOOT **3 → 9**, ITEMS **81 → 87** (+6 yêu phù). **PR-4 (Pending merge — #217)** Mission Pack 1 — mission catalog **12 → 66** (+54 mission, daily/weekly/once × Ngũ Hành element bonus + chain quest), MissionDef forward-compat (`element` / `regionKey` / `storyChainKey` / `realmTier`), +27 vitest. **PR-5 (this PR)** Boss Pack 1 — boss catalog **2 → 12** (+10 named boss), `BossDef` extend forward-compat (`level` / `element` / `regionKey` / `monsterType: 'BOSS'` / `lowDropPool`) cho phase 11.3 elementMultiplier + phase 12 BossRewardService pity / share-ratio. Boss tier kim_dan → nguyen_anh → hoa_than → luyen_hu → hop_the × Ngũ Hành kim/moc/thuy/hoa/tho + 1 cross-element endgame. Combat / boss runtime KHÔNG đổi. Stat budget tuân thủ [`BALANCE_MODEL.md`](./BALANCE_MODEL.md) §6 (hp 100k..5M, atk band hp/8000..hp/1000, def ≤ atk, baseRewardLinhThach band hp/8..hp/2), bound bằng `boss-balance.test.ts` 28 test. 3 helper mới: `bossesByElement` / `bossesByRegion` / `bossesByRealm`.
- **Open PRs**: 
  1. [#217](https://github.com/hoathienmenh-01/xuantoi/pull/217) — `feat(shared): mission pack 1 (+54 mission, ngũ hành chain quest)` — Phase 10 PR-4, **CI GREEN**, ready merge.
  2. this PR — `feat(shared): boss pack 1 (+10 named boss, ngũ hành)` — Phase 10 PR-5.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge PR #217** (mission pack) khi user duyệt — không block PR-5.
  2. **Merge this PR** (boss pack) → Phase 10 content scale CLOSE → Phase 11 gate review.
  3. **Optional Phase 9.5 polish** — viết `smoke:combat` / `smoke:admin` để cover 2 deferred runtime smoke gap (không block Phase 11 nếu user duyệt).
  4. **Phase 11.0 prep** — schema migration `Character.spiritualRoot` (5-tier element) + lookup `elementMultiplier(skill, target)` để wire `element` field hiện chỉ-metadata thành runtime damage modifier.
