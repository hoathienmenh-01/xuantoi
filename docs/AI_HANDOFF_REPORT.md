...
- **Current `main` commit**: `ea2c67f` (Merge PR #209 add `pnpm smoke:ws`, 1/5 ~02:10 UTC).
- **Latest merged PR**: [#209 test: add websocket smoke for realtime safety](https://github.com/hoathienmenh-01/xuantoi/pull/209) (test/QA, +1 root script + 2 docs, zero new dep).
- **Current phase**: **Phase 9 — Closed beta stabilization** (audit refresh, smoke:economy ✅, smoke:ws ✅, Playwright golden path ✅ (this PR — 11 spec / 11 pass `E2E_FULL=1`), bug triage P0/P1, mobile/i18n polish, release checklist). Xem [`LONG_TERM_ROADMAP.md`](./LONG_TERM_ROADMAP.md) §Phase 9.
- **Current test baseline**: **1959 vitest** = api **1133** + shared **238** + web **588**. CI ✅ trên `main`. (Web baseline tăng 555→588 từ các PR session 9q sau snapshot 9q-4.) Playwright `E2E_FULL=1` golden path = **11 spec / 11 pass** trong ~17–19s (manual/gated, không trong CI mặc định — cần Postgres + Redis + API + Web dev cùng chạy, xem [`QA_CHECKLIST.md`](./QA_CHECKLIST.md) §12).
- **Open PRs**: this PR — `test(e2e): expand golden path for closed beta core loop`. Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Bug triage P0/P1** — audit GitHub issue P0/P1 + tags `phase-9` / `closed-beta` / `regression`; ưu tiên fix trước khi mở rộng content. Mỗi fix một PR nhỏ, có vitest regression test nếu có thể.
  2. **Playwright golden path → CI** — config Playwright job runtime trên CI (Postgres + Redis service container) để chạy `E2E_FULL=1` mỗi PR đụng `apps/web` hoặc `apps/api`. Hiện manual/gated.
...
## Recent Changes (PR #33→#209 đã merged trên main; session 9q-7 **this PR** mở rộng Playwright golden path cho closed beta core loop trước Phase 10 content scale)

### PR session 9q-7 (in-flight, this PR) — `test(e2e): expand golden path for closed beta core loop` — **Pending merge**
