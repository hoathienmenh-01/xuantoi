...
- **Current `main` commit**: `6fc0b66` (Merge PR #226 feat(shared): tribulation catalog foundation (Phase 11.6.A)).
- **Latest merged PR**: [#226 feat(shared): tribulation catalog foundation (Phase 11.6.A)](https://github.com/hoathienmenh-01/xuantoi/pull/226) — `packages/shared/src/tribulation.ts` NEW (~480 lines) catalog 8 kiếp baseline cover realm threshold `kim_dan→thanh_nhan`, 5 type (`lei/phong/bang/hoa/tam`) × 4 severity (`minor/major/heavenly/saint`), wave count 3/5/7/9 + baseDamage geometric `severityBase × 1.35^waveIdx`, reward linhThach 5k/25k/150k/1M + expBonus BigInt + uniqueDropItemKey heavenly/saint, failurePenalty expLossRatio 10/20/35/50% + cooldown 30/60/120/240m + Tâm Ma debuff chance 5/10/20/30%, helpers `getTribulationDef/getTribulationForBreakthrough/tribulationsByType/Severity/simulateTribulationWave/simulateTribulation/computeTribulationReward/computeTribulationFailurePenalty` deterministic, +52 vitest (shared 594 → 646), 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). **Phase 11 — Progression Depth** đã mở. **PR #221..#226 (6/6 catalog foundation track) MERGED** vào main `6fc0b66`. Phase 11 catalog foundation track = **6/6 DONE**. **This PR** = **Phase 11.X.A Alchemy catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2367 vitest** = api **1133** + shared **646** + web **588** (post-#226 merge). **This PR** thêm **+55 vitest** (shared 646 → 701) → total **2422/2422** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **PR 11.0 (DONE — #221)** Spiritual Root catalog. **PR 11.1.A (DONE — #222)** CultivationMethod catalog. **PR 11.2.A (DONE — #223)** SkillTemplate catalog. **PR 11.4.A (DONE — #224)** Gem catalog. **PR 11.5.A (DONE — #225)** Refine catalog. **PR 11.6.A (DONE — #226)** Tribulation catalog. **PR 11.X.A (this PR open)** Alchemy catalog foundation — `packages/shared/src/alchemy.ts` NEW + 55 vitest, 13 recipe baseline cover toàn bộ pill HP/MP/EXP × PHAM..THAN, schema `AlchemyRecipeDef { key, outputItem, outputQty, outputQuality, inputs[], furnaceLevel, realmRequirement, linhThachCost, successRate }`, helpers `getAlchemyRecipeDef/alchemyRecipesByQuality/alchemyRecipesByOutputItem/alchemyRecipesAvailableAtFurnace/getAlchemyIngredientTotal/getExpectedAlchemyAttempts/simulateAlchemyAttempt/simulateAlchemyBulk` deterministic. **PR 11.1.B (Pending)** CultivationMethod runtime. **PR 11.2.B (Pending)** SkillTemplate runtime. **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4.B (Pending)** Gem runtime (Equipment.sockets + socketGem/unsocketGem service + applySocketBonus). **PR 11.5.B (Pending)** Refine runtime (Equipment.refineLevel + service `refineEquipment` + wire `getRefineStatMultiplier`). **PR 11.6.B (Pending)** Tribulation runtime (Tribulation Prisma model + service `attemptTribulation` + cooldown enforcement + Tâm Ma debuff modifier). **PR 11.X.B (Pending)** Alchemy runtime (apps/api/src/modules/alchemy + ItemLedger consume/grant atomic + idempotency). **PR 11.7 (Pending)** Talent.
- **Open PRs**:
  1. **this PR (#TBD)** — `feat(shared): alchemy catalog foundation (Phase 11.X.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.X.A Alchemy catalog) → unblock Phase 11.X.B runtime.
  2. **Phase 11.3 PR runtime** (lớn nhất pending): Prisma migration `Character.spiritualRootGrade/primaryElement/secondaryElements/rootPurity/rootRerollCount/cultivationMethodKey` + onboarding roll service + wire `elementMultiplier` + `cultivationMethod.expMultiplier` vào runtime.
