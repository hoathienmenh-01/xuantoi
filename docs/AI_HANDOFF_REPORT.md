...
- **Current `main` commit**: `93431a9` post PR #320 (Phase 11.11.D Alchemy UI). PR #246..#320 MERGED into main. **This session mở PR**: **PR #321 (IN-FLIGHT)** Phase 11.11.E Alchemy + Achievement integration — branch `devin/1777791841-phase-11-11-e-alchemy-achievement` rebased từ `868cd29` lên `93431a9` (resolve docs conflict only). SMALL backend-only PR. **Scope**: extend `MissionGoalKind` union với `'ALCHEMY_CRAFT'` + 2 alchemy achievements (`pill_apprentice` bronze 10/`pill_master` gold 100, milestone category, no rewardTitleKey) trong `packages/shared/src/achievements.ts` + `@Optional() AchievementService` DI vào `AlchemyService` + post-success fail-soft `trackEvent('ALCHEMY_CRAFT', 1)` (KHÔNG track khi outcome.success=false; KHÔNG throw khi achievements service lỗi). 5 vitest mới (`alchemy.service.test.ts` Phase 11.11.E section). **Anti-duplicate verified**: `grep -rn "ALCHEMY_CRAFT" packages/shared apps/api` pre-edit → 0 results, scope unique. Pattern theo combat.service.ts/cultivation.processor.ts (`Phase 11.10.C-2 wire trackEvent vào achievement bằng cùng goalKind` — fail-soft try/catch).
