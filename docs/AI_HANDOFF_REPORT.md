...
- **Current `main` commit**: `e646237` post PR #243 (Phase 11.8.B Buff MVP runtime). PR #244 (Phase 11.7.B Talent MVP runtime) IN FLIGHT — branch `devin/1777698370-phase-11-7-b-talent-mvp-runtime` rebase on top of merged #243.
- **Pending PR #244**: Phase 11.7.B Talent (Thần Thông) MVP runtime — passive talent persistence service. New `TalentService` (`learnTalent` + `listLearned` + `getRemainingTalentPoints` + `getMods`) qua `prisma.$transaction` + new model `CharacterTalent` (composite UNIQUE `(characterId, talentKey)` + 1 index + FK cascade) + reuse `composePassiveTalentMods` từ `packages/shared/src/talents.ts` (Phase 11.7.A catalog đã merge). MVP đổ sang persistence + budget enforcement (`computeTalentPointBudget(realmOrder)` 3-realm milestone rule + `canCharacterLearnTalent(...)` realm/cost gate); KHÔNG wire vào `CharacterStatService.computeStats` + KHÔNG `useActiveTalent` (defer Phase 11.7.C). 17 vitest cover happy path, gating (REALM_TOO_LOW + INSUFFICIENT_TALENT_POINTS + ALREADY_LEARNED + TALENT_NOT_FOUND + CHARACTER_NOT_FOUND), listLearned, getRemainingTalentPoints, getMods (`composePassiveTalentMods` parity), cross-character isolation, error class.
- **Latest merged PR**: [#243 feat(character): Buff/Debuff MVP runtime](https://github.com/hoathienmenh-01/xuantoi/pull/243) — Phase 11.8.B server-authoritative duration buff/debuff persistence. New `BuffService` (`applyBuff`/`removeBuff`/`listActive`/`pruneExpired`/`getMods`) qua `prisma.$transaction` + new model `CharacterBuff` (composite UNIQUE + 2 index + FK cascade) + 18 vitest. Main bumped `a2721b1` → `e646237`. CI 5/5 GREEN.
...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.1.B/11.2.B/11.3.A/B/C MERGED** (Cultivation Method + Skill Template + Linh căn runtime đầy đủ) ✅. **Phase 11.4.B MERGED** (Gem MVP — PR #239) ✅. **Phase 11.5.B MERGED** (Refine MVP — PR #240) ✅. **Phase 11.6.B MERGED** (Tribulation MVP — PR #242) ✅. **Phase 11.8.B MERGED** (Buff persistence MVP — PR #243) ✅. **Phase 11.7.B IN FLIGHT** (Talent persistence MVP — PR #244, this PR). **NEXT runtime track**: 11.7.C Talent wire (`composePassiveTalentMods` vào `CharacterStatService.computeStats` + active talent execution + cooldown), 11.8.C Buff wire (compose `getMods()` vào stat compute + tribulation FAIL applyBuff('debuff_taoma') thay cho `Character.taoMaUntil`), 11.9.B Title runtime, 11.10.B Achievement runtime, 11.3.D UI Linh căn (reroll dialog + element wheel), 11.1.C/11.2.C/11.5.C/11.6.C UI cultivation method + skill book + refine + tribulation.
- **Current test baseline (main `e646237` + PR #244 in flight)**: **~2842 vitest** = api **1300** (PR #244 +17 TalentService over baseline 1283 = main 1266 + PR #243 +18 - 1 minor offset; actual exact API count CI-verified) + shared **954** + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN trên main per CI run của PR #243. **Local verified PR #244 staging**: typecheck ✓ (api+shared+web no error), lint ✓ (zero warnings), shared `pnpm test` 954/954 ✓, web `pnpm test` 588/588 ✓, build ✓. **API test baseline ~1300 cần Postgres** → CI verify. **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
