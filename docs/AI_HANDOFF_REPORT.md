...
> **Snapshot (session 9n-H in-flight, CHANGELOG catch-up sessions 9m + 9n — rebased post-#171)**: `main` @ `c02573a` (Merge PR #171 `fix(web,i18n): api fallback Error messages`, 30 Apr 2026 ~13:15 UTC). **Session 9n progress (7 PR merged)**: PR #165..#171 merged. **This PR (9n-H, branch `devin/1777555378-changelog-session-9n`)**: docs-only — `docs/CHANGELOG.md` backfill section session 9m (PR #160..#164 — API service test coverage push +36 vitest topup/email/giftcode-race) + session 9n (PR #165..#171 — smart audit-ledger/economy-alerts CLI, env-tunable alert thresholds, TROUBLESHOOTING §15/§16, toast i18n title parity, api fallback i18n parity). Trước đây CHANGELOG dừng ở session 9l, không phản ánh các session sau.
