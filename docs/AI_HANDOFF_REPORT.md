...
- **Current `main` commit**: `70337ca` post PR #304 (Phase 11.X.AN catalog 4 hpRegen producers kim/thuy/hoa/tho hoàn tất 5-element hpRegen coverage). PR #246..#304 MERGED into main. **PR #305/#306/#307 status**: trên GitHub các PR này hiển thị `merged` nhưng thực tế chỉ merge vào base BRANCH (stacked devin/...-an-... → ...-ao-... → ...-ap-...) trước khi #304 squash-merge vào main, nên content AO/AP/AQ KHÔNG hiện diện trên main — chỉ AN landed. **1 open PR (this session)**: Phase 11.X.AO+AP+AQ relaunch — combined re-land 15 talent producers (5 mpMax + 5 mpRegen + 5 damage_bonus tương sinh) bị lỡ landing vào main do stacked merge artifact. Baseline test suite @ main `70337ca`: api 1487 vitest, shared 995 vitest (post-AN), web 588 vitest = **3070 vitest** GREEN. Local relaunch work: shared 1015 (+20 new vitest = 6 mpMax + 7 mpRegen + 7 tương sinh), api 1487, web 588 = **3090 vitest** GREEN. **7/7 statTarget path của `PassiveTalentMods` 5-element coverage hoàn tất** (atk/def/hpMax/spirit/mpMax + hpRegen/mpRegen) + **2/2 damage_bonus chain (tương khắc + tương sinh) 5-element coverage hoàn tất** sau khi PR này land — toàn bộ catalog `PassiveTalentMods` paths đã đối xứng ngũ hành.
- **PR pending (this session relaunch combined, branched on main `70337ca`)**: Phase 11.X.AO+AP+AQ relaunch — feat(catalog,talents): re-land 15 producers (5 mpMax `*_linh_hai` cost 1 +10% MP tối đa kim_dan realm + 5 mpRegen `*_linh_dan` cost 1 +5 MP regen flat truc_co realm + 5 tương sinh damage_bonus `*_sinh_dao` cost 1 +10% damage chain kim→thuy→moc→hoa→tho→kim) sau khi #305/#306/#307 chỉ merge vào stacked base branches. MEDIUM PR scope = single objective "complete element symmetry roadmap on main after stacked-merge drift". Catalog metadata-only — KHÔNG đổi runtime/schema/seed. 5-element ceilings: mpMax 1.1⁵ = 1.61051, mpRegen 5×5 = 25 flat, tương sinh 5 distinct elementTargets stack với tương khắc multiplicative (1.15 × 1.1 = 1.265 trên cùng target). 20 vitest mới (995 → 1015 shared): 15 isolation + 4 cumulative + 1 cross multiplicative tương khắc × tương sinh. Risk: 🟢 thấp (catalog +15 entries, deterministic compose pure, runtime stat_mod/regen/damage_bonus paths đã active từ trước). Rollback: revert PR.
- **Latest merged PR**: [#304 feat(catalog,talents): thêm 4 hpRegen producers kim/thuy/hoa/tho hoàn tất 5-element hpRegen coverage (Phase 11.X.AN)](https://github.com/hoathienmenh-01/xuantoi/pull/304) — MEDIUM PR batched 4 producers cho `composePassiveTalentMods.hpRegenFlat`, hoàn tất 5-element hpRegen coverage. 5 vitest mới (990 → 995 shared, +5-element hpRegen ceiling 25 flat). Main bumped `19660ae` → `70337ca`. CI 5/5 GREEN.
