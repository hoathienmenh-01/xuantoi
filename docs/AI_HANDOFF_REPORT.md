...
- **Current `main` commit**: `690648f` (Merge PR #233 feat(combat): wire element multiplier (Ngũ Hành) vào player + monster damage (Phase 11.3.B)).
- **Latest merged PR**: [#233 feat(combat): element multiplier wire (Phase 11.3.B)](https://github.com/hoathienmenh-01/xuantoi/pull/233) — `CombatService.action()` wire `characterSkillElementBonus` (player attack) + `elementMultiplier` (monster counter) + log line tương khắc/sinh / lệch hệ. +5 vitest deterministic. Backward-compat preserved (legacy `spiritualRootGrade=null` → multiplier 1.0).
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track CLOSED** (11/11 catalog foundation merged: PR #221..#231) ✅. **Phase 11.3 PR-A** (#232) + **PR-B** (#233) MERGED ✅. **This PR** = **Phase 11.3 PR-C — Cultivation + Stat bonus wire** (cultivationMultiplier vào tick processor + statBonusPercent vào combat power/def).
- **Current test baseline (main)**: **2690 vitest** = api **1152** + shared **950** + web **588** (post-#233 merge). **This PR** thêm vitest API (3 cultivation processor + 2 combat statBonus = +5) → total ~**2695** sau merge (api 1157 + shared 950 + web 588). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN local.
...
- **Phase 11 progress**: **11.0..11.10.A catalog 11/11 MERGED** ✅. **11.3.A** (#232 SpiritualRootService runtime + Prisma migration) MERGED ✅. **11.3.B** (#233 element multiplier wire CombatService) MERGED ✅. **THIS PR = Phase 11.3 PR-C — Linh căn cultivation + stat bonus wire**: (1) `cultivation.processor.ts` apply `getSpiritualRootGradeDef(grade).cultivationMultiplier` vào exp gain mỗi tick (legacy null → 1.0); (2) `combat.service.ts` apply `(1 + statBonusPercent/100)` vào `effPower` + `effDef`; (3) `+5 vitest` (3 cultivation processor + 2 combat statBonus). **PR 11.1.B/11.2.B/11.3.D (UI display + reroll)/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B/11.8.B/11.9.B/11.10.B (Pending)** runtime hookups tương ứng với từng catalog foundation đã mở.
- **Open PRs**:
  1. **this PR** — `feat(combat,cultivation): wire Linh căn cultivationMultiplier + statBonusPercent (Phase 11.3.C)`.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.3.C) → Linh căn grade ảnh hưởng cả tu luyện exp gain + combat power/def.
  2. **Phase 11.3 PR-D**: UI character profile display Linh căn (icon + grade tooltip + element wheel) + reroll service consume `linh_can_dan` ItemLedger + insert log `source='reroll'`.
  3. **Phase 11.1.B + 11.2.B PR runtime**: Prisma model `CharacterCultivationMethod` + `CharacterSkill { skillKey, masteryLevel, learnedAt }` + service learn/equip/upgrade + UI + wire `applyMasteryEffect` vào CombatService.
  4. **Phase 11.7.B + 11.8.B + 11.9.B + 11.10.B PR runtime** (compose stack): Prisma `CharacterTalent` + `CharacterBuff` + `CharacterTitleUnlock` + `CharacterAchievement { id, characterId, achievementKey, progress, completedAt? }` + service `incrementAchievement` (idempotent) + `claimAchievement` (validate `completedAt!=null` + grant reward + auto-`unlockTitle(rewardTitleKey)`) + event listener cho mỗi `goalKind` (KILL_MONSTER / CLEAR_DUNGEON / BOSS_HIT / BREAKTHROUGH / GAIN_EXP / CULTIVATE_SECONDS / BUY_LISTING / SELL_LISTING / CHAT_MESSAGE / SECT_CONTRIBUTE).
