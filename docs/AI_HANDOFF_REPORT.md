...
## Recent Changes (PR #33→#148 merged; session 9j đóng A→O; session 9k task A merged #149, task C CI xanh #150 pending merge, task D docs PRIVACY+TOS **this PR** in-flight)

### PR session 9k task D (in-flight, this PR) — `docs: bootstrap PRIVACY.md + TOS.md (closed-beta tester agreement)` — **Pending merge**

- **Branch**: `devin/1777542440-docs-privacy-tos`. **Base**: `main` @ `5a815b3` (post PR #149 merge). **Status**: docs-only, in-flight.
- **Vì sao**: Closed beta chuẩn bị mời 50 tester; cần văn bản pháp lý tối thiểu (privacy policy + tester agreement) để (1) nói rõ data retention + quyền xoá data; (2) xác nhận không có thanh toán thật; (3) ghi rõ tester phải tuân thủ (no harassment, no RMT, report bug đúng quy trình); (4) giới hạn liability cho vận hành team hobby. Không có hai file này thì closed beta không có văn bản chấp thuận giữa tester và vận hành team.
- **Files**: `docs/PRIVACY.md` (new — §1 data thu thập + §2 retention 30d chat / permanent ledger + §3 cookies + §4 quyền tester xem/xoá data + §5 security tóm tắt + §6 bên thứ 3 chỉ Postgres/Redis + §7 update + §8 liên hệ); `docs/TOS.md` (new — §1–§13 definition / scope / no payment / data reset / tester behavior / account revocation / no warranty / IP / luật VN / chấm dứt / sửa điều khoản / liên hệ / chấp thuận).
- **Tests added**: 0 (docs-only).
- **CI status (local)**: no code change, nothing to run. `pnpm typecheck` / `pnpm lint` không đụng. CI matrix sẽ verify xanh (docs-only thì `build`/`e2e-smoke` pass như cũ).
- **Risk**: 🟢 thấp — docs-only, không đụng code/schema/seed/test/migration/CI.
- **Rollback**: revert single PR.
- **`AI_HANDOFF_REPORT.md updated`**: header §0 snapshot session 9k task D entry + Recent Changes (this entry) + §20 Roadmap (task D → In-flight) + §21 Exact PR Plan (PR session 9k-D in-flight, plan → executed).

### PR #150 — `test(web): smart AdminView render-level smoke tests — 18 vitest (session 9k task C / admin ops safety)` — **Merged into main** @ `f1214a3` (30/4 ~09:10 UTC, CI ✅ 5/5) — **Resolved**

- **Branch**: `devin/1777541055-adminview-render-tests`. **Merge commit**: `f1214a3`. **Status**: Merged. CI 5/5 xanh sau rerun empty commit trigger flaky `apps/api` chat rate-limit `window trượt` sliding window test (unrelated, timing-based: 50ms window + 80ms sleep).
- **Vì sao**: AdminView.vue là component admin lớn nhất (1695 dòng) + nhiều nhánh gameplay safety (revoke user session, grant currency, approve topup, revoke giftcode). Thiếu render-level test → regression dễ slip. PR thêm 18 vitest cover 5 nhóm behavior cốt lõi: role guard routing, tab badge count, tab switch fetch, Export CSV download, Giftcode revoke ConfirmModal flow.
- **Files**: `apps/web/src/views/__tests__/AdminView.test.ts` (new, 822 line, 5 describe × 18 it).
- **Tests added**: 18 vitest; web baseline `466 → 484` (50 → 51 file).
- **CI status**: build ✅ (x2) · e2e-smoke ✅ (x2) · Devin Review ✅ — tổng 5/5 xanh. Rerun CI sau push empty commit `ci: trigger rerun` để vượt flaky `window trượt` sliding window test (timing test 50ms window + 80ms sleep, known flaky, unrelated to this PR).
- **Risk**: 🟢 thấp — test-only, mock-heavy (18 admin API + boss API + stores + router + AppShell), không touch production code.
- **Rollback**: revert single PR (xoá test file duy nhất).
...
- **Branch**: `devin/1777540074-audit-session-9j-refresh-e342513`. **Merge commit**: `5a815b3`. **Status**: Merged. **Impact**: docs-only audit refresh sync main state sau session 9j cascade; zero code risk.

### PR session 9k task A (merged, PR #149) — `docs(handoff): session 9k kickoff` — **Merged** (giữ entry cũ cho lịch sử)

- **Branch**: `devin/1777540074-audit-session-9j-refresh-e342513`. **Base**: `main` @ `e342513` (post PR #148 merge). **Status**: docs-only, in-flight.
...
| **Web Vitest** | **466 test** (50 file, post session 9j close @ `e342513`) — baseline cũ 187 (23 file) + session 9i +115 (K/K1/K2/F/G tasks: AppShell 15 + HomeView 9 + GiftCodeView 10 + LeaderboardView 10 skeleton + ProfileView 12 + apiError helper 17 + extractApiErrorCode migration) + session 9j +164 (task B Topup 10 + Mail 14 + task D Shop 19 + task E Inventory 15 + task F Auth 14 + task G Onboarding 16 + task H Dungeon 13 + task I Sect 12 + task J NotFound+router 8 + task K Boss 12 + task L Chat+Locale 17 + task M MButton+MToast 14). | Render-level cho `AppShell` nav badge interaction (`breakthroughReady` vừa wire PR #107); runtime smoke for full claim/buy flow + AdminView render-level vitest cho revoke modal flow (session 9k task C). | Low |
...
**Tổng (`vitest run` thực tế, baseline session 9k audit trên main @ `e342513`)**: **~395 test API (chưa chạy local session 9k, cần infra:up) + 96 test shared + 466 test web = ~957 test pass expected**. CI xanh trên main `e342513` (PR #148 ✅). Real Postgres + real Redis service trên CI; local dùng `infra/docker-compose.dev.yml` (`docker compose up -d pg redis`). Web count tăng 187 → 466 qua session 9i (+115) + session 9j (+164). Shared count tăng 55 → 96 qua session 9j task N+O (+19 shop/topup + +22 boss = +41).
...
pnpm --filter @xuantoi/web test        # 466 test (50 file, vitest 2.1.9 happy-dom)
...
A. **Docs audit refresh session 9k** — **In-flight (this PR)** — bump snapshot `2ed8c29 → e342513`, mark PR #134..#148 tất cả Merged, bump baseline web 302 → 466 (35 → 50 file) + shared 55 → 96 (3 → 6 file), sửa PR #136 status (merged stale branch, replay qua #138), thêm session 9k roadmap A→E.
...
C. **AdminView render-level vitest** — **Merged, session 9k task C (PR #150 @ `f1214a3`)** — `apps/web/src/views/__tests__/AdminView.test.ts` (18 vitest) bao phủ: onMounted role guard (unauth/PLAYER/ADMIN/MOD — 4 test) + tab badge rendering (alerts/pendingTopup/activeGiftcode — 4 test) + tab switch fetch (Users/Audit — 2 test) + Export CSV flow (success/truncated/UNAUTHENTICATED — 3 test) + Giftcode revoke ConfirmModal (open/cancel/confirm/CODE_REVOKED/REVOKED+EXPIRED filter — 5 test). Local 484/484, CI 5/5 xanh sau rerun flaky chat rate-limit test. File chính: `apps/web/src/views/__tests__/AdminView.test.ts` (new, 822 line).

D. **Docs — PRIVACY + TOS closed-beta tester agreement** — **In-flight, session 9k task D (this PR)** — `docs/PRIVACY.md` (new §1–§8: data thu thập + retention + cookies + quyền xem/xoá data + security tóm tắt + bên thứ 3 chỉ Postgres/Redis + update policy + liên hệ) + `docs/TOS.md` (new §1–§13: definition, scope, no payment, data reset, hành vi tester, account revocation, no warranty + liability limited, IP, luật VN, chấm dứt beta, sửa điều khoản, liên hệ, chấp thuận). **Risk**: 🟢 thấp — docs-only, không đụng code/schema/seed/test/CI.

E. **Smart QA helper — `pnpm smoke:beta` 5-phút CLI** — **Pending, session 9k task E** — script `scripts/smoke-beta.ts` (Node + fetch): register 1 user random → onboard (chọn sect) → cultivate 60s tick → claim daily login → claim 1 mission (nếu có) → buy 1 shop item LINH_THACH → list inventory (verify item count +1) → đọc 1 mail (nếu có) → mở leaderboard (verify top 50). Exit 0 nếu all pass, exit 1 nếu fail bất kỳ step với stderr diagnostic. Docs cách chạy local trong `docs/QA_CHECKLIST.md`. **Risk**: 🟢 thấp — test script-only, không touch production. File chính: `scripts/smoke-beta.ts` (new), `package.json` root (thêm script `smoke:beta`).
...
#### PR session 9k-A (in-flight, **this PR**) — `docs(handoff): session 9k kickoff — bump snapshot e342513 + mark PR #134..#148 Merged + sync baseline web 466/466 (50 file) + shared 96/96 (6 file) + session 9k roadmap`
- **Branch**: `devin/1777540074-audit-session-9j-refresh-e342513`. **Base**: `main` @ `e342513`. **Status**: docs-only, in-flight.
- **File**: `docs/AI_HANDOFF_REPORT.md` (header §0/§2/Recent Changes/§12 baseline/§20 roadmap session 9k/§21 PR Plan).
- **Tests added**: 0 (docs-only).
- **CI status (local audit)**: typecheck ✅ / lint ✅ / shared 96/96 ✅ / web 466/466 ✅ / build ✅. API skipped (cần infra:up).
- **Risk**: 0 (docs-only).
- **Rollback**: revert single PR.

#### PR session 9k-B (planned, after 9k-A merge) — `test(web,e2e): expand Playwright golden path — E2E_FULL=1 matrix register→cultivate→claim→buy→inventory→mail→leaderboard + optional admin export CSV + admin revoke inventory`
- **Branch**: TBD `devin/<ts>-e2e-full-golden-expand`. **Base**: `main` post-9k-A.
...
#### PR session 9k-C (Merged, PR #150 @ `f1214a3`) — `test(web): smart AdminView render-level smoke tests — 18 vitest (session 9k task C / admin ops safety)`
- **Branch**: `devin/1777541055-adminview-render-tests`. **Merge commit**: `f1214a3`.
- **Status**: Merged. 18 vitest thực tế (vượt plan 12-15). Web baseline `466 → 484` (50 → 51 file).
- **Files**: `apps/web/src/views/__tests__/AdminView.test.ts` (new, 822 line).

#### PR session 9k-D (in-flight, this PR) — `docs: bootstrap PRIVACY.md + TOS.md (closed-beta tester agreement)`
- **Branch**: `devin/1777542440-docs-privacy-tos`. **Base**: `main` @ `5a815b3`, sau đó merge `main` @ `f1214a3` (post PR #150) để resolve conflict ở header §0 + §Recent Changes + §21 PR Plan (3 vùng docs/AI_HANDOFF_REPORT.md).
- **Status**: In-flight. Docs-only.
- **Files**: `docs/PRIVACY.md` (new, closed beta data retention + delete-my-data flow) + `docs/TOS.md` (new, closed beta tester agreement §1–§13).
- **Tests added**: 0 (docs-only).
- **Verify**: `pnpm typecheck` ✅ · `pnpm lint` ✅ (docs không qua ESLint nhưng check an toàn nếu có markdown lint sau này).
- **Risk**: 🟢 thấp — docs-only, không đụng code/schema/seed/test/CI.
- **Rollback**: revert single PR.
