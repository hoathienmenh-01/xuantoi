...
- **Current `main` commit**: `6076fb8` (Merge PR #221 feat(shared): spiritual root catalog foundation (Phase 11.0)).
- **Latest merged PR**: [#221 feat(shared): spiritual root catalog foundation (Phase 11.0)](https://github.com/hoathienmenh-01/xuantoi/pull/221) — `packages/shared/src/spiritual-root.ts` NEW (~340 lines) catalog 5-grade Linh căn (`pham/linh/huyen/tien/than`) + Ngũ Hành helper `elementMultiplier(attacker, defender)` (1.30 tương khắc / 1.20 tương sinh / 0.70 bị khắc / 0.85 bị sinh / 0.90 cùng hệ / 1.00 vô hệ) + `validateSpiritualRootState` + `characterSkillElementBonus`, +43 vitest, 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). **Phase 11 — Progression Depth (Spiritual Root + Cultivation Method + Elemental Combat)** đã mở. **PR #221 (Phase 11.0 Spiritual Root catalog) đã MERGED** vào main `6076fb8`. **This PR (#222)** = **Phase 11.1.A CultivationMethod catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2122 vitest** = api **1133** + shared **401** + web **588** (post-#221 merge). **This PR** thêm **+35 vitest** (shared 401 → 436) → total **2157/2157** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **PR 11.0 (DONE — merged #221)** Spiritual Root catalog foundation — `packages/shared/src/spiritual-root.ts` + 43 vitest, runtime/schema future. **PR 11.1.A (this PR #222 open)** CultivationMethod catalog foundation — `packages/shared/src/cultivation-methods.ts` NEW + 35 vitest, 12 method baseline (1 starter + 5 huyen + 3 tien + 3 than). **PR 11.1.B (Pending)** CultivationMethod runtime (Prisma schema + service + UI). **PR 11.2 (Pending)** SkillTemplate + CharacterSkill DB. **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4..7 (Pending)** Thiên kiếp / Alchemy / Refinery / Talent.
- **Open PRs**: 
  1. **this PR (#222)** — `feat(shared): cultivation method catalog foundation (Phase 11.1.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR (#222)** (Phase 11.1.A CultivationMethod catalog) → unblock Phase 11 runtime track.
  ...
  3. **Phase 11.3 PR runtime** (sau khi 11.0 + 11.1.A + 11.2 catalog ổn): Prisma migration `Character.spiritualRootGrade/primaryElement/secondaryElements/rootPurity/rootRerollCount/cultivationMethodKey` + onboarding roll service + wire `elementMultiplier` + `cultivationMethod.expMultiplier` vào runtime.
