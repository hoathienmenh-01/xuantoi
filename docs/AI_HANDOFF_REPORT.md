...
- **Current `main` commit**: `908678d` (Merge PR #229 feat(shared): buff / debuff catalog foundation (Phase 11.8.A)).
- **Latest merged PR**: [#229 feat(shared): buff / debuff catalog foundation (Phase 11.8.A)](https://github.com/hoathienmenh-01/xuantoi/pull/229) — `packages/shared/src/buffs.ts` NEW catalog 18 buff/debuff baseline (10 buff + 8 debuff × 5 element + 8 source), +56 vitest, 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged) ✅. **Phase 11 — Progression Depth catalog track**: PR #221..#229 (9/9 catalog foundation) MERGED. **This PR** = **Phase 11.9.A Title (Danh hiệu) catalog foundation** — catalog-only KHÔNG runtime/schema, không block.
- **Current test baseline (main)**: **2575 vitest** = api **1133** + shared **854** + web **588** (post-#229 merge). **This PR** thêm **+51 vitest** (shared 854 → 905) → total **2626/2626** sau merge. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **11.0 (#221) / 11.1.A (#222) / 11.2.A (#223) / 11.4.A (#224) / 11.5.A (#225) / 11.6.A (#226) / 11.X.A (#227) / 11.7.A (#228) / 11.8.A (#229) MERGED**. Phase 11 catalog track = **9/9 DONE**. **PR 11.9.A (this PR open)** Title catalog foundation — `packages/shared/src/titles.ts` NEW + 51 vitest, 24 title baseline cover 9 realm milestone (luyenkhi → hu_khong_chi_ton) + 5 element mastery (kim/moc/thuy/hoa/tho) + 4 achievement + 3 sect rank + 2 event + 1 donation, 5 rarity tier `common/rare/epic/legendary/mythic`, schema `TitleDef { key, nameVi, nameEn, description, rarity, source, element, unlockRealmKey?, unlockAchievementKey?, unlockSectRole?, flavorStatBonus? }`, helpers `getTitleDef/titlesByRarity/Source/Element/titleForRealmMilestone/Achievement/SectRole/composeTitleMods` deterministic. **PR 11.1.B/11.2.B/11.3/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B/11.8.B/11.9.B (Pending)** runtime hookups tương ứng với từng catalog foundation đã mở. Phase 11 catalog track = **10/10 DONE** sau khi merge this PR.
- **Open PRs**:
  1. **this PR** — `feat(shared): title (danh hiệu) catalog foundation (Phase 11.9.A)` — catalog-only, không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR** (Phase 11.9.A Title catalog) → Phase 11 catalog foundation track = 10/10 DONE.
  2. **Phase 11.10.A Achievement catalog** (catalog-only safe): `packages/shared/src/achievements.ts` NEW — 30+ achievement baseline reference từ titleForAchievement keys (`first_monster_kill` / `first_dungeon_clear` / `first_boss_kill` / `first_breakthrough` …) + chain.
  ...
  5. **Phase 11.7.B + 11.8.B + 11.9.B PR runtime**: Prisma model `CharacterTalent` + `CharacterBuff` + `CharacterTitleUnlock { id, characterId, titleKey, unlockedAt, source }` + service `unlockTitle`/`equipTitle` + auto-grant trigger trên realm breakthrough event + wire `composeTitleMods` vào `CharacterStatService.computeStats`.
