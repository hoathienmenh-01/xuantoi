...
- **Current `main` commit**: `0119b0f` (Merge PR #208 add `pnpm smoke:economy`, 30/4 ~22:18 UTC).
- **Latest merged PR**: [#208 test: add economy smoke for ledger safety](https://github.com/hoathienmenh-01/xuantoi/pull/208) (test/QA, +1 root script + 3 docs, zero new dep).
- **Current phase**: **Phase 9 — Closed beta stabilization** (audit refresh, smoke:economy ✅, smoke:ws (in-flight — this PR), Playwright golden path, bug triage P0/P1, mobile/i18n polish, release checklist). Xem [`LONG_TERM_ROADMAP.md`](./LONG_TERM_ROADMAP.md) §Phase 9.
...
- **Open PRs**: this PR — `test: add websocket smoke for realtime safety`. Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
...
## Recent Changes (PR #33→#208 đã merged trên main; session 9q-6 **this PR** thêm `pnpm smoke:ws` cho realtime safety trước Phase 10 content scale)

### PR session 9q-6 (in-flight, this PR) — `test: add websocket smoke for realtime safety` — **Pending merge**

- **Branch**: `devin/$(date +%s)-smoke-ws`. **Base**: `main` @ `0119b0f` (Merge PR #208 `pnpm smoke:economy`).
