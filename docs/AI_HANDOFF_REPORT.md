...
- **Current `main` commit**: `761af38` post PR #268 (Phase 11.X.Q Buff control boss). PR #246..#268 MERGED. **PR pending (this session 9r-26 part 5b)**: **feat(boss,character): Buff cultivationBlocked wire vào BossService.attack() (Phase 11.X.R)** — wire `buffMods.cultivationBlocked` (Tâm Ma debuff_taoma) throw `BossError('CULTIVATION_BLOCKED')` BEFORE state mutation. Tâm Ma'd char đã bị block tu luyện EXP (Phase 11.8.D wire ở CultivationProcessor) giờ cũng bị block boss attack — semantically nhất quán. Cùng lần `getMods` với Phase 11.X.Q control check. Add `'CULTIVATION_BLOCKED'` vào `BossError` codes. Map → HTTP 409. 3 vitest mới. Branch `devin/1777729224-phase-11-x-r-boss-cultblock-v2` từ `761af38` (sau PR #269 v1 closed do base branch deleted khi PR #268 merge). Test baseline post-PR-#268 = 2967 → post-this-PR = **2970 vitest** (1428 api + 954 shared + 588 web).
- **Latest merged PR**: [#268 feat(boss,character): Buff control wire vào BossService.attack() (Phase 11.X.Q)](https://github.com/hoathienmenh-01/xuantoi/pull/268) — Wire `buffMods.controlTurnsMax > 0` throw `BossError('CONTROLLED')` BEFORE state mutation in `BossService.attack()`. Inject `@Optional() buffs?: BuffService`. Map CONTROLLED → HTTP 409. Catalog `debuff_root_thuy`/`debuff_stun_tho`/`debuff_silence_kim`. 5 vitest. Main bumped `35db2d5` → `761af38`. CI 5/5 GREEN.
