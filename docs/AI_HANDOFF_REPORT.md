...
- **Current `main` commit**: `a70c733` post PR #263 (docs audit refresh post-PR-#262). PR #246..#263 MERGED. **PR pending (this session 9r-26)**: **Phase 11.X.O Buff control (root/stun/silence) wire vào CombatService.action()** — wire `buffMods.controlTurnsMax > 0` throw `CombatError('CONTROLLED')` ngay sau khi compose buffMods, TRƯỚC mọi state mutation (encounter status / character HP/MP/stamina / ledger không đụng). Catalog `debuff_root_thuy` (3 turns), `debuff_stun_tho` (1 turn), `debuff_silence_kim` (2 turns) — trước đó controlTurnsMax compute nhưng KHÔNG consume runtime, control debuff de facto no-op. 5 vitest mới: stun throw, root throw, DOT no-throw (kind != control), no buff identity, BuffService not injected identity. Branch `devin/1777724550-phase-11-x-o-buff-control-combat`. Test baseline post-PR-#262: **2947 vitest** (1405 api + 954 shared + 588 web). Post-this-PR target: **2952 vitest** (1410 api).
- **Latest merged PR**: [#263 docs(audit): refresh AI_HANDOFF_REPORT post PR #262 merged](https://github.com/hoathienmenh-01/xuantoi/pull/263) — Pure docs/audit refresh. Bump main pointer ec29c2f → a70c733, mark PR #262 MERGED, list 3 next-task candidates (Phase 11.X.K hpMaxMul / 11.X.N shield / 11.X.O control). CI GREEN.
