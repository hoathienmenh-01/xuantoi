...
- **Current `main` commit**: `f208df1` post PR #309 (Phase 11.X.AR — TalentCatalogView read-only `/talents` route MERGED). PR #246..#309 MERGED into main. **PassiveTalentMods catalog symmetry HOÀN TẤT** (7/7 statTarget × 5/5 elements = 35 stat producers + 2/2 damage_bonus chains × 5/5 elements = 10 damage_bonus producers, tương khắc 1.15 + tương sinh 1.1 multiplicative stack 1.265). **Talent UI catalog read-only LIVE**: 54 cards (47 passive + 7 active) tại `/talents` route, filter type/element + effect summary. **1 open PR (this session)**: Phase 11.X.AS — server-authoritative talent learn API + state endpoint (`POST /character/talents/learn` + `GET /character/talents/state`) wire `TalentService` hiện có (Phase 11.7.B MVP) qua `CharacterController`. Baseline test suite @ main `f208df1`: api 1487 vitest, shared 1015 vitest, web 602 vitest = **3104 vitest** GREEN. Local AS work: api 1501 (+14 new vitest cho controller talents endpoints — state success/auth/unwired/no_character/INVALID_REALM + learn ok/INVALID_INPUT/TALENT_NOT_FOUND/ALREADY_LEARNED/REALM_TOO_LOW/INSUFFICIENT_TALENT_POINTS/unwired/auth/no_character), shared 1015, web 602 = **3118 vitest** GREEN (1 pre-existing chat rate-limit flake unaffected). Server-authoritative learn flow ready cho frontend wire (Phase 11.X.AT future).
- **PR pending (this session, branched on main `f208df1`)**: Phase 11.X.AS — feat(api,talents): server-authoritative talent learn API + state endpoint, wire `TalentService` (đã có từ Phase 11.7.B MVP, full CRUD + budget compute + canCharacterLearnTalent + composePassiveTalentMods) qua `CharacterController`. Endpoints mới: (1) `GET /character/talents/state` returns `{ learned: [{ talentKey, learnedAt }], spent, remaining, budget }`; (2) `POST /character/talents/learn` body `{ talentKey }` returns `{ learn: { talentKey, learnedAt }, remaining }`. MEDIUM PR scope = single objective "unblock frontend Học button + budget UI". 3 files: `apps/api/src/modules/character/character.controller.ts` (+import + DTO + 2 endpoints + `mapTalentErrorStatus` + `@Optional() talent?: TalentService` constructor inject), `apps/api/src/modules/character/character.controller.test.ts` (+1 undefined positional cho talent inject), `apps/api/src/modules/character/character.controller.talents.test.ts` (new, 14 vitest). 14 vitest mới api (1487 → 1501). Risk: 🟢 thấp (controller-only, service đã unit-tested, atomic transaction trong service đảm bảo composite UNIQUE chống double-learn race + idempotent budget compute). Rollback: revert PR. Frontend wire Phase 11.X.AT future.
- **Latest merged PR**: [#309 feat(web,talents,ui): TalentCatalogView read-only `/talents` route (Phase 11.X.AR)](https://github.com/hoathienmenh-01/xuantoi/pull/309) — MEDIUM PR expose 54-talent catalog read-only qua route mới `/talents` + sidebar nav `技 Ngộ Đạo` + filter type/element + card layout (name/realm/cost/desc/effect summary). 14 vitest mới (588 → 602 web, full suite 3090 → 3104). Main bumped `a8ece95` → `f208df1`. CI all GREEN.
