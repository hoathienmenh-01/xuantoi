...
- **Current `main` commit**: `19660ae` post PR #303 (Phase 11.X.AM catalog 4 damage_bonus producers tương khắc chain hoàn tất 5-element coverage). PR #246..#303 MERGED. **3 open PRs (stacked)**: PR #304 (Phase 11.X.AN catalog 4 hpRegen producers — CI 5/5 GREEN) + PR #305 (Phase 11.X.AO catalog 5 mpMax producers — CI 5/5 GREEN, stacked on #304) + PR #(this session) (Phase 11.X.AP catalog 5 mpRegen producers **hoàn tất 5-element mpRegen coverage** — stacked on #305). Baseline test suite @ main `19660ae`: api 1487 / shared 990 / web 588 = **3065 vitest** GREEN. Local stacked AN+AO+AP work: shared 1008 (+18 new vitest = 5 hpRegen + 6 mpMax + 7 mpRegen), api 1487, web 588 = **3083 vitest** GREEN. **7/7 statTarget path của `PassiveTalentMods` đã có 5-element coverage** (atkMul + defMul + hpMaxMul + spiritMul + mpMaxMul + hpRegenFlat + mpRegenFlat) — toàn bộ stat_mod + regen path hoàn tất element symmetry.
- **PR #304 (Phase 11.X.AN)**: feat(catalog,talents): thêm 4 hpRegen producers kim/thuy/hoa/tho — hoàn tất 5-element hpRegen coverage. CI 5/5 GREEN. Local 990 → 995 shared. URL: https://github.com/hoathienmenh-01/xuantoi/pull/304
- **PR #305 (Phase 11.X.AO, stacked on PR #304)**: feat(catalog,talents): thêm 5 mpMax producers kim/thuy/moc/hoa/tho — hoàn tất 5-element mpMax coverage, 5/5 stat_mod path complete. CI 5/5 GREEN. Local 995 → 1001 shared. URL: https://github.com/hoathienmenh-01/xuantoi/pull/305
- **PR pending (this session, stacked on PR #305)**: Phase 11.X.AP batched — feat(catalog,talents): thêm `talent_kim_linh_dan` (Kim Linh Đan, kim), `talent_thuy_linh_dan` (Thuỷ Linh Đan, thuy), `talent_moc_linh_dan` (Mộc Linh Đan, moc), `talent_hoa_linh_dan` (Hoả Linh Đan, hoa), `talent_tho_linh_dan` (Thổ Linh Đan, tho) +5 MP regen flat mỗi tick combat — producers thứ 1–5 cho `composePassiveTalentMods.mpRegenFlat`. **Hoàn tất 5-element mpRegen coverage roadmap** (lần đầu fill mpRegenFlat path — trước đó 0/5). MEDIUM PR scope = single objective "complete 5-element mpRegen coverage". Cùng `realmRequirement: 'truc_co'` early-game + `talentPointCost: 1` + `kind: 'regen'` + `statTarget: 'mpMax'` + `value: 5`. Catalog metadata-only — KHÔNG đổi runtime/schema/seed. 5-element mpRegen ceiling = 5 × 5 = 25 flat (additive, đối xứng với hpRegen path Phase 11.X.AN). **2/2 regen path complete + 7/7 statTarget complete**. 7 vitest mới (1001 → 1008 shared): 5 isolation tests + 1 cumulative `combine all 5 mpRegen producers → 25 flat` + 1 cross cumulative `combine 5 hpRegen + 5 mpRegen → 25/25 isolated theo statTarget`. Risk: 🟢 thấp (catalog +5 entries, deterministic compose pure, runtime mpRegen path đã active qua `mpRegenFlat += eff.value` từ trước). Rollback: revert PR.
