...
> **Snapshot (session 9n-E in-flight)**: `main` @ post-PR-#168 (PR #167 economy alerts env thresholds + PR #168 docs TROUBLESHOOTING §15/§16 đã merge 30 Apr 2026 ~12:55 UTC). **Session 9n progress (4 PR merged)**: PR #165 audit refresh · PR #166 audit-ledger CLI `--json` · PR #167 economy alerts env thresholds · PR #168 docs TROUBLESHOOTING. **This PR (9n-E, branch `devin/1777553525-economy-alerts-cli`)**: smart economy-alerts CLI `pnpm --filter @xuantoi/api alerts:economy` parallel với `audit:ledger` — `--json` flag + `--stale-hours=N` flag + 18 vitest unit (parseArgs 11 + formatResult 3 + formatResultJson 4) + extract `queryEconomyAlerts()` pure function khỏi `admin.service.ts` + ADMIN_GUIDE §11.3. Read-only, exit code 0/1/2.
