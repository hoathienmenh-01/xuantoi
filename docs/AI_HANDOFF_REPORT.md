...
- **Current `main` commit**: `817c404` post PR #278 (docs CHANGELOG catch-up 9r-27 part 5). PR #246..#278 MERGED. **PR pending (this session 9r-27 part 6)**: **test(achievements): catalog cross-ref invariants — itemKey + rewardTitleKey (Phase 11.10.G)** — add 2 vitest tới `packages/shared/src/achievements.test.ts`: (1) `mọi reward.items[*].itemKey PHẢI tồn tại trong ITEMS catalog`, (2) `mọi rewardTitleKey PHẢI tồn tại trong TITLES catalog`. Mirror `missions.test.ts:101` referential integrity pattern. Bảo vệ Phase 11.10.D `claimReward` items grant path từ catalog drift — catch broken refs ở build time thay vì runtime claim error. CI baseline 2983 → 2985 vitest (956 shared + 1441 api + 588 web).
- **Latest merged PR**: [#278 docs(CHANGELOG): catch-up session 9r-27 part 5 — PR #276 + #277](https://github.com/hoathienmenh-01/xuantoi/pull/278) — Pure docs/CHANGELOG catch-up cho 2 PR (PR #276 self-catchup + PR #277 Phase 11.10.D Achievement item rewards). Main bumped `ee1765f` → `817c404`. CI 4/4 GREEN.
...
- **Open PRs (state 2/5 2026 session 9r-27 part 4)**: 1 open PR on main = this PR (docs(CHANGELOG): catch-up session 9r-27 — PR #272 + #273 + #274 + #275).

- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 11.9.C Title wire** — compose `TitleService.getMods()` vào stat compute / combat. Auto-unlock title trên breakthrough realm milestone.
  2. **Phase 11.10.D Achievement item rewards** — extract `AchievementModule` + inject `InventoryService`. 32 achievement với item reward.
