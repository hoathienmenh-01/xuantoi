...
> **Snapshot (session 9n-F in-flight, i18n parity fix toast titles)**: `main` @ post-PR-#169 (PR #169 economy-alerts CLI đã merge 30 Apr 2026 ~13:00 UTC). **Session 9n progress (5 PR merged)**: PR #165 audit refresh · PR #166 audit-ledger CLI `--json` · PR #167 economy alerts env thresholds · PR #168 docs TROUBLESHOOTING · PR #169 economy-alerts CLI. **This PR (9n-F, branch `devin/1777554164-i18n-parity-audit`)**: i18n parity fix — `apps/web/src/stores/toast.ts` Pinia store trước đây hard-code VN titles (`'Tin tức' / 'Cảnh báo' / 'Lỗi' / 'Thành công' / 'Thiên Đạo Sứ Giả'`) → giờ dùng `i18n.global.t('toast.title.<type>')` (key đã có sẵn ở `vi.json` + `en.json`). +4 vitest cho i18n locale switch (vi/en). Web vitest baseline 509 → **513**.

> **Snapshot (session 9n-E merged as PR #169)**: `main` @ post-PR-#168 (PR #167 economy alerts env thresholds + PR #168 docs TROUBLESHOOTING §15/§16 đã merge 30 Apr 2026 ~12:55 UTC). PR #169 smart economy-alerts CLI `pnpm --filter @xuantoi/api alerts:economy` parallel với `audit:ledger` — `--json` flag + `--stale-hours=N` flag + 18 vitest unit (parseArgs 11 + formatResult 3 + formatResultJson 4) + extract `queryEconomyAlerts()` pure function khỏi `admin.service.ts` + ADMIN_GUIDE §11.3. Read-only, exit code 0/1/2.
