...
- **Current `main` commit**: `1edabb4` post PR #281 (Phase 11.9.C-2 tribulation title auto-unlock wire). PR #246..#281 MERGED. **PR pending (this session 9r-28 part 3)**: **feat(character): Title auto-unlock onboard `realm_luyenkhi_initiate` wire vào CharacterService.onboard() (Phase 11.9.C-3)** — wire `titles?.unlockTitle(charId, 'realm_luyenkhi_initiate', 'realm_milestone')` fail-soft trong `CharacterService.onboard()` cho character mới (idempotent retry-safe). Đây là phần thứ 3 và cuối cùng của 11.9.C trilogy: PR #280 breakthrough low-tier + PR #281 tribulation high-tier + PR này onboard. Sau PR này, mọi character life-cycle event (creation, breakthrough, tribulation) đều auto-grant title milestone tương ứng. Idempotent qua composite UNIQUE. Fail-soft try/catch — title unlock lỗi KHÔNG fail onboard core path. CI baseline 2994 → 2997 vitest (956 shared + 1453 api + 588 web).
- **Latest merged PR**: [#281 feat(character): Title auto-unlock realm milestone wire vào TribulationService.attemptTribulation() SUCCESS path (Phase 11.9.C-2)](https://github.com/hoathienmenh-01/xuantoi/pull/281) — Inject `TitleService` (3rd positional optional) vào `TribulationService`. Sau `currency.applyTx` trong SUCCESS branch (cùng tx), gate `if (this.titles)` rồi lookup `titleForRealmMilestone(next.key)` → call `titles.unlockTitleTx(tx, charId, def.key, 'realm_milestone')` — atomic-trong-tx. Khác Phase 11.9.C low-tier: KHÔNG fail-soft DB error (rollback toàn bộ tx). 5 vitest mới (1445 → 1450 api). Main bumped `6414796` → `1edabb4`. CI 5/5 GREEN.
- **Previous merged PR**: [#280 feat(character): Title auto-unlock realm milestone wire vào CharacterService.breakthrough() (Phase 11.9.C)](https://github.com/hoathienmenh-01/xuantoi/pull/280) — Inject `TitleService` (6th positional optional) vào `CharacterService`. Sau `prisma.character.update` trong `breakthrough()` thành công, gate `if (this.titles && next)` rồi lookup `titleForRealmMilestone(newRealm)` → call `titles.unlockTitle(charId, def.key, 'realm_milestone')` trong try/catch. Fail-soft. Idempotent qua composite UNIQUE. 4 vitest mới (1441 → 1445 api). Main bumped `5d5c667` → `6414796`. CI 5/5 GREEN.
...
- **Open PRs (state 2/5 2026 session 9r-28 part 3)**: 1 open PR on main = this PR (feat(character): Title auto-unlock onboard `realm_luyenkhi_initiate` wire vào CharacterService.onboard() — Phase 11.9.C-3).

- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 11.7.D Talent active wire** — wire active talent skill cast vào `CombatService.action(skillKey)` lookup.
  2. **Phase 11.8.D Buff → Tribulation Tâm Ma wire** — tribulation FAIL → `applyBuff('debuff_taoma', 'tribulation')` thay cho `Character.taoMaUntil`.
