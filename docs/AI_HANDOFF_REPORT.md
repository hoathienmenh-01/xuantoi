...
- **Current `main` commit**: `af15ffc` post PR #255 (Phase 11.8.D Buff cultivationBlocked wire). PR #246..#255 MERGED. **PR pending (this session)**: **Phase 11.8.E Buff hp/mpRegenFlat wire vào CultivationProcessor** — compose `BuffService.getMods().hpRegenFlat` / `mpRegenFlat` (per-second values) vào cultivation tick HP/MP regen. Buff catalog values là per-second (vd `pill_hp_regen_t1` "+5 HP/giây", `sect_aura_thuy` "+4 MP/giây trong tu luyện"). Mỗi tick = 30s → tổng hồi = `value × tickSeconds`. Cap LEAST(hp+delta, hpMax) qua raw SQL. Refactor `buffMods` fetch ONCE per character per tick (reuse cho cả cultivationBlocked check + regen apply). Tâm Ma block → continue skip toàn bộ (hp/mp regen cũng skip). Inject `@Optional() BuffService` (đã có từ PR #255). 6 vitest mới. Branch `devin/1777717014-phase-11-8-e-buff-hp-mp-regen-cultivation`.
- **Latest merged PR**: [#255 feat(cultivation,character): Buff cultivationBlocked (Tâm Ma) wire vào CultivationProcessor (Phase 11.8.D)](https://github.com/hoathienmenh-01/xuantoi/pull/255) — Wire `buffs.getMods().cultivationBlocked` flag vào cultivation tick. Tâm Ma debuff giờ thực sự block tu luyện runtime. 4 vitest. Main bumped `8123509` → `af15ffc`. CI 5/5 GREEN.
