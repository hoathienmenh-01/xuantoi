...
- **Current `main` commit**: `160d0b4` post PR #314 (Phase 11.10.F achievement reward.items wire MERGED, CI 5/5 GREEN). PR #246..#314 MERGED into main. **This session mở PR Phase 11.5.C UI Refine**: wire UI Luyện Khí (Refine) cho `InventoryView.vue` consume `RefineService` runtime (Phase 11.5.B) đã có ở backend — thêm `refineEquipment(id, useProtection)` API client, refine level badge `+N` trên equipped slot + unequipped item, cost preview (linhThach + material qty từ `getRefineAttemptCost`), per-row protection toggle checkbox, button `Luyện Khí` / `Luyện Khí đỉnh` (disabled khi `refineLevel >= REFINE_MAX_LEVEL=15`), 5 toast outcome (success/failNoLoss/failProtected/failLoss/broken). Server-authoritative — frontend re-fetch `listInventory()` sau success. Local checks GREEN: web 629 → **641 vitest** (+12 Phase 11.5.C: badge visibility, button visibility cho equipment-only, cost label, max disabled, success → re-fetch + toast, protection toggle → API call với `useProtection=true`, failNoLoss/failLoss/broken/protectedFail toast routing, error map INSUFFICIENT_FUNDS, submitting guard double-click), api 1503 unchanged, shared 1019 unchanged. Full suite **3163 vitest** GREEN. **PR Phase 11.5.C mở, đang chờ CI**.
- **Closed (do NOT merge)**: PR #313 [Phase 11.6.B Tribulation/Tâm Ma MVP runtime] — duplicate work, scope đã merge từ trước qua PR #242 (`a2721b1` `feat(character): Tribulation/Tâm Ma MVP runtime — attemptTribulation deterministic via TribulationAttemptLog + CurrencyLedger TRIBULATION_REWARD`) + extended PR #281 (Phase 11.9.C-2 Title milestone wire) + PR #286 (Phase 11.8.D-2 BuffService.applyBuffTx Tâm Ma debuff). Main `TribulationService` 364 lines integrate `BuffService`+`TitleService`, strictly superior. Branch off từ `8e3081e` (trước khi #242..#286+ merged) — merge PR #313 sẽ regress logic của #281+#286. Comment đã post asking user close.
...
- **Immediate next task** (theo thứ tự khuyến nghị):
  ...
  6. **Phase 11.5.C UI Refine** — inventory equip detail refine button.
  7. **Phase 11.10.F Achievement catalog item rewards** — add 1-2 milestone achievements với `reward.items` non-empty để verify wire của PR #277 runtime thực tế (vd `first_market_buy` grant `huyet_chi_dan` x 5; balance review needed).
  8. **Optional smoke gap polish**: `smoke:cultivation` / `smoke:topup` / `smoke:admin` v2 / `smoke:breakthrough`.
