...
- **Current `main` commit**: `c013fb3` (Merge PR #214 feat(shared): item pack 1 (+50 item), 1/5 ~10:05 UTC).
- **Latest merged PR**: [#214 feat(shared): item pack 1 (+50 item)](https://github.com/hoathienmenh-01/xuantoi/pull/214) — Phase 10 PR-1 (item catalog 31 → 81, +21 vitest balance guard, 5/5 CI GREEN).
- **Current phase**: **Phase 10 — Content scale** (PR-2 Skill Pack 1 — Ngũ Hành — in-flight). Phase 9 closed beta stabilization đã APPROVED gate (xem [#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). Phase 10 PR-1..5 plan: items (DONE) / skills (in-flight) / monsters & dungeons / missions / boss catalog — theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md).
- **Current test baseline**: **2006 vitest** = api **1133** + shared **285** (was 259, +26 từ `skills-balance.test.ts` Phase 10 PR-2) + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR).
...
- **Phase 10 progress**: **PR-1 (DONE)** Item Pack 1 — catalog **31 → 81** items (+50). **PR-2 (this PR)** Skill Pack 1 — catalog **10 → 25** skill (+15) tổ chức theo Ngũ Hành (Kim/Mộc/Thuỷ/Hoả/Thổ × ≥1 active + ≥1 passive) + 1 vô hệ early. SkillDef extend forward-compat (`element` / `type` / `role` / `unlockRealm` / `cooldownTurns`) cho phase 11 Spiritual Root + Elemental Combat — combat runtime KHÔNG đổi (vẫn formula `damage = atk * scale - def * 0.5`); `element` chỉ là metadata. Stat budget tuân thủ [`BALANCE_MODEL.md`](./BALANCE_MODEL.md) §4 (atkScale ≤ 5, mpCost ≤ 80, selfHealRatio ≤ 0.5, selfBloodCost ≤ 0.3, cooldownTurns ≤ 6), bound bằng `skills-balance.test.ts` (+26 test). FE picker thêm helper `activeSkillsForSect` để loại passive khỏi dropdown — BossView + DungeonView migrated.
- **Open PRs**: this PR — `feat(shared): skill pack 1 (+15 skill, ngũ hành)`. Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 10 PR-3** — Monster & Dungeon Pack 1 (+20 monster, +6 dungeon) theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md) §4.3. Bây giờ skill Ngũ Hành đã đủ → AI moveset compose theo `element` + `role`.
  2. **Phase 10 PR-4** — Mission Pack 1 (+50 mission daily/weekly/main).
  3. **Phase 10 PR-5** — Boss Catalog (+10 named boss).
  4. **Optional Phase 9.5 polish** — viết `smoke:combat` / `smoke:admin` để cover 2 deferred runtime smoke gap (không block Phase 10).
  5. **Phase 11.0 prep** — schema migration `Character.spiritualRoot` (5-tier element) + lookup `elementMultiplier(skill, target)` để wire `element` field hiện chỉ-metadata thành runtime damage modifier.
