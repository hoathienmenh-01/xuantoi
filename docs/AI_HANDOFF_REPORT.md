...
>
> **Trạng thái (29/4 session 9g, ~19:05 UTC)**: PR #33..#105 đã merge `main` (PR #105 audit refresh task A merged @ `a907eb1`, 29/4 ~19:00 UTC). Toàn bộ Critical/High/Medium đã Resolved trừ M7 (CSP CDN — chỉ verify khi prod deploy), M9 (logout-all `passwordVersion` — intentional trade-off, document trong `docs/SECURITY.md`), M10 (shop daily limit — post-beta nice-to-have). Low: tất cả Resolved (L1..L7 BE; L7 FE đang được đóng nốt bởi PR #106 in-flight). **PR #106 in-flight** (FE admin inventory revoke UI consume PR #66 BE) — sẽ là PR open duy nhất trên GitHub khi push. Roadmap session 9g còn lại task C (smart UX polish + i18n re-audit) + task D (runtime smoke 9d→9f tích hợp).
>
...
## Recent Changes (PR #33→#105 đã merged trên main; PR #106 admin inventory revoke FE in-flight session 9g)

### PR #106 — `feat(web): admin inventory revoke UI — consume PR #66 BE (`POST /admin/users/:id/inventory/revoke`) + 7 vitest helper + i18n vi/en` — **In-flight (Pending merge)** session 9g

- **Branch**: `devin/1777489060-admin-inventory-revoke-fe`. **Base**: `main` @ `a907eb1` (sau PR #105 merge). **Status**: code complete + local typecheck/lint/web test/build all xanh.
...
| **Web Vitest** | **168 test** (21 file) — AdminGiftcodeApi 11 + AdminInventoryRevoke 7 (PR #106) + ConfirmModal 13 + DailyLoginCard 4 + NextActionPanel 6 + OnboardingChecklist 8 + Skeleton 5 + adminGuards 12 (PR #100) + itemName 11 + missionProgress 7 + auth 7 + badges 9 + game 8 + toast 9 + ActivityView 10 + ForgotPasswordView 5 (PR #102) + LeaderboardView 10 (PR #99 +4) + MarketView 3 + MissionView 9 + ResetPasswordView 7 (PR #102) + SettingsView 7 | Render-level cho `AppShell` nav badge interaction; runtime smoke for full claim/buy flow + AdminView render-level vitest cho revoke modal flow (helper-only test đã có PR #106). | Low |
...
**Tổng (`vitest run` thực tế, sau PR #106 in-flight session 9g)**: **~395 test API + 55 test shared + 168 test web = ~618 test pass**. CI xanh trên main `a907eb1` (PR #105 4/4 ✅); PR #106 ETA 5/5 ✅. Real Postgres + real Redis service trên CI; local dùng `infra/docker-compose.dev.yml` (`docker compose up -d pg redis`). Web count tăng 161 → 168 do PR #106 (+7 vitest helper `adminRevokeInventory`, file mới `admin.inventory-revoke.test.ts`).
...
pnpm --filter @xuantoi/web test        # 168 test (21 file, vitest 2.1.9 happy-dom)
...
B. **(IN PROGRESS — PR #106 in-flight)** **FE Admin Inventory Revoke UI** — consume BE `POST /admin/users/:id/inventory/revoke` (PR #66 đã merged) + helper `adminRevokeInventory(...)` trong `apps/web/src/api/admin.ts`. Thêm nút "Thu hồi item" + modal trong `AdminView.vue` cho phép admin nhập itemKey/qty/reason → toast + refresh list. ADMIN-only (`v-if="isAdmin()"` FE + `@RequireAdmin()` BE defense-in-depth). +7 vitest helper. **L7 FE**: đóng nốt phần FE còn lại của L7 (BE đã Resolved by PR #66). i18n vi/en đầy đủ (10 key mới cho mỗi locale).
C. **Smart UX polish + i18n gap audit follow-up** — re-audit hard-coded text trong `.vue/.ts` sau khi thêm các view mới (ForgotPasswordView, ResetPasswordView, LeaderboardView tabs, AdminView revoke modal). Grep regex `['"`].*[À-ỹ].*['"`]` + ensure namespace mới có cả VI và EN. Tích hợp empty-state + skeleton vào ForgotPasswordView/ResetPasswordView nếu thiếu.
D. **Runtime smoke tích hợp full session 9d→9f** — chạy local `pnpm infra:up && pnpm dev`, smoke 15 phút theo `docs/QA_CHECKLIST.md` (register/login → forgot-password flow → reset-password → admin self-demote guard → leaderboard tabs Power/Topup/Sect → admin inventory revoke modal happy/error path → mobile drawer 375px → backup/restore script). Ghi kết quả + bug phát hiện vào `docs/AI_HANDOFF_REPORT.md` Recent Changes; nếu pass đầy đủ, đổi các mục "Needs runtime smoke" trong §6 Completed Features sang "Done".
...
#### PR #106 (session 9g task B, in-flight) — `feat(web): admin inventory revoke UI — consume PR #66 BE (POST /admin/users/:id/inventory/revoke) + 7 vitest helper + i18n vi/en`
- **Branch**: `devin/1777489060-admin-inventory-revoke-fe`. **Base**: `main` @ `a907eb1` (sau PR #105 merge). **Status**: in-flight (this PR), local typecheck/lint/web test 168/168/build all xanh, CI ETA 5/5 ✅ sau khi push.
- **File**: `apps/web/src/api/admin.ts` (+helper `adminRevokeInventory`), `apps/web/src/views/AdminView.vue` (+button "Thu hồi item" + modal + 2 fn `openRevoke`/`submitRevoke`), `apps/web/src/i18n/{vi,en}.json` (+10 key cho mỗi locale), `apps/web/src/api/__tests__/admin.inventory-revoke.test.ts` (mới, +7 vitest).
- **Risk**: thấp — BE đã tồn tại với 16 vitest cover edge case từ PR #66, FE pure additive, không đổi logic cũ. ADMIN-only `v-if` + BE `@RequireAdmin()` defense-in-depth.
- **Rollback**: revert single PR. Không touch schema/migration/seed.
