...
- **Open PRs**: KHÔNG còn open PR sau khi #236 merged (post-audit `92358d1`). Repo đã sẵn sàng để pick task tiếp theo.
- **Immediate next task** (theo thứ tự khuyến nghị, scope nhỏ + server-authoritative + ledger-bắt-buộc cho mọi grant/spend/reward):
  1. **Phase 10 PR-2 v2 — Skill Pack Ngũ Hành expansion** (catalog-only): bổ sung kim_dan-tier ULT skill cho mỗi Ngũ Hành (5 active) + 1 specialized role/passive bù role gap mỗi hệ → catalog từ ~26 → ~36 skill, vẫn trong stat budget `BALANCE_MODEL.md` §4 (atkScale ≤ 5, mpCost ≤ 80, cooldown ≤ 6, selfHealRatio ≤ 0.5, selfBloodCost ≤ 0.3). Catalog-only — Phase 11.2.B `applyMasteryEffect` đã wire generic nên skill mới tự inherit mastery system. Test: unique key, valid element, required fields, mỗi hệ ≥ 2 ACTIVE + ≥ 2 PASSIVE (nâng từ ≥1+≥1). Ưu tiên cao vì user yêu cầu trực tiếp "tiếp tục Phase 10 PR-2 Skill Pack 1 theo Ngũ Hành".
