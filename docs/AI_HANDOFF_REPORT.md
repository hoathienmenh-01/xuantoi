...
- **Current `main` commit**: `c9dc5cf` post PR #253 (Phase 11.9.C Title wire). PR #246..#253 MERGED. **PR pending (this session)**: **Phase 11.7.D Talent expMul wire vào CultivationProcessor** — compose `TalentService.getMods().expMul` vào `CultivationProcessor.process()` tick gain. Catalog (`talent_ngo_dao` "+15% EXP tu vi mỗi lần tu luyện") đã ghi rõ semantic ngộ đạo apply cho cả cultivation EXP (không chỉ monster EXP drop in Phase 11.7.C). Compose multiplicatively với `cultivationMul` (Linh căn) × `methodMul` (Công pháp): `gain = max(1, round(baseGain × cultivationMul × methodMul × talentExpMul))`. Inject `@Optional() TalentService`. Service không inject hoặc character chưa học talent với `exp_bonus` effect → fallback `talentExpMul=1.0` identity. 4 vitest mới. Branch `devin/1777715687-phase-11-7-d-talent-exp-cultivation`.
- **Latest merged PR**: [#253 feat(combat,character): Title flavor wire into CombatService (Phase 11.9.C)](https://github.com/hoathienmenh-01/xuantoi/pull/253) — Wire `titles.getMods()` vào `CombatService.action()`: atkMul × effPower, defMul × effDef, spiritMul × spirit defense. 5 vitest. Main bumped `fb6c451` → `c9dc5cf`. CI GREEN.
