...
- **Current `main` commit**: `19660ae` post PR #303 (Phase 11.X.AM catalog 4 damage_bonus producers tương khắc chain hoàn tất 5-element coverage). PR #246..#303 MERGED. **1 open PR**: PR #(this session) — Phase 11.X.AN catalog 4 hpRegen producers (kim/thuy/hoa/tho) **hoàn tất 5-element hpRegen coverage roadmap** (moc ✅ via `talent_moc_linh_quy` từ trước, + 4 element còn lại). Baseline test suite @ main `19660ae`: api 1487 vitest, shared 990 vitest, web 588 vitest — total **3065 vitest** all GREEN. Local Phase 11.X.AN work: shared 995 (+5 new vitest including 5-element hpRegen ceiling 25), api 1487, web 588 = **3070 vitest** GREEN.
- **PR pending (this session AI handoff continuation, branched on main `19660ae`)**: Phase 11.X.AN batched — feat(catalog,talents): thêm `talent_kim_linh_quy` (Kim Linh Quy, kim), `talent_thuy_linh_quy` (Thuỷ Linh Quy, thuy), `talent_hoa_linh_quy` (Hoả Linh Quy, hoa), `talent_tho_linh_quy` (Thổ Linh Quy, tho) +5 HP regen flat mỗi tick combat — producers thứ 2/3/4/5 cho `composePassiveTalentMods.hpRegenFlat`. **Hoàn tất 5-element hpRegen coverage roadmap**, đối xứng đầy đủ với `talent_moc_linh_quy` (moc) từ trước. MEDIUM PR scope = single objective "complete 5-element hpRegen coverage". Cùng `realmRequirement: 'truc_co'` early-game + `talentPointCost: 1` + `kind: 'regen'` + `statTarget: 'hpMax'` + `value: 5`. Catalog metadata-only — KHÔNG đổi runtime/schema/seed. 5-element hpRegen ceiling = 5 × 5 = 25 flat (additive, khác với atk/def/hpMax/spirit ceiling 1.61051 multiplicative). 5 vitest mới (990 → 995 shared): 4 isolation tests + 1 cumulative `combine all 5 hpRegen producers → 25 flat`. Risk: 🟢 thấp (catalog +4 entries, deterministic compose pure, runtime regen path đã additive qua `hpRegenFlat += eff.value` từ trước). Rollback: revert PR.
