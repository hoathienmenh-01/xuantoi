...
> **Snapshot (session 9k task E — this PR, `pnpm smoke:beta` CLI)**: `main` @ `f1214a3` (Merge PR #150 AdminView render-level 18 vitest, 30 Apr 2026 ~09:10 UTC). **Session 9k task A Merged** — audit refresh docs-only PR #149 @ `5a815b3`. **Session 9k task C Merged** — AdminView render-level 18 vitest PR #150 @ `f1214a3` (web baseline `466 → 484` test, 50 → 51 file). **Session 9k task D Pending merge** — docs PRIVACY.md + TOS.md PR #151 (CI ✅ 5/5, closed beta tester agreement). **In-flight (this PR — session 9k task E)**: `feat(ops): smart QA helper — pnpm smoke:beta ≤2-phút CLI` — `scripts/smoke-beta.mjs` (ESM, zero-dep, 16 step HTTP smoke: healthz → register → session → onboard → character/me → cultivate start/stop → daily-login claim → missions claim → shop buy → inventory before+after → mail read+claim → leaderboard/power → logout) + `package.json` root thêm `"smoke:beta": "node scripts/smoke-beta.mjs"` + `docs/QA_CHECKLIST.md §9` hướng dẫn chạy local + env overrides (`SMOKE_API_BASE`, `SMOKE_TIMEOUT_MS`, `SMOKE_VERBOSE`, `SMOKE_SECT_KEY`, `SMOKE_BUY_ITEM`) + 16 step list + giới hạn (không verify WS tick, không admin flow, không cleanup user test). Pending merge. Baseline: web 484/484, shared 96/96 (unchanged — script-only, không touch code/test). **Session 9k task B pending** — Playwright `E2E_FULL=1` matrix end-to-end (🟡 risk, cần docker infra).
>
...
## Recent Changes (PR #33→#150 merged; session 9j A→O merged; session 9k task A Merged #149, task C Merged #150, task D PR #151 Pending merge, task E **this PR** smoke:beta script In-flight)

### PR session 9k task E (in-flight, this PR) — `feat(ops): smart QA helper — pnpm smoke:beta ≤2-phút CLI (register→cultivate→claim→buy→mail→leaderboard)` — **Pending merge**

- **Branch**: `devin/1777543248-smoke-beta-script`. **Base**: `main` @ `f1214a3` (post PR #150 merge). **Status**: script + docs, in-flight.
- **Vì sao**: §20 Roadmap session 9k task E — closed beta cần smoke CLI nhanh (≤ 2 phút, không cần browser / Playwright) để operator verify API vẫn xanh sau deploy. Bổ trợ cho Playwright `E2E_FULL=1` matrix (task B còn pending) vì không phụ thuộc docker / web build. Zero dependency mới — native Node 20+ fetch + AbortController. Không cleanup user test để đơn giản → note trong `docs/QA_CHECKLIST.md` §9 Giới hạn.
- **Files**: `scripts/smoke-beta.mjs` (new, ESM, ~350 line — cookie jar + HTTP helper + 16 step runner + stderr diagnostic); `package.json` root (thêm script `smoke:beta` → `node scripts/smoke-beta.mjs`); `docs/QA_CHECKLIST.md` (thêm §9 hướng dẫn chạy + env overrides + 16 step list + giới hạn).
- **16 step cover**: healthz → register → session → onboard → character/me → cultivate start → cultivate stop → daily-login/me + claim (skip ALREADY_CLAIMED) → missions/me + claim (skip no-complete) → shop/npc → inventory before → shop/buy (skip no-funds) → inventory after → mail/me + read + claim (skip empty) → leaderboard/power → logout.
- **Env overrides**: `SMOKE_API_BASE`, `SMOKE_TIMEOUT_MS`, `SMOKE_VERBOSE`, `SMOKE_SECT_KEY`, `SMOKE_BUY_ITEM`.
- **Exit code**: 0 all green (bao gồm `SKIP` hợp lệ như ALREADY_CLAIMED / no-funds). 1 nếu bất kỳ step fail, kèm stderr `[smoke] failed steps: - name: reason`.
- **Tests added**: 0 (script + docs). Đã smoke local `node --check` pass; `SMOKE_API_BASE=http://localhost:65535 SMOKE_TIMEOUT_MS=500 node scripts/smoke-beta.mjs` → fail fast đúng pattern (16/17 fail, 1 SKIP, exit 1, diagnostic rõ). Integration test thật (API up + DB seed) pending local infra smoke trước merge.
- **CI status (local)**: `node --check scripts/smoke-beta.mjs` ✅. CI matrix sẽ verify typecheck/lint/test/build không bị regression (smoke script không tham gia CI — chỉ chạy manual).
- **Risk**: 🟢 thấp — script-only, không touch production code / schema / migration / seed. Smoke CLI chỉ gọi public endpoint qua fetch, không có side-effect trên DB khác với 1 user test mới + 1 character + vài shop buy (data trong test env, không ảnh hưởng production).
- **Rollback**: revert single PR (xóa 1 script mới + 1 dòng `package.json` + 1 section `docs/QA_CHECKLIST.md`).
- **`AI_HANDOFF_REPORT.md updated`**: header §0 snapshot + session 9k task E entry (this PR); Recent Changes (this entry) + §20 Roadmap (task E → In-flight; task D Pending merge PR #151; task C Merged) + §21 Exact PR Plan (PR session 9k-E in-flight, plan → executed).

### PR #151 — `docs: bootstrap PRIVACY.md + TOS.md (closed-beta tester agreement, session 9k task D)` — **Pending merge (CI ✅ 5/5)**

- **Branch**: `devin/1777542440-docs-privacy-tos`. **Base**: `main` @ `5a815b3`, merge main @ `f1214a3` để resolve conflict docs/AI_HANDOFF_REPORT.md sau khi PR #150 vào main. **Status**: CI 5/5 xanh (build×2, e2e-smoke×2, Devin Review). Không touch code.
- **Files**: `docs/PRIVACY.md` (new, §1–§8) + `docs/TOS.md` (new, §1–§13).

### PR session 9k task C — `test(web): smart AdminView render-level smoke tests — 18 vitest (session 9k task C / admin ops safety)` — **Merged into main** @ `f1214a3` (30/4 ~09:10 UTC, CI ✅ 5/5) — **Resolved**
...
### Immediate (session 9k — sau khi session 9j đóng toàn bộ task A→O cascade vào main @ `e342513`, 30/4 ~08:55 UTC)

**Top priority (session 9k sẽ tự làm theo thứ tự, **xanh rồi mới qua**)**:

A. **Docs audit refresh session 9k** — **Merged into main PR #149 @ `5a815b3`** (30/4 ~09:00 UTC, CI ✅ 5/5) — bump snapshot `2ed8c29 → e342513`, mark PR #134..#148 tất cả Merged, bump baseline web 302 → 466 (35 → 50 file) + shared 55 → 96 (3 → 6 file), sửa PR #136 status (merged stale branch, replay qua #138), thêm session 9k roadmap A→E.

B. **Beta runtime smoke matrix end-to-end** (Playwright `E2E_FULL=1`) — **Pending, session 9k task B** — mở rộng `apps/web/e2e/golden.spec.ts` cover: register → onboarding → cultivate 60s tick → claim daily login → claim 1 mission → buy 1 shop item → check inventory → đọc 1 mail → mở leaderboard → (optional admin mock) admin user export CSV download + admin revoke inventory. Document trong `docs/QA_CHECKLIST.md` cách chạy local matrix. **Risk**: 🟡 vừa — phụ thuộc infra docker compose; có thể flaky. File chính: `apps/web/e2e/golden.spec.ts`, `apps/web/e2e/fixtures/*`, `.github/workflows/ci.yml`.

C. **AdminView render-level vitest** — **In-flight (this PR — session 9k task C)** — `apps/web/src/views/__tests__/AdminView.test.ts` **18 vitest pass local** bao phủ: (1) onMounted role guard 4 test (unauth redirect `/auth`; PLAYER noPermission toast + `/home` redirect; ADMIN + MOD pass → adminStats + adminEconomyAlerts + adminListTopups(PENDING,0) gọi); (2) tab badge rendering 4 test (`admin-tab-stats-alerts-badge` alertsCount > 0 render / = 0 no render; `admin-tab-topups-pending-badge` pending count; `admin-tab-giftcodes-active-badge` ACTIVE count qua `giftCodeStatusOf`); (3) tab switch fetch 2 test (Users → adminListUsers, Audit → adminListAudit); (4) Export CSV flow 3 test (success toast, truncated warning toast, UNAUTHENTICATED error toast); (5) Giftcode revoke ConfirmModal wiring 5 test (click revoke → modal open với title+code, cancel đóng, confirm → adminRevokeGiftcode + toast success + refreshGiftcodes reload, CODE_REVOKED error → toast i18n `admin.errors.CODE_REVOKED` không dùng handleErr UNKNOWN, mã REVOKED/EXPIRED không render nút revoke). Risk 🟢 thấp. File: `apps/web/src/views/__tests__/AdminView.test.ts` (new, 18 test, 750ms). Baseline web: `466 → 484` (50 → 51 file).

D. **Docs — PRIVACY + TOS closed-beta tester agreement** — **Pending merge, session 9k task D (PR #151 CI ✅ 5/5)** — `docs/PRIVACY.md` (new §1–§8) + `docs/TOS.md` (new §1–§13). Đã resolve conflict với main sau PR #150. Chờ user review/merge.

E. **Smart QA helper — `pnpm smoke:beta` 5-phút CLI** — **In-flight, session 9k task E (this PR)** — `scripts/smoke-beta.mjs` (ESM zero-dep, 16 step HTTP smoke: healthz → register → session → onboard → character/me → cultivate start/stop → daily-login claim → missions claim → shop/npc → inventory before → shop/buy → inventory after → mail read+claim → leaderboard/power → logout) + `package.json` root `"smoke:beta": "node scripts/smoke-beta.mjs"` + `docs/QA_CHECKLIST.md §9` hướng dẫn. Env overrides: `SMOKE_API_BASE`, `SMOKE_TIMEOUT_MS`, `SMOKE_VERBOSE`, `SMOKE_SECT_KEY`, `SMOKE_BUY_ITEM`. Exit 0 all pass (bao gồm SKIP hợp lệ), exit 1 fail + diagnostic. **Risk**: 🟢 thấp — script-only, không touch production / schema / test / CI.

**Backlog (post-9k, an toàn nếu cần thêm)**:
- `M7` CSP CDN prod verify (chỉ khi deploy).
- `M9` document SECURITY.md trường hợp logout-all không bump `passwordVersion` (verify đã có mục đó trong `SECURITY.md`; nếu không → bổ sung).
...
- Performance: page-load benchmark cho `/leaderboard` + `/admin` users tab khi >100 user.
...
#### PR session 9k-D (Pending merge, PR #151, CI ✅ 5/5) — `docs: bootstrap PRIVACY.md + TOS.md (closed-beta tester agreement)`
- **Branch**: `devin/1777542440-docs-privacy-tos`. **Base**: `main` @ `5a815b3`, merge `main` @ `f1214a3` resolve conflict docs/AI_HANDOFF_REPORT.md sau khi PR #150 vào main.
- **Files**: `docs/PRIVACY.md` (new, §1–§8 — data thu thập / retention / cookies / quyền tester / security / bên thứ 3 / update / liên hệ) + `docs/TOS.md` (new, §1–§13 — def / scope / no payment / data reset / hành vi tester / revocation / no warranty / IP / luật VN / chấm dứt / sửa điều khoản / liên hệ / chấp thuận).
- **Risk**: 🟢 thấp — docs-only.
- **Rollback**: revert single PR.

#### PR session 9k-E (in-flight, this PR) — `feat(ops): smart QA helper — pnpm smoke:beta ≤2-phút CLI (register→cultivate→claim→buy→mail→leaderboard)`
- **Branch**: `devin/1777543248-smoke-beta-script`. **Base**: `main` @ `f1214a3` (post PR #150 merge).
- **Files**: `scripts/smoke-beta.mjs` (new ESM zero-dep, ~350 line, 16 step HTTP smoke runner) + `package.json` root (thêm `"smoke:beta": "node scripts/smoke-beta.mjs"`) + `docs/QA_CHECKLIST.md §9` (hướng dẫn chạy + env overrides + step list + giới hạn).
- **Env overrides**: `SMOKE_API_BASE` (default `http://localhost:3000`), `SMOKE_TIMEOUT_MS` (10000), `SMOKE_VERBOSE` (0), `SMOKE_SECT_KEY` (thanh_van), `SMOKE_BUY_ITEM` (huyet_chi_dan).
- **Exit code**: 0 all pass (SKIP hợp lệ = OK). 1 fail + stderr diagnostic.
- **Verify local**: `node --check scripts/smoke-beta.mjs` ✅. `SMOKE_API_BASE=http://localhost:65535 SMOKE_TIMEOUT_MS=500 node scripts/smoke-beta.mjs` → fail fast đúng pattern (exit 1, 16 fail "fetch failed", 1 SKIP no-catalog-entry, diagnostic cụ thể từng step).
- **Risk**: 🟢 thấp — script-only, không touch production code / schema / test / CI. Script không tham gia CI (chạy manual).
- **Rollback**: revert single PR (xóa 1 script + 1 dòng `package.json` + 1 section `docs/QA_CHECKLIST.md`).
