...
- **Current `main` commit**: `153be68` post PR #292 (Phase 11.X.X boss title atkMul/spiritMul wire). PR #246..#292 MERGED, PR #293 (Phase 11.X.AB catalog `talent_moc_linh_co` moc spirit) MERGEABLE / awaiting merge. **No open PR / no pending branch** (this PR is in-flight Phase 11.X.Y — boss talent + buff `damageBonusByElement` wire). Baseline test suite @ main 153be68: api 1481 vitest (85 file), shared 960 vitest (29 file), web 588 vitest (61 file) — total **3029 vitest** all GREEN. Local PR #294 work: api 1484 (+3 new vitest), shared 960, web 588 = 3032 vitest GREEN.
- **PR pending (this session 9r-29 part 8)**: Phase 11.X.Y — feat(boss): Talent + Buff `damageBonusByElement` wire vào BossService.attack() — wire `talentMods.damageBonusByElement.get(skillElement)` × `buffMods.damageBonusByElement.get(skillElement)` multiplicative compose vào `damage = round(raw × talentElementMul × buffElementMul)`. Symmetric với Phase 11.7.C/11.8.C combat path (combat.service.ts L271-283 — combat đã wire). Catalog producer talent: `talent_hoa_tam_dao` (passive damage_bonus 1.15, elementTarget=kim). Catalog producer buff: `buff_*` damage_bonus có `elementTarget`. Skill element=null (basic attack / utility) → identity. Service không inject talents/buffs → `compose*Mods([])` empty Map → identity. Title KHÔNG có `damageBonusByElement` (skip). 3 vitest mới: kim skill (`kim_quang_tram`) + `talent_hoa_tam_dao` damage > baseline, basic skill + talent identity, no-inject TalentService identity. Public surface 100% backward-compat. Risk: 🟡 thấp-trung (boss balance: player kim_dan + `talent_hoa_tam_dao` + skill kim → +15% boss damage; symmetric với combat). Rollback: revert PR.
- **PR mergeable (this session 9r-29 part 7)**: [#293 feat(catalog,talents): thêm `talent_moc_linh_co` moc +10% spirit (Phase 11.X.AB)](https://github.com/hoathienmenh-01/xuantoi/pull/293) — Producer thứ 2 cho `composePassiveTalentMods.spiritMul`, fill 5-element coverage roadmap. Symmetric với `talent_huyen_thuy_tam` (PR #290, thuy). 2 vitest mới (960 → 962 shared). CI 5/5 GREEN, awaiting merge.
- **Latest merged PR**: [#292 feat(boss,character): Title atkMul/spiritMul wire vào BossService.attack() (Phase 11.X.X)](https://github.com/hoathienmenh-01/xuantoi/pull/292) — Inject `TitleService` (10th positional `@Optional()`) và wire `titleMods.atkMul` × `titleMods.spiritMul` multiplicative compose với `talentMods.atkMul`/`spiritMul` × `buffMods.atkMul`/`spiritMul`. Symmetric với Phase 11.X.S/U combat path (PR #274). 4 vitest mới (1477 → 1481 api). Main bumped `300676c` → `153be68`. CI 5/5 GREEN.
- **Previous merged PR**: [#291 feat(boss,character): Buff atkMul/spiritMul wire vào BossService.attack() (Phase 11.X.W)](https://github.com/hoathienmenh-01/xuantoi/pull/291) — Wire `buffMods.atkMul` × `buffMods.spiritMul` multiplicative compose với `talentMods`. Refactor `buffMods` fetch consolidate. 4 vitest mới (1473 → 1477 api). Main bumped `db218b6` → `300676c`. CI 5/5 GREEN.
