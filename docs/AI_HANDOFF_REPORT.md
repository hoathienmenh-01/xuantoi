...
- **Current `main` commit**: `ba1443c` (Merge PR #228 feat(shared): talent / thần thông catalog foundation (Phase 11.7.A)).
- **Latest merged PR**: [#228 feat(shared): talent / thần thông catalog foundation (Phase 11.7.A)](https://github.com/hoathienmenh-01/xuantoi/pull/228) — `packages/shared/src/talents.ts` NEW (~440 lines) catalog 14 talent baseline (7 passive + 7 active Thần Thông), 5 element coverage + 4 neutral, +70 vitest, 5/5 CI GREEN.
- **Current phase**: **Phase 10 — Content scale** (4/5 PR merged, **PR-4 #217 Mission Pack still OPEN ←  this PR / audit fix in-flight**). **Phase 11 — Progression Depth catalog track**: PR #221..#228 (8/8 catalog foundation) **MERGED** vào main `ba1443c`. Audit correction this PR: Exec Summary trên main trước đó claim "Phase 10 5/5 closed" là SAI — PR #217 Mission Pack mở từ session 9r-5 nhưng chưa merge (do branch race với #218 Boss merge trước). Branch `devin/1777634914-phase10-mission-pack-1` rebase từ `ba1443c` resolve conflict docs only, mission catalog content + 27 vitest unchanged.
- **Current test baseline (main)**: **2492 vitest** = api **1133** + shared **771** + web **588** (post-#228 merge, includes talents +70). **This PR** thêm **+27 vitest** (shared 771 → 798) → total **2519/2519** sau merge (mission 12 → 66 + 27 balance vitest). Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
- **Phase 9 readiness** (snapshot session 9r-9 — chi tiết bảng A–O ở [`BETA_CHECKLIST.md`](./BETA_CHECKLIST.md) §"Phase 9 readiness audit"): **11/15 nhóm Done**, **3 nhóm Partial / Needs runtime smoke**, **1 nhóm Needs prod review**. **Phase 10 gate**: ✅ APPROVED. **Phase 9.5 smoke gap CLOSED**: nhóm D (Dungeon / combat / loot) và nhóm J (Admin / topup / giftcode) đều có HTTP smoke. Còn PARTIAL: nhóm C (cultivation breakthrough), nhóm H (mission claim end-to-end), nhóm G (mail UI claim Playwright).
- **Phase 10 progress**: 4/5 PR merged + 1 OPEN — PR-1 #214 items 31→81 ✅, PR-2 #215 skills 10→25 ✅, PR-3 #216 monsters 9→29 + dungeons 3→9 ✅, **PR-4 #217 missions 12→66 OPEN ← this PR (rebased onto main `ba1443c`, CI GREEN, audit correction)**, PR-5 #218 boss 2→12 ✅.
- **Phase 11 progress**: **11.0 (#221) / 11.1.A (#222) / 11.2.A (#223) / 11.4.A (#224) / 11.5.A (#225) / 11.6.A (#226) / 11.X.A (#227) / 11.7.A (#228) MERGED**. Phase 11 catalog track = **8/8 DONE**. **PR 11.1.B/11.2.B/11.3/11.4.B/11.5.B/11.6.B/11.X.B/11.7.B (Pending)** runtime hookups tương ứng với từng catalog foundation đã mở.
- **Open PRs**:
  1. **this PR (#217)** — `feat(shared): mission pack 1 (+54 mission, ngũ hành chain quest)` — Phase 10 PR-4 catalog-only, rebased onto main `ba1443c` để resolve doc conflicts (mission catalog content + 27 vitest unchanged), không đụng runtime, không block.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge this PR (#217)** Mission Pack 1 → Phase 10 closes 5/5 (audit correction).
  2. **Phase 11.8 PR catalog** (nếu còn credit): Buff/Debuff system catalog (`packages/shared/src/buffs.ts`) — catalog-only, low-risk.
