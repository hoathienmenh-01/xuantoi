...
> **Snapshot (session 9k task B — this PR, Playwright golden expand)**: `main` @ `f1214a3` (Merge PR #150 AdminView render-level 18 vitest, 30 Apr 2026 ~09:10 UTC). **Session 9k task A Merged** — audit refresh docs-only PR #149 @ `5a815b3`. **Session 9k task C Merged** — AdminView render-level 18 vitest PR #150 @ `f1214a3` (web `466 → 484`). **Session 9k task D Pending merge** — docs PRIVACY.md + TOS.md PR #151 (CI ✅ 5/5). **Session 9k task E Pending merge** — `pnpm smoke:beta` zero-dep CLI PR #152 (CI ✅ 5/5). **In-flight (this PR — session 9k task B)**: `test(web-e2e): expand golden.spec.ts +3 test (shop buy / mail / profile) — session 9k task B` — bổ sung 3 best-effort Playwright test gate `E2E_FULL=1` (CI default không chạy): (1) `shop buy → inventory reflect new item` — /shop click nút "Mua" đầu (best-effort skip nếu INSUFFICIENT_FUNDS), sau đó /inventory verify render; (2) `mail inbox open → read → claim nếu có reward` — /mail open, nếu có mail row click đọc, nếu có nút "Nhận thưởng" thì claim; (3) `profile /profile/:id public view` — thử click link từ leaderboard; fallback direct `/profile/<fake_id>` verify không crash (NotFound/empty OK). `docs/QA_CHECKLIST.md` update test case list. Risk 🟡 medium — test-only, gated bởi `E2E_FULL=1` nên không chạy CI default (tránh flaky); ops enable local khi muốn verify trước release. File: `apps/web/e2e/golden.spec.ts` (+ ~140 line, 3 test mới) + `docs/QA_CHECKLIST.md` (bullet list update).
>
...
B. **Beta runtime smoke matrix end-to-end** (Playwright `E2E_FULL=1`) — **In-flight, session 9k task B (this PR)** — mở rộng `apps/web/e2e/golden.spec.ts` +3 best-effort test gate `E2E_FULL=1`: (1) `shop buy → inventory reflect new item` (/shop click "Mua", /inventory verify render); (2) `mail inbox open → read → claim nếu có reward` (/mail open, click mail row + "Nhận thưởng" nếu visible); (3) `profile /profile/:id public view` (click link từ leaderboard; fallback direct navigate verify không crash). Tổng 6 test gated (3 baseline 9h-C + 3 mới). Admin flow (export CSV / revoke inventory) không thêm vào E2E_FULL vì cần admin seed riêng — đã cover render-level trong AdminView vitest PR #150. `docs/QA_CHECKLIST.md` cập nhật test case list. **Risk**: 🟡 vừa — test-only, gated bởi `E2E_FULL=1` nên CI default không chạy (tránh flaky); ops enable local khi muốn verify pre-release.
...
D. **Docs — PRIVACY + TOS closed-beta tester agreement** — **Pending, session 9k task D** — `docs/PRIVACY.md` (data retention: account, login logs, chat 30d, currency ledger, item ledger, topup history; delete-my-data flow; analytics scope; 3rd-party — chỉ Postgres/Redis) + `docs/TOS.md` (closed beta tester agreement: scope "beta thử nghiệm riêng tư", no payment, account revocable, no harassment, report-bugs SLA best-effort, liability limited, data backup). **Risk**: 🟢 thấp — docs-only.
...
#### PR session 9k-B (in-flight, this PR) — `test(web-e2e): expand golden.spec.ts +3 test (shop buy / mail / profile)`
- **Branch**: `devin/1777543884-e2e-full-golden-expand`. **Base**: `main` @ `f1214a3`.
- **Files**: `apps/web/e2e/golden.spec.ts` (+ ~140 line, 3 test mới best-effort gated bởi `E2E_FULL=1`) + `docs/QA_CHECKLIST.md` (cập nhật test case list).
- **Tests added**: (1) `shop buy → inventory reflect new item`; (2) `mail inbox open → read → claim nếu có reward`; (3) `profile /profile/:id public view` (click link từ leaderboard; fallback direct navigate).
- **Verify local**: `pnpm lint` ✅ + `pnpm typecheck` ✅ (3 project, ~22s). E2E_FULL run full-stack sẽ chạy local khi có infra (không gate CI).
- **Risk**: 🟡 vừa — test-only, gated bởi `E2E_FULL=1` nên CI default không chạy (AuthView smoke 1 test vẫn chạy, full golden skip).
