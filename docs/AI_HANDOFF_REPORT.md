...
- **Current phase**: **Phase 10 — Content scale CLOSED** (5/5 PR merged). **Phase 9.5 smoke polish gap CLOSED 2/2** (combat #219 + admin #220). **Phase 11 — Progression Depth** đã mở với 2 catalog foundation PR open: **PR #221 Spiritual Root catalog** (Phase 11.0) + **this PR Cultivation Method catalog** (Phase 11.1.A) — cả hai catalog-only KHÔNG runtime/schema, không block nhau (branch độc lập từ main `f0cd240`).
- **Current test baseline (main)**: **2079 vitest** = api **1133** + shared **358** + web **588**. **PR #221** thêm +43 vitest (shared 358 → 401). **This PR** thêm **+35 vitest** (shared 358 → 393 tách riêng từ main). Tổng sau khi cả hai merge: shared **436** → total **2157/2157**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR). **Smoke scripts**: `smoke:beta` / `smoke:economy` (20 step) / `smoke:ws` (19 step) / `smoke:combat` (18 step) / `smoke:admin` (30 step). Tất cả GREEN local (catalog-only PR không đụng runtime API → smoke không cần re-run).
...
- **Phase 11 progress**: **PR 11.0 (open #221)** Spiritual Root catalog foundation — `packages/shared/src/spiritual-root.ts` NEW + 43 vitest. **PR 11.1.A (this PR)** CultivationMethod catalog foundation — `packages/shared/src/cultivation-methods.ts` NEW + 35 vitest, 12 method baseline (1 starter + 5 huyen + 3 tien + 3 than). **PR 11.1.B (Pending)** CultivationMethod runtime (Prisma schema + service + UI). **PR 11.2 (Pending)** SkillTemplate + CharacterSkill DB. **PR 11.3 (Pending)** Linh căn + Thể chất schema migration + onboarding roll service runtime + wire `elementMultiplier` vào `CombatService`. **PR 11.4..7 (Pending)** Thiên kiếp / Alchemy / Refinery / Talent.
- **Open PRs**: 
  1. **PR #221** — `feat(shared): spiritual root catalog foundation (Phase 11.0)` — CI 5/5 GREEN, chờ merge.
  2. **this PR** — `feat(shared): cultivation method catalog foundation (Phase 11.1.A)` — catalog-only, không đụng runtime, không block, không depend PR #221.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Merge PR #221 + this PR** (catalog foundation) → unblock Phase 11 runtime track.
  2. **Phase 11.2 PR**: SkillTemplate catalog static (`packages/shared/src/skill-templates.ts`) trước khi DB-backed runtime — catalog-only, low-risk.
  3. **Phase 11.3 PR runtime** (sau khi 11.0 + 11.1 + 11.2 catalog ổn): Prisma migration `Character.spiritualRootGrade/primaryElement/secondaryElements/rootPurity/rootRerollCount/cultivationMethodKey` + onboarding roll service + wire `elementMultiplier` + `cultivationMethod.expMultiplier` vào runtime.
