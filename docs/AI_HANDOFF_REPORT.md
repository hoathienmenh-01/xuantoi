...
- **Current `main` commit**: `6414796` post PR #280 (Phase 11.9.C breakthrough title auto-unlock wire). PR #246..#280 MERGED. **PR pending (this session 9r-28 part 2)**: **feat(character): Title auto-unlock realm milestone wire vào TribulationService.attemptTribulation() SUCCESS path (Phase 11.9.C-2)** — wire `TitleService.unlockTitleTx(tx, charId, def.key, 'realm_milestone')` atomic-trong-tx sau realm advance + currency grant trong SUCCESS branch của `TribulationService.attemptTribulation()`. Inject `TitleService` (positional optional 3rd param, NestJS DI auto-resolve). Use `titleForRealmMilestone(next.key)` từ shared catalog. Skip nếu (a) `titles` chưa inject (backward-compat), (b) catalog không có title cho realm (vd hoa_than → luyen_hu, luyen_hu → hop_the, hop_the → dai_thua, do_kiep → nhan_tien). Idempotent qua `CharacterTitleUnlock` composite UNIQUE. Khác Phase 11.9.C low-tier: KHÔNG fail-soft DB error (rollback toàn bộ tx) — chỉ catch `TITLE_NOT_FOUND` (catalog drift) → `Logger.warn`. CI baseline 2989 → 2994 vitest (956 shared + 1450 api + 588 web).
- **Latest merged PR**: [#280 feat(character): Title auto-unlock realm milestone wire vào CharacterService.breakthrough() (Phase 11.9.C)](https://github.com/hoathienmenh-01/xuantoi/pull/280) — Inject `TitleService` (6th positional optional) vào `CharacterService`. Sau `prisma.character.update` trong `breakthrough()` thành công, gate `if (this.titles && next)` rồi lookup `titleForRealmMilestone(newRealm)` → call `titles.unlockTitle(charId, def.key, 'realm_milestone')` trong try/catch. Fail-soft: title unlock lỗi KHÔNG fail breakthrough core path. Idempotent qua composite UNIQUE. 4 vitest mới (1441 → 1445 api). Main bumped `5d5c667` → `6414796`. CI 5/5 GREEN.
...
- **Open PRs (state 2/5 2026 session 9r-28 part 2)**: 1 open PR on main = this PR (feat(character): Title auto-unlock realm milestone wire vào TribulationService.attemptTribulation() SUCCESS path — Phase 11.9.C-2).

- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 11.9.C-3 Title auto-unlock — onboard `realm_luyenkhi_initiate`** — wire `titles?.unlockTitle(charId, 'realm_luyenkhi_initiate', 'realm_milestone')` fail-soft trong `CharacterService.onboard()` cho character mới (idempotent retry-safe). Luyenkhi là realm khởi đầu, mọi nhân vật unlock 1 lần. Sau PR này 11.9.C trilogy (breakthrough + tribulation + onboard) hoàn tất.
  2. **Phase 11.7.D Talent active wire** — wire active talent skill cast vào `CombatService.action(skillKey)` lookup.
  3. **Phase 11.8.D Buff → Tribulation Tâm Ma wire** — tribulation FAIL → `applyBuff('debuff_taoma', 'tribulation')` thay cho `Character.taoMaUntil`.
  4. **Phase 11.3.D UI Linh căn** — reroll dialog + element wheel.
  5. **Phase 11.1.C UI cultivation method** — equip switcher.
  6. **Phase 11.2.C UI skill book** — skill grimoire + mastery upgrade.
  7. **Phase 11.5.C UI Refine** — inventory equip detail refine button.
  8. **Phase 11.10.F Achievement catalog item rewards** — add 1-2 milestone achievements với `reward.items` non-empty để verify wire của PR #277 runtime thực tế (vd `first_market_buy` grant `huyet_chi_dan` x 5; balance review needed).
