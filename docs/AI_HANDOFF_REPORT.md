...
> **Snapshot (session 9n-L in-flight, mail.service WS emit + pruneExpired + validateInput edge cases — rebased post-#175)**: `main` @ `9648077` (Merge PR #175 `test(shared) core-types catalog 66 vitest`, 30 Apr 2026 ~16:40 UTC). **Session 9n progress (11 PR merged + 0 in-flight)**: PR #165..#175 merged. **This PR (9n-L, branch `devin/1777566200-mail-ws-prune-tests`)**: test bổ sung cho `apps/api/src/modules/mail/mail.service.ts` — cover 3 area cũ chưa test: (a) WS `mail:new` emit (spy `realtime.emitToUser`) cho `sendToCharacter` (payload `{mailId, subject, senderName, hasReward}`) + `broadcast` (emit đúng 1 frame/mỗi user, không miss/duplicate); (b) `pruneExpired` — xoá thư đã claim cũ hơn `olderThan`, xoá thư expired không reward, **GIỮ thư expired nhưng có reward chưa claim (bao gồm item-only rewards — bugfix sau Devin Review)**; (c) `validateInput` edge cases — subject > 120, body > 2000, rewardItems > 10, item qty <= 0 hoặc âm, itemKey rỗng, rewardExp âm, rewardLinhThach âm, broadcast cũng validate, + boundary test (subject=120/body=2000/items=10 OK). **Bugfix:** `pruneExpired` trước chỉ check `rewardLinhThach/rewardTienNgoc/rewardExp = 0` → item-only expired mail bị xoá nhầm. Fixed bằng `rewardItems.equals = []` trong Prisma JSON filter + thêm test regression. +22 vitest (file mới `mail.service.ws-and-prune.test.ts`). API test baseline 523 → **545**.
