...
- **Current `main` commit**: `b47686f` post PR #260 (docs CHANGELOG catch-up session 9r-25 part 2). PR #246..#260 MERGED. **No PR pending** — session 9r-25 part 2 closed; ready for next safe task. Test baseline: **2940 vitest** (1398 api + 954 shared + 588 web). Phase 11 passive system wire batch (PR #251–#259) hoàn tất, CHANGELOG đã catch-up đến #259 (PR #260). Next candidates còn an toàn: Phase 11.X.M wire `buffs.dotPerTickFlat` (catalog `debuff_burn_hoa` 8 dmg/tick, `debuff_poison_moc` 6 dmg/tick) vào CombatService per-turn HP loss; Phase 11.X.K wire `talents.hpMaxMul` cap stat; Phase 11.X.N wire `buffs.shieldHpMaxRatio` damage absorb pre-HP. **Branch (this PR)**: `devin/1777719889-docs-audit-refresh-9r-25-part2-closeout` — pure docs/audit refresh, no code.
