...
- **Current `main` commit**: `eed0d77` post PR #274 (Phase 11.X.U talent spiritMul wire). PR #246..#274 MERGED. **PR pending (this session 9r-27 part 3)**: **feat(combat,character): Buff invuln wire vào CombatService.action() override damage (Phase 11.X.V)** — wire `buffMods.invulnActive` (kind=invuln) PRE-shield gate trong reply branch (line 410-438) + DOT branch (line 449). Pattern coverage nhất quán với cultivationBlocked (#270) + control (#264) — boolean buff state gates damage path. Identity false hiện tại (catalog chưa có producer cho `kind=invuln`) → zero balance impact. 4 vitest (spy invulnActive=true reply nullified, spy invuln+DOT=8 cả hai skip, no buff identity, no service identity). Branch `devin/1777733092-phase-11-x-v-buff-invuln-combat-wire` từ `eed0d77`. Test baseline post-PR-#274 = 2976 → post-this-PR = **2980 vitest** (1438 api + 954 shared + 588 web).
- **Latest merged PR**: [#274 feat(combat,character): Talent spiritMul wire vào CombatService.action() effSpirit defense calc (Phase 11.X.U)](https://github.com/hoathienmenh-01/xuantoi/pull/274) — Wire `talentMods.spiritMul` × effSpirit ở monster reply branch (line 386-390). Pattern coverage nhất quán với `atkMul/defMul/damageBonusByElement/dropMul/expMul` đã wire (#251). Identity 1.0 hiện tại (catalog talent không producer spirit) → zero balance impact. 3 vitest. Main bumped `30460d3` → `eed0d77`. CI 5/5 GREEN.
...
- **Open PRs (state 2/5 2026 session 9r-27 part 3)**: 1 open PR on main = this PR (feat(combat,character): Phase 11.X.V Buff invuln wire vào CombatService.action() override damage).
