...
- **Current `main` commit**: `a02d728` (Merge PR #213 docs(phase9): audit closed beta readiness after full-stack e2e, 1/5 ~07:21 UTC).
- **Latest merged PR**: [#213 docs(phase9): audit closed beta readiness after full-stack e2e](https://github.com/hoathienmenh-01/xuantoi/pull/213) (docs-only — Phase 9 readiness table A–O, Phase 10 gate decision APPROVED, stale test count fix). CI 4/4 GREEN.
- **Current phase**: **Phase 10 — Content scale** (PR-1 Item Pack 1 in-flight). Phase 9 closed beta stabilization đã APPROVED gate (xem [#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). Phase 10 PR-1..5 plan: items / skills / monsters & dungeons / missions / boss catalog — theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md).
- **Current test baseline**: **1980 vitest** = api **1133** + shared **259** (was 238, +21 từ `items-balance.test.ts` Phase 10 PR-1) + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR — verified GREEN trên main HEAD sau #212).
...
- **Phase 10 progress**: PR-1 Item Pack 1 — catalog **31 → 81** items (+50). Distribution: WEAPON 4→12, ARMOR 4→10, BELT 3→6, BOOTS 3→6, HAT 3→6, TRAM 2→5, ARTIFACT 4→8, PILL_HP 3→6, PILL_MP 2→5, PILL_EXP 2→5, ORE 1→7, MISC 0→5. Stat budget tuân thủ [`BALANCE_MODEL.md`](./BALANCE_MODEL.md) §3.3 (PHAM atk≤10 / LINH≤25 / HUYEN≤60 / TIEN≤200 / THAN≤800), bound bằng `items-balance.test.ts` (+21 test). Kèm fix nhỏ test pollution: `email.service.test.ts` không còn lệ thuộc `apps/api/.env` leak vào `process.env` (xem session 9r-3 §Fix tangential).
- **Open PRs**: this PR — `feat(shared): item pack 1 (+50 item)`. Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 10 PR-2** — Skill Pack 1 (+15 skill) theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md) §4.2 + [`BALANCE_MODEL.md`](./BALANCE_MODEL.md) §4 (skill power budget). Pre-gate: `pnpm smoke:economy` 20/0 + `pnpm smoke:ws` 19/0 + `E2E_FULL=1 e2e` 16/16.
  2. **Phase 10 PR-3** — Monster & Dungeon Pack 1 (+20 monster, +6 dungeon). Sau khi PR-1 + PR-2 merge (skill cần để compose enemy moveset).
  3. **Phase 10 PR-4** — Mission Pack 1 (+50 mission daily/weekly/main).
  4. **Phase 10 PR-5** — Boss Catalog (+10 named boss).
  5. **Optional Phase 9.5 polish** — viết `smoke:combat` / `smoke:admin` để cover 2 deferred runtime smoke gap (không block Phase 10).
