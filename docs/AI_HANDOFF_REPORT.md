...
- **Current `main` commit**: `10f463e` post PR #258 (Phase 11.4.D Equip spiritBonus wire combat). PR #246..#258 MERGED. **PR pending (this session)**: **Phase 11.X.G Talent dropMul wire vào BossService reward distribution** — wire `talents.getMods().dropMul` × linhThach reward in `distributeRewards`. Catalog `talent_thien_di` (drop_bonus +20%) etc apply multiplicatively per-character on top1/top23/top410/rest reward share. Inject `@Optional() talents?: TalentService` vào BossService constructor. CharacterModule đã exports TalentService nên không cần module changes. CurrencyLedger reflects boosted delta (single source of truth — BOSS_REWARD ledger row's delta = boosted amount, không phải base amount). 3 vitest mới: talent_thien_di → 6000 thay vì 5000, no talent → identity (5000), no service inject → identity (5000). Branch `devin/1777718781-phase-11-x-g-talent-dropmul-boss-reward`.
- **Latest merged PR**: [#258 feat(combat,inventory): Equip spiritBonus wire vào CombatService.action() (Phase 11.4.D)](https://github.com/hoathienmenh-01/xuantoi/pull/258) — Wire `inventory.equipBonus.spiritBonus` vào `effSpirit` defense calc. Pattern same as atk wire `(base + flat) × multipliers`. 2 vitest. Main bumped `09a1bc5` → `10f463e`. CI 5/5 GREEN.
