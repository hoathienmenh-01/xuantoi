...
- **Current `main` commit**: `42a15fc` post PR #250 (hotfix typecheck red). PR #246 + #247 + #248 + #249 + #250 MERGED. **PR pending (this session 9r-20)**: **PR #251** Phase 11.7.C Talent passive wire — compose `talents.getMods()` vào `CombatService.action()` stat compute. Wire 5 mods: `atkMul` × effPower, `defMul` × effDef, `damageBonusByElement[skill.element]` × dmg, `expMul` × monster.expDrop, `dropMul` × monster.linhThachDrop. Inject `@Optional() TalentService`. Service không inject → fallback `composePassiveTalentMods([])` identity (1.0 all mul, 0 flats). 4 vitest mới. Branch `devin/1777706753-phase-11-7-c-talent-wire-combat`.
- **Latest merged PR**: [#250 hotfix(test): fix combat.service.test.ts AchievementService constructor — main typecheck red after PR #249 merge](https://github.com/hoathienmenh-01/xuantoi/pull/250) — Test fixture 3-arg `new AchievementService(prisma, currency, title)` matches PR #248's extended signature. Main bumped `57d43b9` → `42a15fc`. CI 4/4 GREEN.
...
- **Current test baseline (main `a2ce4f8` post PR #247 merged)**: **~2907 vitest** = api **1365** (post-PR-#247 baseline + 9 new claimReward tests + adjustments — verified locally vs Postgres dev) + shared **954** + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. **Smoke scripts**: `smoke:beta` / `smoke:economy` / `smoke:ws` / `smoke:combat` / `smoke:admin`. Tất cả GREEN trên main per CI run của PR #247. **Local verified session 9r-19 (Phase 11.10.C-1 branch pre-PR)**: api `pnpm test` 1365/1365 ✓ (Postgres infra lên), shared `pnpm test` 954/954 ✓, web `pnpm test` 588/588 ✓, typecheck ✓ (api+shared+web no error), lint ✓ (zero warnings), build ✓ (shared+web dist OK). **Pre-existing flaky test**: `chat.service.test.ts > window trượt` (50ms timing window — flake intermittently — NOT regression; passes on rerun).
...
- **Open PRs**: 0 open PR. Current session = audit-only PR (this) — `docs/AI_HANDOFF_REPORT.md` sync sau khi PR #240 (Phase 11.5.B Refine MVP) đã merged vào main `8e3081e`.
- **Immediate next task** (theo thứ tự khuyến nghị, scope nhỏ + server-authoritative + ledger-bắt-buộc cho mọi grant/spend/reward):
  1. **Phase 11.6.B Tribulation/Tâm Ma MVP runtime** (per spec `XuanToi_AI_Build_Functional_Spec.docx` §P11-6 + `packages/shared/src/tribulation.ts` catalog đã có): Prisma `TribulationAttemptLog { id, characterId, realmKey, attemptIndex, result (PASS/FAIL_LIGHT/FAIL_HEAVY), rngSeed, deltaExpStage9, mpDrain, createdAt }` + `TribulationService.attemptBreakthrough(charId, realmKey, useItems[], rng?)` server-authoritative deterministic RNG (inject `rng?` cho test) tích hợp vào `BreakthroughService` chỉ cho realm ≥ `nguyen_anh` (theo spec §5.1) — manual breakthrough risk realm cao, fail nhẹ cooldown / fail nặng mất phần EXP stage 9, KHÔNG multi-phase kiếp ngay. Wire `CurrencyLedger.TRIBULATION_COST` + `ItemLedger.TRIBULATION_PILL`. Test 10+ vitest: success deterministic / fail safe / fail heavy / EXP loss bounded / cooldown enforce / item consume / realm gate.
  2. **Phase 11.7.B Talent runtime** (per `packages/shared/src/talents.ts` catalog): Prisma `CharacterTalent { id, characterId, talentKey, level, learnedAt }` + `TalentService.learnTalent(charId, talentKey)` consume `talent_essence` qua `ItemLedger` + `upgradeTalent` cap by realm. Wire `composeTalentBonus` vào `equipBonus()` (sau socket+refine multiplier).
  3. **Phase 11.8.B Buff runtime** (per `packages/shared/src/buffs.ts` catalog): Prisma `CharacterBuff { id, characterId, buffKey, expiresAt, source }` + `BuffService.applyBuff` (idempotent stacking rule) + `BuffSweeper` cron clear expired + wire `composeBuffBonus` vào `CombatService.computeAttackerStat()`.
  4. **Phase 11.9.B Title runtime** (per `packages/shared/src/titles.ts` catalog): Prisma `CharacterTitleUnlock { id, characterId, titleKey, unlockedAt }` + `TitleService.unlockTitle` (idempotent) + `setActiveTitle` (single active) + wire `composeTitleBonus` vào `equipBonus()`.
  5. **Phase 11.10.B Achievement runtime** (per `packages/shared/src/achievements.ts` catalog): Prisma `CharacterAchievement { id, characterId, achievementKey, progress, completedAt? }` + service `incrementAchievement` (idempotent) + `claimAchievement` (validate `completedAt!=null` + grant reward + auto-`unlockTitle(rewardTitleKey)`) + event listener cho mỗi `goalKind` (KILL_MONSTER / CLEAR_DUNGEON / BOSS_HIT / BREAKTHROUGH / GAIN_EXP / CULTIVATE_SECONDS / BUY_LISTING / SELL_LISTING / CHAT_MESSAGE / SECT_CONTRIBUTE).
  6. **Phase 11.3.D UI Linh căn**: UI character profile display Linh căn (icon + grade tooltip + element wheel) + reroll dialog consume `linh_can_dan` qua server-authoritative `SpiritualRootService.reroll(charId, item)` + insert `SpiritualRootRollLog source='reroll'`. Frontend chỉ render + gửi action.
  7. **Phase 11.1.C UI cultivation method**: UI character profile display equipped method + list learned + equip switcher (call `POST /character/cultivation-method/equip`). Optional 24h cooldown anti-spam re-equip.
  8. **Phase 11.2.C UI skill book**: UI skill book / skill grimoire display learned skills + equip switcher (call `POST /character/skill/equip`) + mastery upgrade button (call `POST /character/skill/upgrade-mastery` với cost preview). Item-based skill book drop + consume via `ItemLedger` để wire kind='item' SkillUnlockRequirement.
  9. **Phase 11.5.C UI Refine** (sau 11.6.B nếu cần thiết, hoặc làm song song): inventory equip detail dialog show `refineLevel` badge + “Luyện khí” button gọi `POST /character/refine` với material qty preview + protection charm toggle + outcome animation (success/fail/break).
  10. **Optional smoke gap polish**: `smoke:cultivation` / `smoke:topup` / `smoke:admin` v2 / `smoke:breakthrough` (gắn 11.6.B runtime ngay khi có).
