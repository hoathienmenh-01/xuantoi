...
- **Current `main` commit**: `19660ae` post PR #303 (Phase 11.X.AM catalog 4 damage_bonus producers tương khắc chain hoàn tất 5-element coverage). PR #246..#303 MERGED. **2 open PRs (stacked)**: PR #304 (Phase 11.X.AN catalog 4 hpRegen producers kim/thuy/hoa/tho — CI 5/5 GREEN, mergeable=clean) + PR #(this session) (Phase 11.X.AO catalog 5 mpMax producers kim/thuy/moc/hoa/tho **hoàn tất 5-element mpMax coverage** — stacked on PR #304). Baseline test suite @ main `19660ae`: api 1487 / shared 990 / web 588 = **3065 vitest** GREEN. Local Phase 11.X.AN+AO stacked work: shared 1001 (+11 new vitest = 5 hpRegen + 6 mpMax), api 1487, web 588 = **3076 vitest** GREEN. **5/5 stat_mod path đã có 5-element coverage** (atk + def + hpMax + spirit + mpMax) — toàn bộ statTarget của `PassiveTalentMods` đã hoàn tất element symmetry.
- **PR #304 (Phase 11.X.AN, stacked beneath PR for AO)**: feat(catalog,talents): thêm `talent_kim_linh_quy`, `talent_thuy_linh_quy`, `talent_hoa_linh_quy`, `talent_tho_linh_quy` +5 HP regen flat — hoàn tất 5-element hpRegen coverage. CI 5/5 GREEN. Local 990 → 995 shared. URL: https://github.com/hoathienmenh-01/xuantoi/pull/304
- **PR pending (this session, stacked on PR #304)**: Phase 11.X.AO batched — feat(catalog,talents): thêm `talent_kim_linh_hai` (Kim Linh Hải, kim), `talent_thuy_linh_hai` (Thuỷ Linh Hải, thuy), `talent_moc_linh_hai` (Mộc Linh Hải, moc), `talent_hoa_linh_hai` (Hoả Linh Hải, hoa), `talent_tho_linh_hai` (Thổ Linh Hải, tho) +10% MP tối đa — producers thứ 1–5 cho `composePassiveTalentMods.mpMaxMul`. **Hoàn tất 5-element mpMax coverage roadmap** (lần đầu fill mpMaxMul path — trước đó 0/5). MEDIUM PR scope = single objective "complete 5-element mpMax coverage". Cùng `realmRequirement: 'kim_dan'` + `talentPointCost: 1` + `kind: 'stat_mod'` + `statTarget: 'mpMax'` + `value: 1.1`. Catalog metadata-only — KHÔNG đổi runtime/schema/seed. 5-element mpMax ceiling = 1.1⁵ = 1.61051, khớp atk/def/hpMax/spirit ceiling. **5/5 stat_mod path complete**. 6 vitest mới (995 → 1001 shared): 5 isolation tests + 1 cumulative `combine all 5 mpMax producers → 1.61051`. Risk: 🟢 thấp (catalog +5 entries, deterministic compose pure, runtime mpMax stat_mod path đã active qua `mpMaxMul *= eff.value` từ trước). Rollback: revert PR.
