...
> **Snapshot (session 9n-G in-flight, i18n parity fix api fallback errors)**: `main` @ post-PR-#170 (PR #170 toast i18n title merged 30 Apr 2026 ~13:15 UTC). **Session 9n progress (6 PR merged)**: PR #165 audit refresh · PR #166 audit-ledger CLI `--json` · PR #167 economy alerts env thresholds · PR #168 docs TROUBLESHOOTING · PR #169 economy-alerts CLI · PR #170 toast i18n title. **This PR (9n-G, branch `devin/1777554636-i18n-api-fallback-errors`)**: i18n parity fix cho fallback Error messages ở `apps/web/src/api/{auth,shop,character}.ts` — trước đây hard-code VN ('Đăng ký thất bại' / 'Đăng nhập thất bại' / 'Đổi mật khẩu thất bại' / 'Gửi yêu cầu thất bại' / 'Đặt lại mật khẩu thất bại' / 'Logout all thất bại' / 'Lấy shop thất bại' / 'Mua thất bại' / 'Onboard thất bại') → thêm i18n keys `common.apiFallback.{register,login,changePassword,forgotPassword,resetPassword,logoutAll,shopLoad,shopBuy,onboard}` ở vi.json + en.json + helper `fallbackError(op)` dùng `i18n.global.t(...)`. +19 vitest cover cả 2 locale + BE error precedence. Web vitest baseline 513 → **532**.
