...
- **Current `main` commit**: `1163552` (Merge PR #215 feat(shared): skill pack 1 (+15 skill, ngũ hành), 1/5 ~10:55 UTC).
- **Latest merged PR**: [#215 feat(shared): skill pack 1 (+15 skill, ngũ hành)](https://github.com/hoathienmenh-01/xuantoi/pull/215) — Phase 10 PR-2 (skill catalog 10 → 25, SkillDef forward-compat extends, +26 vitest balance guard, 5/5 CI GREEN).
- **Current phase**: **Phase 10 — Content scale** (PR-3 Monster & Dungeon Pack 1 — Ngũ Hành — in-flight). Phase 9 closed beta stabilization đã APPROVED gate (xem [#213](https://github.com/hoathienmenh-01/xuantoi/pull/213)). Phase 10 PR-1..5 plan: items (DONE) / skills (DONE) / monsters & dungeons (in-flight) / missions / boss catalog — theo [`CONTENT_PIPELINE.md`](./CONTENT_PIPELINE.md).
- **Current test baseline**: **2051 vitest** = api **1133** + shared **330** (was 285, +45 từ `monsters-balance.test.ts` 23 + `dungeons-balance.test.ts` 22 — Phase 10 PR-3) + web **588**. Playwright `E2E_FULL=1` golden path = **16 spec / 16 pass** trong ~20–25s. Workflow `e2e-full.yml` gated path-filter + manual `workflow_dispatch` (NOT required every PR).
...
- **Phase 10 progress**: **PR-1 (DONE)** Item Pack 1 — catalog **31 → 81** items (+50). **PR-2 (DONE)** Skill Pack 1 — catalog **10 → 25** skill (+15, Ngũ Hành). **PR-3 (this PR)** Monster & Dungeon Pack 1 — monster catalog **9 → 29** (+20, Ngũ Hành kim/moc/thuy/hoa/tho × BEAST/HUMANOID/SPIRIT/ELITE/BOSS), dungeon catalog **3 → 9** (+6 element-thematic + 1 single-boss endgame), DUNGEON_LOOT **3 → 9** entries, ITEMS **81 → 87** (+6 yêu phù mới cho 6 dungeon mới). MonsterDef + DungeonDef extend forward-compat (`element` / `monsterType` / `regionKey` / `dailyLimit`) cho phase 11.3 elementMultiplier + phase 11.5 DungeonRun service — combat runtime KHÔNG đổi. Stat budget tuân thủ [`BALANCE_MODEL.md`](./BALANCE_MODEL.md) §5 (hp ≤ 200×level, atk ≤ 25×level, def ≤ 8×level, stamina luyenkhi ≤ 15 / truc_co ≤ 30 / kim_dan ≤ 40 / nguyen_anh ≤ 65), bound bằng `monsters-balance.test.ts` + `dungeons-balance.test.ts`. 4 helper mới: `monstersByElement` / `dungeonsByElement` / `monstersByRegion` / `dungeonsByRegion`.
- **Open PRs**: this PR — `feat(shared): monster & dungeon pack 1 (+20 monster, +6 dungeon, ngũ hành)`. Xem [PR list](https://github.com/hoathienmenh-01/xuantoi/pulls) để check live.
- **Immediate next task** (theo thứ tự khuyến nghị):
  1. **Phase 10 PR-4** — Mission Pack 1 (+50 mission daily/weekly/main, theo Ngũ Hành element bonus + chain quest). Skill + monster + dungeon đã đủ → mission có thể reference deterministic.
  2. **Phase 10 PR-5** — Boss Catalog (+10 named boss). Bây giờ MonsterType=BOSS đã có → catalog boss có thể reuse `MonsterDef` schema, chỉ thêm `BossDef` superset cho damage tier reward + pity.
  3. **Optional Phase 9.5 polish** — viết `smoke:combat` / `smoke:admin` để cover 2 deferred runtime smoke gap (không block Phase 10).
