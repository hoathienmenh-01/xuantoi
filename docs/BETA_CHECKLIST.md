...
| M | Smoke scripts | **DONE** | — | smoke:beta 16-step (`scripts/smoke-beta.mjs`), smoke:economy 20-step (`scripts/smoke-economy.mjs`), smoke:ws 19-step (`scripts/smoke-ws.mjs`), **smoke:combat 18-step (`scripts/smoke-combat.mjs`, session 9r-6)**. All gated/manual, KHÔNG vào CI vì cần API + DB + Redis live cùng process. | — |
