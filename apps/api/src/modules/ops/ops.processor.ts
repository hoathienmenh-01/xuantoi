...
export class OpsProcessor extends WorkerHost {
  ...
  async process(job: Job): Promise<void> {
    ...
    const mailClaimedCutoff = new Date(
      Date.now() - MAIL_CLAIMED_TTL_DAYS * 24 * 60 * 60 * 1000,
    );
    ...
    try {
      ...
      const deletedMails = await this.mail.pruneExpired(mailClaimedCutoff);

      this.logger.log(
        `prune done — LoginAttempt: ${deletedAttempts.count}, RefreshToken: ${deletedRefresh.count}, Mail: ${deletedMails}`,
      );
    } catch (e) {
